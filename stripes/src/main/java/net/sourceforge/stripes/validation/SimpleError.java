 ...
 * parameters are provided by default, they are the field name and field value, and are indices 0
 * and 1 respectively. To use replacement parameters a message must contain the replacement token
 * {#} where # is the numeric index of the replacement parameter.
 *
 * <p>For example, to construct an error message with one additional replacement parameter which is
 * the action the user was trying to perform, you might supply a message like:
 *
...
public class SimpleError extends SimpleMessage implements ValidationError {
  ...
  /**
   * Helper method that is used to widen the replacement parameter array to make room for the two
   * standard parameters, the field name and field value.
   *
  ...
  /**
   * Takes the form field name supplied and tries first to resolve a String in the locale specific
   * bundle for the field name, and if that fails, will try to make a semi-friendly name by parsing
   * the form field name. The result is stored in replacementParameters[0] for message template
    ...
    if (this.fieldNameKey == null) {
      ...
      getReplacementParameters()[0] =
          LocalizationUtility.getLocalizedFieldName(
              this.fieldNameKey, this.actionPath, this.beanclass, locale);
  ...
  /**
   * Sets the name of the form field in error. This is the programmatic name, and hence probably not
  ...
  /**
   * Generated equals method that ensures the message, field, parameters and action path are all
   * equal.
   */
  @Override
