 ...
 * specification of the most common types of validation logic, as well as a way to specify custom
 * validations.
 *
 * @author Tim Fennell
 */
...
public @interface Validate {
  ...
  /**
   * If true, then a parameter value to be bound to this field must be an encrypted string. It also
  ...
  /**
   * If required=true, restricts the set of events to which the required check is applied. If
   * required=false (or omitted) this setting has <i>no effect</i>. This setting is entirely
  ...
  /**
   * Specifies an expression in the JSP expression language that should be evaluated to check the
   * validity of this field. In the case of lists, arrays and maps the expression is evaluated once
   * for each value supplied. The expression is evaluated <i>only if a value is supplied</i> - it
   * will not be evaluated if the user did not supply a value for the field.
   *
   * <p>The value being validated is available in the EL variable 'self'. Properties of the
   * ActionBean (including the context) can be referenced directly, as can values in request and
