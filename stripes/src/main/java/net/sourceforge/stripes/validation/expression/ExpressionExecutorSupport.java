...
/**
 * A base class that provides the general plumbing for running expression validation using the old
 * JSP 2.0 style ExpressionEvaluator. Uses a custom VariableResolver to make fields of the
 * ActionBean available in the expression.
 *
 * @author Tim Fennell
 * @since Stripes 1.5
 */
...
public abstract class ExpressionExecutorSupport implements ExpressionExecutor {
  ...
  protected static class BeanVariableResolver implements VariableResolver {
    ...
    /**
     * Recognizes a couple of special variables, and if the property requested isn't one of them,
     * just looks up a property on the action bean.
     *
  ...
  /**
   * Must be implemented by subclasses to return an instance of ExpressionEvaluator that can be used
  ...
  /**
   * Utility method for checking deprecated use of 'this' in expressions. Checks if <code>prop
   * </code> == 'this' and logs a Warning message inviting the user to update to the new keyword.
   */
  static boolean isSelfKeyword(ActionBean bean, Object prop) {
    ...
    if (isDeprecatedThis) {
      ...
      log.warn(
          "You are using the 'this' keyword in ActionBean class '"
              ...
              + SELF
              + "' in replacement of 'this' in your EL expressions.");
    }
