 ...
 * and their superclasses for properties annotated with {@link Validate} and/or
 * {@link ValidateNestedProperties} and exposes the validation metadata
 * specified by those annotations. When searching for annotations, this
 * implementation looks first at the property's read method (getter), then its
 * write method (setter), and finally at the field itself.
 *
...
public class DefaultValidationMetadataProvider implements ValidationMetadataProvider {
    ...
    /**
     * Get the {@link Configuration} object that was passed into
     ...
     * of the given class. The {@link Validate} and/or
     * {@link ValidateNestedProperties} annotations may be applied to the
     * property's read method, write method, or field declaration. If a property
     * has a {@link ValidateNestedProperties} annotation, then the nested
     * properties named in its {@link Validate} annotations will be included as
     * well.
     *
     * @param beanType a class
     * @return A map of (possibly nested) property names to
     * {@link ValidationMetadata} for the property.
     * @throws StripesRuntimeException if conflicts are found in the validation
     * annotations
     */
    ...
    /**
     * Looks at a class's properties, searching for the specified annotations on
     * the properties (field, getter method, or setter method). An exception is
     * thrown if annotations are found in more than one of those three places.
     *
     * @param beanType the class on which to look for annotations.
     * @param annotationClasses the classes of the annotations for which to look
     * for.
     * @return a map of property names to AnnotationInfo objects, which contain
     * the class on which the annotations were found (if any), and the
     * annotation objects that correspond to the annotation classes.
     */
    ...
    /**
     * Looks at a class's properties, searching for the specified annotations on
     * the given property objects. An exception is thrown if annotations are
     * found in more than one of the specified property accessors (normally
     * field, getter method, and setter method).
     *
     ...
     * annotations were found (if any), and the annotation objects that
     * correspond to the annotation classes.
     */
     ...
     * given property accessor of a class. The property object must not be
     * <code>null</code>, must be declared on the class, must be public if it is
     * a method, and must not be static if it is a field, for it to be
     ...
     * @return the annotation object, or <code>null</code> if no annotation was
     * found.
     */
    ...
    /**
     * Contains the class on which the annotations were found (if any), and the
     * annotation objects that correspond to the annotation classes.
     */
    ...
    /**
     * For some reason, methods common to both the Field and Method classes are
     * not in their parent class, AccessibleObject, so this class works around
     * that limitation.
     */
