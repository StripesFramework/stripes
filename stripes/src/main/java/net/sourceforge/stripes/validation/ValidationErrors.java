...
public class ValidationErrors extends HashMap<String, List<ValidationError>> {
   ...
   * specified. Will lazily instantiate a List&lt;ValidationError&gt; and insert it into the map if
   * necessary.
   *
   * @param field the name of the field in error
   * @param error a ValidationError to add to that field
   ...
   * Purely a convenience method to avoid having to check and possibly instantiate the collection of
  ...
  /**
   * Synonym for putAll(). Add multiple errors for a particular field. Does not destroy or override
   * any existing errors. Purely a convenience method to avoid having to check and possibly
   * instantiate the collection of errors for a field if it does not already exist.
   *
   * @param field the name of the field in error
   * @param errors a non-null list of errors to add for the field
   */
  ...
  /**
   * Allows for the addition of errors that are not tied to a specific field. Errors added in this
   * way will only be displayed as part of the complete list of errors, and never when showing
  ...
  /**
   * Returns true if there are field-specific errors present, and false if there are only global
