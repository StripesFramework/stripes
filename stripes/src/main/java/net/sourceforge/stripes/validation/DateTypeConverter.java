...
/**
 * A TypeConverter that aggressively attempts to convert a String to a java.util.Date object. Under
 * the covers it uses DateFormat instances to do the heavy lifting, but since SimpleDateFormat is
 * quite picky about its input a couple of measures are taken to improve our chances of parsing a
 * Date.
 *
 * <p>First the String is pre-processed to replace commas, slashes, hyphens and periods with spaces
 * and to collapse multiple white spaces into a single space. Then, to ensure that input like "Jan
 * 1" and "3/19" are parseable a check is performed to see if there are only two segments in the
 * input string (e.g. "Jan" and "1" but no "2007"). If that is the case then the current four digit
 * year is appended to improve chances or parsing because a two segment date is not parsable by a
 * DateFormat that expects a year.
 *
 ...
 * conversion. If all DateFormats fail, a validation error will be produced.
 *
 ...
 *
 * <ul>
 *   <li>SHORT (e.g. 'M d yy' for Locale.US)
 *   <li>MEDIUM (e.g. 'MMM d yyyy' for Locale.US)
 *   <li>LONG (e.g. 'MMMM d yyyy' for Locale.US)
 *   <li>d MMM yy (note that for parsing MMM and MMMM are interchangeable)
 *   <li>yyyy M d (note that for parsing M and MM are interchangeable)
 *   <li>yyyy MMM d
 *   <li>EEE MMM dd HH:mm:ss zzz yyyy (the format created by Date.toString())
 * </ul>
 *
 ...
 * patterns should be specified using single spaces as separators instead of slashes, dashes or
 * other characters.
 *
 * <p>The regular expression pattern used in the pre-process method can also be changed in the
 * Stripes resource bundle, or by subclassing and overriding the getPreProcessPattern() method.
 *
 * <p>The keys used in the resource bundle to specify the format strings and the pre-process pattern
 * are:
 *
 * <ul>
 *   <li>stripes.dateTypeConverter.formatStrings
 *   <li>stripes.dateTypeConverter.preProcessPattern
 * </ul>
 *
 * <p>DateTypeConverter can also be overridden in order to change its behaviour. Subclasses can
 * override the preProcessInput() method to change the pre-processing behavior if desired.
 * Similarly, subclasses can override getDateFormats() to change how the DateFormat objects get
 * constructed.
 */
public class DateTypeConverter implements TypeConverter<Date> {
   ...
   * SimpleDateFormat strictly enforces that the separator characters in the input are the same as
   * those in the pattern, this regular expression is used to remove commas, slashes, hyphens and
   * periods from the input String (replacing them with spaces) and to collapse multiple white-space
   * characters into a single space.
   *
   * <p>This pattern can be changed by providing a different value under the <code>
   * 'stripes.dateTypeConverter.preProcessPattern'</code> key in the resource bundle. The default
   * value is <code>(?&lt;!GMT)[\\s,-/\\.]+</code>.
   */
  ...
  public static final String[] formatStrings =
      new String[] {"d MMM yy", "yyyy M d", "yyyy MMM d", "EEE MMM dd HH:mm:ss zzz yyyy"};
  ...
  /**
   * Returns an array of format strings that will be used, in order, to try and parse the date. This
  ...
  protected String[] getFormatStrings() {
    try {
      ...
      String[] formatStrings =
          new String[dateFormats.length + DateTypeConverter.formatStrings.length];

      for (int i = 0; i < dateFormats.length; i++) {
        SimpleDateFormat dateFormat =
            (SimpleDateFormat) DateFormat.getDateInstance(dateFormats[i], locale);
        formatStrings[i] = preProcessInput(dateFormat.toPattern());
      }

      // Then copy the default format strings over too
      System.arraycopy(
          DateTypeConverter.formatStrings,
          0,
          formatStrings,
          dateFormats.length,
  ...
  /**
   * Returns an array of DateFormat objects that will be used in sequence to try and parse the date
   ...
   * expression Pattern returned by getPreProcessPattern() to remove all separator chars and ensure
  ...
  /**
   * Checks to see how many 'parts' there are to the date (separated by spaces) and if there are
   * only two parts it adds the current year to the end by geting the Locale specific year string
