...
/**
 * <p>
 * A TypeConverter that aggressively attempts to convert a String to a
 * java.util.Date object. Under the covers it uses DateFormat instances to do
 * the heavy lifting, but since SimpleDateFormat is quite picky about its input
 * a couple of measures are taken to improve our chances of parsing a Date.</p>
 *
 * <p>
 * First the String is pre-processed to replace commas, slashes, hyphens and
 * periods with spaces and to collapse multiple white spaces into a single
 * space. Then, to ensure that input like "Jan 1" and "3/19" are parseable a
 * check is performed to see if there are only two segments in the input string
 * (e.g. "Jan" and "1" but no "2007"). If that is the case then the current four
 * digit year is appended to improve chances or parsing because a two segment
 * date is not parsable by a DateFormat that expects a year.</p>
 *
 * <p>
 * Then an array of DateFormat instances are used in turn to attempt to parse
 * the input. If any DateFormat succeeds and returns a Date, that Date will be
 * returned as the result of the conversion. If all DateFormats fail, a
 * validation error will be produced.</p>
 *
 * <p>
 * The set of formats is obtained from getFormatStrings(). The default set of
 * formats used is constructed by taking the default SHORT, MEDIUM and LONG
 * formats for the input locale (and replacing all non-space separator
 * characters with spaces), and adding formats to obtain the following
 * patterns:</p>
 *
 * <ul>
 ...
 *
 * <p>
 * This default set of formats can be changed by providing a different set of
 * format strings in the Stripes resource bundle, or by subclassing and
 * overriding getFormatStrings(). In all cases patterns should be specified
 * using single spaces as separators instead of slashes, dashes or other
 * characters.</p>
 *
 * <p>
 * The regular expression pattern used in the pre-process method can also be
 * changed in the Stripes resource bundle, or by subclassing and overriding the
 * getPreProcessPattern() method.</p>
 *
 * The keys used in the resource bundle to specify the format strings and the
 * pre-process pattern are:
 * <ul>
 ...
 *
 * <p>
 * DateTypeConverter can also be overridden in order to change its behaviour.
 * Subclasses can override the preProcessInput() method to change the
 * pre-processing behavior if desired. Similarly, subclasses can override
 * getDateFormats() to change how the DateFormat objects get constructed.
 * </p>
 */
public class DateTypeConverter implements TypeConverter<Date> {
    ...
    /**
     * Used by Stripes to set the input locale. Once the locale is set a number
     * of DateFormat instances are created ready to convert any input.
     */
     ...
     * Since SimpleDateFormat strictly enforces that the separator characters in
     * the input are the same as those in the pattern, this regular expression
     * is used to remove commas, slashes, hyphens and periods from the input
     * String (replacing them with spaces) and to collapse multiple white-space
     * characters into a single space.</p>
     *
     * <p>
     ...
     * resource bundle. The default value is
     * <code>(?&lt;!GMT)[\\s,-/\\.]+</code>.</p>
     */
    ...
    /**
     * Attempts to convert a String to a Date object. Pre-processes the input by
     * invoking the method preProcessInput(), then uses an ordered list of
     * DateFormat objects (supplied by getDateFormats()) to try and parse the
     * String into a Date.
     */
     ...
     * Looks for a pattern in the resource bundle under the key
     * 'stripes.dateTypeConverter.preProcessPattern'. If no value is found, the
     ...
     * First uses the regular expression Pattern returned by
     * getPreProcessPattern() to remove all separator chars and ensure that
     * components are separated by single spaces. Then invokes
     ...
     * spaces) and if there are only two parts it adds the current year to the
     * end by geting the Locale specific year string from a Calendar instance.
     *
