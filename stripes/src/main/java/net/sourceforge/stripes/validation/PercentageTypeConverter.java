...
/**
 * A locale aware number converter that parses percentages. Consistent with other areas of Java (and
 * computing) values are divided by 100 before being returned. For example "100%" will return 1,
 * "58%" will return 0.58 etc. The value returned is either a double, float, or BigDecimal depending
 * on the target type supplied (which is usually driven by the type of the property being
 * converted).
 *
 * @author Tim Fennell
 */
public class PercentageTypeConverter extends NumberTypeConverterSupport
  ...
  /**
   * Converts the input to a subclass of Number based on the targetType provided. Uses a
   * NumberFormat Percentage instance to do the parsing, making sure that the number is divided by
   * 100 and any percent signs etc. are handled.
   */
  public Number convert(
      String input, Class<? extends Number> targetType, Collection<ValidationError> errors) {
