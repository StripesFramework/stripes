...
public interface ValidationMetadataProvider extends ConfigurableComponent {
   ...
   * @return A map of property names to {@link ValidationMetadata}. If no validation information is
   *     present for the given class, then an empty map will be returned.
   */
  Map<String, ValidationMetadata> getValidationMetadata(Class<?> beanType);

  /**
   * Get the validation metadata associated with the named {@code property} of the given {@link
   * ActionBean} class.
   *
   * @param beanType any class
