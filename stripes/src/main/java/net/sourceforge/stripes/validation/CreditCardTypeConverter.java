...
/**
 * A faux TypeConverter that validates that the String supplied is a valid credit card number. The
 * Luhn algorithm is used in addition to valid credit card prefixes to determine if the string being
 * converted qualifies as a credit card number. This DOES NOT check to see if an actual card with
 ...
 *   <li>actionPath.fieldName.invalidCreditCard
 *   <li>fieldName.invalidCreditCard
 *   <li>beanClassFQN.invalidCreditCard
 *   <li>actionPath.invalidCreditCard
 *   <li>converter.creditCard.invalidCreditCard
 * </ul>
 *
 * @author Aaron Porter
 * @since Stripes 1.5
...
public class CreditCardTypeConverter implements TypeConverter<String> {
  ...
  /**
   * Accepts the Locale provided, but does nothing with it since credit card numbers are
   * Locale-less.
   */
  ...
  public String convert(
    ...
    if (getCardType(cardNumber) != null) return cardNumber;
  ...
  public static Type getCardType(String cardNumber) {
    if (!isLuhnValid(cardNumber)) return null;

    if (checkCard(cardNumber, 15, "34", "37")) return Type.AMEX;

    if (checkCard(cardNumber, 14, "30", "36", "38")) return Type.DinersClub;

    if (checkCard(cardNumber, 16, "6011")) return Type.Discover;

    if (checkCard(cardNumber, 15, "2014", "2149")) return Type.enRoute;

    if (checkCard(cardNumber, 16, "3088", "3096", "3112", "3158", "3337", "3528")) return Type.JCB;

    if (checkCard(cardNumber, 16, "51", "52", "53", "54", "55")) return Type.MasterCard;

    if (checkCard(cardNumber, 13, "4") || checkCard(cardNumber, 16, "4")) return Type.VISA;

    return null;
  }

  /**
   * Checks cardNumber to see if it is the correct length and starts with one of the specified
   * prefixes.
   *
   * @param cardNumber the card number to check
  ...
  private static boolean checkCard(String cardNumber, int length, String... prefixes) {
    ...
    for (String prefix : prefixes) if (cardNumber.startsWith(prefix)) return true;

    return false;
  }

  /**
   * Performs the Luhn algorithm on the card number to determine if it is valid as a credit card
   * number.
   *
   * @param cardNumber the card number to check
