 ...
 * credit card number. The Luhn algorithm is used in addition to valid credit
 * card prefixes to determine if the string being converted qualifies as a
 * credit card number. This DOES NOT check to see if an actual card with the
 * specified number exists, only that it appears to be a valid card number.</p>
 *
 * <p>
...
public class CreditCardTypeConverter implements TypeConverter<String> {
    ...
    /**
     * Accepts the Locale provided, but does nothing with it since credit card
     * numbers are Locale-less.
     */
    ...
    /**
     * Validates the user input to ensure that it looks like a valid credit card
     * number.
     *
     ...
     * @return the type of card or null if the card number is invalid for all
     * known card types
     */
    ...
    /**
     * Checks cardNumber to see if it is the correct length and starts with one
    ...
    /**
     * Performs the Luhn algorithm on the card number to determine if it is
     * valid as a credit card number.
     *
     * @param cardNumber the card number to check
