...
/**
 * An implementation of {@link ExpressionExecutor} that uses the new EL API available in Java EE 5
 * in the {@link jakarta.el} package. While more complicated that the JSP 2.0 API it has one
 * advantage which is that it can be used without the need to allocate a PageContext object and
 * without any other libraries being available.
 *
 * @author tfenne
 * @since Stripes 1.5
 */
public class Jsp21ExpressionExecutor implements ExpressionExecutor {
  ...
  /**
   * Implementation of the EL interface to resolve variables. Resolves variables by checking two
   * special names ("this" and "actionBean") and then falling back to retrieving property values
   * from the ActionBean passed in to the constructor.
   *
   * @author Tim Fennell
   * @since Stripes 1.5
   */
  ...
  protected static class StripesELResolver extends ELResolver {
    ...
    public Iterator<FeatureDescriptor> getFeatureDescriptors(
        final ELContext elContext, final Object o) {
      return null;
    }
  ...
  /**
   * Implementation of the EL interface for managing expression context. Resolves variables using
   * the StripesELResolver above. Both the FunctionMapper and VariableResolver are essentially no-op
   * implementations.
   *
   * @author Tim Fennell
   * @since Stripes 1.5
  ...
  protected static class StripesELContext extends ELContext {
    ...
    /**
     * Constructs a new instance using the ActionBean provided as the source for most property
     * resolutions.
     *
      ...
      this.vmapper =
          new VariableMapper() {
            ...
            public ValueExpression setVariable(
                final String s, final ValueExpression valueExpression) {
              return null;
            }
          };
    }
  ...
  public void evaluate(
      ...
      final ValidationMetadata validationInfo,
    ...
    try {
      ...
      throw new StripesRuntimeException(
          "Could not parse the EL expression being used to validate field "
              + name.getName()
              + ". This is not a transient error. Please double "
