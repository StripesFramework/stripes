 ...
 * TypeConverters included in the Stripes distribution, and their applicable classes. Can handle all
 * the primitive and wrapper types as well as the rich types for which type converters exist.
 *
 * @author Tim Fennell
 */
public class DefaultTypeConverterFactory implements TypeConverterFactory {
  ...
  /**
   * Gets the (rather confusing) Map of TypeConverter objects. The Map uses the target class as the
   * key in the Map, and the Class object representing the TypeConverter as the value.
   *
  ...
  /**
   * Adds a TypeConverter to the set of registered TypeConverters, overriding an existing converter
   * if one was registered for the type.
   *
  ...
  /**
   * Gets the applicable type converter for the class passed in. This is based on the default set of
   * type converters which are stored in a Map on this class. Enums are a special case, whereby if
   * there is no converter in the map, the EnumeratedTypeConverter will be returned.
   *
