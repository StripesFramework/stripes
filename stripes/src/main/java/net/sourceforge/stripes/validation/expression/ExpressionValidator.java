...
/**
 * <p>
 * A Facade to the classes that perform expression based validation. Hides the
 * fact that we might be using one of many implementations to actually run
 * expression validation. When the
 * {@link net.sourceforge.stripes.validation.expression.ExpressionExecutor} is
 * first requested an attempt is made to find the best working executor
 * available. The following classes will be tried in turn until a working
 * instance is found:</p>
 *
 * <ul>
...
public class ExpressionValidator {
    ...
    /**
     * Attempts to instantiate executor classes (as described in the class level
     * javadoc) until a working one is found.
     */
    ...
    /**
     * Gets the executor that will be used to run expression evaluation. If none
     * is yet set the {@link #initialize()} method will be run to set one up.
     *
     ...
     * ExpressionExecutor instance instead of the one picked by the
     * ExpressionValidator.
     *
