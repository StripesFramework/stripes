...
/**
 * Provides a mechanism for creating localizable error messages for presentation to the user. Uses
 * ResourceBundles to provide the localization of the error message. Messages may contain one or
 * more &quot;replacement parameters &quot;. Two replacement parameters are provided by default,
 * they are the field name and field value, and are indices 0 and 1 respectively. To use replacement
 ...
 * the action the user was trying to perform, you might have a properties file entry like:
 *
 ...
 * LocalizableError("insufficientBalance")} might look for message resources with the following
 * keys:
 *
 * <ul>
 *   <li>{@code com.myco.TransferActionBean.insufficientBalance}
 *   <li>{@code /account/Transfer.action.insufficientBalance}
 ...
 * <em>actionPath.fieldName</em> where actionPath is the action of the form in the JSP (or equally,
 * the path given in the @UrlBinding annotation in the ActionBean class). Finally, the last attempt
...
public class LocalizableError extends SimpleError {
  ...
  /**
   * Method responsible for using the information supplied to the error object to find a message
   * template. In this class this is done simply by looking up the resource corresponding to the
   * messageKey supplied in the constructor, first with the FQN prepended, then with the action path
  ...
  protected String getMessageTemplate(Locale locale) {
    ...
    if (beanclass != null) {
      template =
          LocalizationUtility.getErrorMessage(locale, beanclass.getName() + "." + messageKey);
    }
    if (template == null) {
