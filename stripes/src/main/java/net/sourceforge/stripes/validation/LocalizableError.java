...
/**
 * <p>
 * Provides a mechanism for creating localizable error messages for presentation
 * to the user. Uses ResourceBundles to provide the localization of the error
 * message. Messages may contain one or more &quot;replacement parameters
 * &quot;. Two replacement parameters are provided by default, they are the
 * field name and field value, and are indices 0 and 1 respectively. To use
 ...
 * parameter which is the action the user was trying to perform, you might have
 * a properties file entry like:</p>
 *
 ...
 *
 * <p>
 * First looks for a resource with the action bean FQN prepended to the supplied
 * message key. If If that cannot be found then looks with the action path as a
 * prefix instead of the FQN. Failing that, the last attempt looks for a
 * resource with the exact message key provided. This allows developers to
 * segregate their error messages by action without having to repeat the action
 * path in the ActionBean. For example a message constructed with
 * {@code new LocalizableError("insufficientBalance")} might look for message
 * resources with the following keys:</p>
 *
 * <ul>
...
public class LocalizableError extends SimpleError {
     ...
     * optionally zero or more replacement parameters to use in the message. It
     * should be noted that the replacement parameters provided here can be
     * referenced in the error message <b>starting with number 2</b>.
     *
     ...
     * to find a message template. In this class this is done simply by looking
     * up the resource corresponding to the messageKey supplied in the
     * constructor, first with the FQN prepended, then with the action path
     * prepended and finally bare.
     * @return 
     */
    @Override
