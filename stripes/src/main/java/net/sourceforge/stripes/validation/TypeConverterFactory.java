...
/**
 * Provides access to a set of TypeConverters for converting Strings to various
 * types. Implementations may use any mechanism desired to map a type to a
 * TypeConverter, and may optionally choose to cache TypeConverter instances.
 * The behaviour of the type conversion lookups in Stripes can be altered either
 * by directly implementing this interface, or by subclassing
...
public interface TypeConverterFactory extends ConfigurableComponent {
    ...
    /**
     * Gets the applicable type converter for the class passed in. The
     * TypeConverter returned must create objects of the type supplied, or
     * possibly a suitable derived type.
     *
     ...
     * overriding an existing converter if one was already registered for the
     * type. This is an optional operation. Implementations that do not support
     * adding type converters at runtime must throw
