 ...
 * available to {@link Interceptor} classes that are interleaved with the regular request processing
 * lifecycle.
 *
 * <p>The ExecutionContext is not populated all at once, but in pieces as the request progresses.
 * Check the accessor method for each item for information on when that item becomes available in
...
public class ExecutionContext {
  ...
  /**
   * Used by the {@link DispatcherServlet} to initialize and/or swap out the list of {@link
   * Interceptor} instances which should wrap the current {@link LifecycleStage}.
   *
  ...
  /**
   * Used by the {@link DispatcherServlet} to wrap a block of lifecycle code in {@link Interceptor}
   * calls.
   *
   * @param target a block of lifecycle/request processing code that is contained inside a class
   *     that implements Interceptor
   * @return a Resolution instance or null depending on what is returned by the lifecycle code and
   *     any interceptors which intercept the execution
  ...
  public Resolution wrap(Interceptor target) throws Exception {
    ...
    if (lifecycleStage == LifecycleStage.RequestInit) currentContext.set(this);
  ...
  /**
   * Retrieves the ActionBean instance that is associated with the current request. Available to
   * interceptors only after {@link LifecycleStage#ActionBeanResolution} has occurred.
   *
  ...
  /**
   * Retrieves the handler Method that is targeted by the current request. Available to interceptors
   * only after {@link LifecycleStage#HandlerResolution} has occurred.
   *
  ...
  /**
   * Gets the Resolution that will be executed at the end of the execution. This value is generally
   * not populated until just prior to {@link LifecycleStage#ResolutionExecution}.
   *
   ...
   * lifecycle stage before invoking any interceptors or lifecycle code, so that interceptors that
   * intercept at multiple lifecycle stages can be aware of which stage is being intercepted.
   *
