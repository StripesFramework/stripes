...
public class BindingPolicyManager {
  /**
   * List of classes that, for security reasons, are not allowed as a {@link NodeEvaluation} value
   * type.
   */
  ...
  public static BindingPolicyManager getInstance(Class<?> beanType) {
    if (instances.containsKey(beanType)) return instances.get(beanType);
  ...
  public boolean isBindingAllowed(PropertyExpressionEvaluation eval) {
    ...
    boolean allow =
        (allowPattern != null && allowPattern.matcher(paramName).matches())
  ...
  /**
   * Indicates if any node in the given {@link PropertyExpressionEvaluation} has a value type that
   * is assignable from any of the classes listed in {@link #ILLEGAL_NODE_VALUE_TYPES}.
   *
   * @param eval a property expression that has been evaluated against an {@link ActionBean}
   ...
   * corresponding {@link ValidationMetadata}, as returned by {@link
   * ValidationMetadataProvider#getValidationMetadata(Class, ParameterName)}. The idea here is that
   * if the bean property must be validated, then it is expected that the property may be bound to
   * the bean.
   *
  ...
  protected Pattern globToPattern(String... globArray) {
    ...
    for (String glob : globs) {
      ...
      for (int i = 0; i < properties.length; i++) {
        ...
        if (i < properties.length - 1) buf.append("\\.");
      }
