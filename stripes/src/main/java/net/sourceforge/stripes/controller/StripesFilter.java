...
/**
 * The Stripes filter is used to ensure that all requests coming to a Stripes application are
 * handled in the same way. It detects and wraps any requests that contain multipart/form data, so
 * that they may be treated much like any other request. Also ensures that all downstream components
 * have access to essential configuration and services whether the request goes through the
...
public class StripesFilter implements Filter {
   ...
   * weak references to allow garbage collection to reap these objects if this is the only reference
   ...
   * configuration class to use, and then instantiating and initializing the chosen Configuration.
   *
  ...
  public void init(FilterConfig filterConfig) throws ServletException {
    ...
    log.info(
        "Stripes Initialization Complete. Version: ", pkg.getSpecificationVersion(),
  ...
  protected static Configuration createConfiguration(FilterConfig filterConfig)
    ...
    Class<? extends Configuration> clazz =
        bootstrap.getClassProperty(CONFIG_CLASS, Configuration.class);

    if (clazz == null) clazz = RuntimeConfiguration.class;

    try {
      Configuration configuration = clazz.getDeclaredConstructor().newInstance();
  ...
  public static Configuration getConfiguration() {
    ...
    if (configuration == null) {
      StripesRuntimeException sre =
          new StripesRuntimeException(
  ...
  /**
   * Returns the configuration for this instance of the StripesFilter for any class that has a
   * reference to the filter. For normal runtime access to the configuration during a request cycle,
   * call getConfiguration() instead.
   *
  ...
  /**
   * Performs the primary work of the filter, including constructing a StripesRequestWrapper to wrap
   * the HttpServletRequest, and using the configured LocalePicker to decide which Locale will be
    ...
    try {
      ...
      if (initial) {
        ...
        if (encoding != null) {
          ...
          log.debug(
              "LocalePicker did not pick a character encoding, using default: ",
        ...
        StripesRequestWrapper.findStripesWrapper(httpRequest)
            .pushUriParameters((HttpServletRequestWrapper) httpRequest);
      }
  ...
  /**
   * Wraps the HttpServletRequest with a StripesServletRequest. This is done to ensure that any form
   * posts that contain file uploads get handled appropriately.
   *
  ...
  /**
   * Checks to see if there is a flash scope identified by a parameter to the current request, and
   * if there is, retrieves items from the flash scope and moves them back to request attributes.
   */
  ...
  /**
   * Manages the work that ensures that flash scopes get cleaned up properly when requests go
   * missing. Firstly timestamps the current flash scope (if one exists) to record the time that the
   * request exited the container. Then checks all flash scopes to make sure none have been hanging
