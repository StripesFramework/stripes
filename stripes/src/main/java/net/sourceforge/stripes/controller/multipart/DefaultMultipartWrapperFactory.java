 ...
 * to the {@link CommonsMultipartWrapper}. An additional configuration parameter is supported to
 * specify the maximum post size allowable.
 *
 * @author Tim Fennell
 * @since Stripes 1.4
 */
public class DefaultMultipartWrapperFactory implements MultipartWrapperFactory {
   ...
   * initialization. Components are expected to fail loudly if they are not going to be in a valid
  ...
  public void init(Configuration config) throws Exception {
    ...
    if (this.multipartClass == null) {
      ...
      log.info(
          "Using ",
          this.multipartClass.getName(),
          " as ",
          MultipartWrapper.class.getSimpleName(),
          " implementation.");
    }
    ...
    if (tempDir != null) {
      ...
      if (tmpDir != null) {
        ...
        log.warn(
            "The tmpdir system property was null! File uploads will probably fail. ",
    ...
    if (limit != null) {
      ...
      if (!matcher.matches()) {
        log.error(
            "Did not understand value of configuration parameter ",
            MAX_POST,
            " You supplied: ",
            limit,
            ". Valid values are any string of numbers ",
   ...
   * @throws FileUploadLimitExceededException if the post size of the request exceeds any configured
   *     limits
   */
