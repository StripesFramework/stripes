 ...
 * whether or not the name refers to an indexed or mapped property.
 *
 * @author Tim Fennell
 */
public class ParameterName implements Comparable<ParameterName> {
     ...
     * As it is constructed, detects whether or not the name contains indexing
     * or mapping components, and if it does, also creates and stores the
     * stripped name.
     *
    ...
    /**
     * Returns true if the name has indexing or mapping components, otherwise
     * false.
     *
     * @return Whether or not this parameter name has indexing or mapping
     * components.
     */
    ...
    /**
     * Always returns the parameter name as passed in to the constructor. If it
     * contained indexing or mapping components (e.g. [3] or (foo)) they will be
     * present in the String returned.
     *
    ...
    /**
     * Returns the name with all indexing and mapping components stripped. E.g.
     * if the name in the request was 'foo[1].bar', this method will return
     * 'foo.bar'.
     *
    ...
    /**
     * Orders ParameterNames so that those with shorter (unstripped) names come
     * first. Two names of the same length are then ordered alphabetically by
     * String.compareTo().
     *
     ...
     * equality to see if we can short circuit, and then checks for equality of
     * the name attribute for a real test.
     */
    @Override
    ...
    /**
     * Uses the original name as the string representation of the class. This is
     * probably the most useful thing to see in log messages, which is the only
     * place toString will be used.
     */
    @Override
