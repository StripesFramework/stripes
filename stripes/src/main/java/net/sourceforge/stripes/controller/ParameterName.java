...
public class ParameterName implements Comparable<ParameterName> {
  ...
  /**
   * Always returns the parameter name as passed in to the constructor. If it contained indexing or
   * mapping components (e.g. [3] or (foo)) they will be present in the String returned.
   *
  ...
  /**
   * Returns the name with all indexing and mapping components stripped. E.g. if the name in the
   * request was 'foo[1].bar', this method will return 'foo.bar'.
   *
   ...
   * @return -1 if this value sorts first, 0 if the values are identical and +1 if the parameter
   *     passed in sorts first.
   */
  public int compareTo(ParameterName that) {
    int result = Integer.compare(this.name.length(), that.name.length());
    if (result == 0) {
  ...
  /**
   * Checks for equality as efficiently as possible. First checks for JVM equality to see if we can
   * short circuit, and then checks for equality of the name attribute for a real test.
   */
  @Override
  ...
  /**
   * Uses the original name as the string representation of the class. This is probably the most
   * useful thing to see in log messages, which is the only place toString will be used.
   */
  @Override
