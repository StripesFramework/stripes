 ...
 * subsequent request as a request attribute.
 *
 * <p>To make values available to the subsequent request a parameter must be included in the
 * redirect URL that identifies the flash scope to use (this avoids collisions where two concurrent
 * requests in the same session might otherwise cause problems for one another). The Stripes {@link
 * net.sourceforge.stripes.action.RedirectResolution} will automatically insert this parameter into
 * the URL when a flash scope is present. Should you wish to issue redirects using a different
 * mechanism you will need to add the parameter using code similar to the following:
 *
 * <pre>
 ...
 * FlashScopes are manufactured using lazy instantiation when {@code FlashScope.getCurrent(request,
 * true)} is called. When a request is completed, the StripesFilter notifies the current FlashScope
 * that the request is over, which causes it to record the time when the request terminated. On the
 * subsequent request, if the flash scope is referenced by a URL parameter, then it is removed from
 * session and it's contents are pushed into request attributes for the current request.
 *
 * <p>To ensure that orphaned FlashScopes do not consume increasing amounts of HttpSession memory,
 * the StripesFilter, after each request, checks to see if any FlashScopes have recently expired. A
 ...
 * (or two minutes), and can be varied by calling {@link #setTimeout(int)} Since the timer starts
 * when a request completes, and FlashScopes are only meant to live from the end of one request to
 * the beginning of a subsequent request this value is set quite low.
 *
 * @author Tim Fennell
 * @since Stripes 1.2
 */
public class FlashScope extends HashMap<String, Object> {
  ...
  public void completeRequest() {
    ...
    HttpServletResponse flashResponse =
        (HttpServletResponse)
  ...
  public void beginRequest(HttpServletRequest request) {
    ...
    try {
      ...
      if (!acquired) {
        log.error(
            "Something is amiss! A timeout occurred while trying to copy a flash "
      ...
      if (acquired) getSemaphore().release();
    }
  }

  /**
   * Returns the time in seconds since the request that generated this flash scope completed. Will
   * return 0 if this flash scope has not yet started to age.
   */
  public long age() {
    if (startTime == 0) {
      return 0;
  ...
  /**
   * Stores an ActionBean into the flash scope. Additional checking is performed to see if the
   * ActionBean is the currently resolved (main) ActionBean for the request. The result is that on
   * the next request the ActionBean will appear in the request as if it was created on that
   * request.
   *
  ...
  public void put(ActionBean bean) {
    String binding =
        StripesFilter.getConfiguration().getActionResolver().getUrlBinding(bean.getClass());
  ...
  /**
   * Gets the current flash scope into which items can be stored temporarily. If <code>create</code>
   * is true, then a new one will be created.
   *
   * <p>It is assumed that the request object will be used by only one thread so access to the
  ...
  /**
   * Internal helper method to retrieve (and selectively create) the container for all the flash
   * scopes. Will return null if the container does not exist and <i>create</i> is false. Will also
   * return null if the current session has been invalidated, regardless of the value of
   * <i>create</i>.
   *
   * @param req the current request
  ...
  private static Map<Integer, FlashScope> getContainer(HttpServletRequest req, boolean create) {
    try {
      ...
      log.warn(
          "An IllegalStateException got thrown trying to create a flash scope. ",
