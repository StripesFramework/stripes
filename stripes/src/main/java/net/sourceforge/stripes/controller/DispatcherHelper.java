...
public class DispatcherHelper {
  ...
  /**
   * A Map that is used to cache the validation method that are discovered for each ActionBean.
   * Entries are added to this map the first time that a request is made to a particular ActionBean.
   * The Map will contain a zero length array for ActionBeans that do not have any validation
   * methods.
   */
   ...
   * to a PageContext object that can be used to manufacture expression evaluator instances.
   *
  ...
  /**
   * Used by the validation subsystem to access a page context that can be used to create an
   * expression evaluator for use in the expression validation code.
   *
   ...
   * ExecutionContext. If no ActionBean can be found the ActionResolver will throw an exception,
   * thereby aborting the current request.
   *
   ...
   * ActionBeanContext contained within the ExecutionContext. Once the event name has been
   * determined this method must resolve the handler method. If a handler method cannot be
   * determined an exception should be thrown to abort processing.
   *
   * @param ctx the ExecutionContext being used to process the current request
   * @return a Resolution if any interceptor determines that the request processing should be
   *     aborted in favor of another Resolution, null otherwise.
   ...
   * ActionBeanContext contained within the ExecutionContext (regardless of whether any errors were
   * generated or not).
   *
   * @param ctx the ExecutionContext being used to process the current request
  ...
  public static Resolution doBindingAndValidation(
    ...
    final boolean doValidate =
        doBind
            && validate
            && (handler == null || handler.getAnnotation(DontValidate.class) == null);
    final Configuration config = StripesFilter.getConfiguration();
  ...
  /**
   * Responsible for coordinating the invocation of any custom validation logic exposed by the
   * ActionBean. Will only call the validation methods if certain conditions are met (there are no
  ...
  /**
   * Determines if the ValidationMethod annotation should be applied to the named event. True if the
   * list of events to apply the validation to is empty, or it contains the event name, or it
   * contains event names starting with "!" but not the event name. Some examples to illustrate the
   * point
   *
   * <ul>
   *   <li>info.on={}, event="save" => true
   *   <li>info.on={"save", "update"}, event="save" => true
   *   <li>info.on={"save", "update"}, event="delete" => false
   *   <li>info.on={"!delete"}, event="save" => true
   *   <li>info.on={"!delete"}, event="delete" => false
   * </ul>
   *
  ...
  /**
   * Finds and returns all methods in the ActionBean class and it's superclasses that are marked
   * with the ValidationMethod annotation and returns them ordered by priority (and alphabetically
   * within priorities). Looks first in an instance level cache, and if that does not contain
   * information for an ActionBean, examines the ActionBean and adds the information to the cache.
   *
  ...
  /**
   * Responsible for checking to see if validation errors exist and if so for handling them
   * appropriately. This includes ensuring that the error objects have all information necessary to
   * render themselves appropriately and invoking any error handling code.
   *
   * @param ctx the ExecutionContext being used to process the current request
  ...
  /**
   * Makes sure that validation errors have all the necessary information to render themselves
   * properly, including the UrlBinding of the action bean and the field value if it hasn't already
   * been set.
   *
   * @param ctx the ExecutionContext being used to process the current request
   */
  ...
  /**
   * Responsible for invoking the event handler identified. This method will only be called if an
   * event handler was identified, and can assume that the bean and handler are present in the
   * ExecutionContext.
   *
   * @param ctx the ExecutionContext being used to process the current request type conversion
   *     should occur
   * @return a Resolution if the error handling code determines that some kind of resolution should
   *     be processed in favor of continuing on to handler invocation
  ...
  /**
   * Responsible for executing the Resolution returned by the request. Transitions the execution
   * context to {@link net.sourceforge.stripes.controller.LifecycleStage#ResolutionExecution}, sets
   * the resolution on the execution context and then invokes the interceptor chain to execute the
   * Resolution.
   *
  ...
  public static void logValidationErrors(ActionBeanContext context) {
    ...
    for (List<ValidationError> list : context.getValidationErrors().values()) {
      for (ValidationError error : list) {
        ...
        if (ValidationErrors.GLOBAL_ERROR.equals(fieldName)) fieldName = "GLOBAL";
