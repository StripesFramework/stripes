...
/**
 * <p>
 * Interceptor that inspects ActionBeans for {@link Before} and {@link After}
 * annotations and runs the annotated methods at the requested point in the
 * request lifecycle. There is no limit on the number of methods within an
 * ActionBean that can be marked with {@code @Before} and {@code @After}
 * annotations, and individual methods may be marked with one or both
 * annotations.</p>
 *
 * <p>
 * To configure the BeforeAfterMethodInterceptor for use you will need to add
 * the following to your {@code web.xml} (assuming no other interceptors are yet
 * configured):</p>
 *
 * <pre>
 * &lt;init-param&gt;
 ...
 *
 * <p>
 * If one or more interceptors are already configured in your {@code web.xml}
 * simply separate the fully qualified names of the interceptors with commas
 * (additional whitespace is ok).</p>
 *
...
public class BeforeAfterMethodInterceptor implements Interceptor {
     ...
     * javadoc. Executed the before and after methods for the ActionBean as
     * appropriate for the current lifecycle stage. Lazily examines the
     * ActionBean to determine the set of methods to execute, if it has not yet
     * been examined.
     *
     * @param context the current ExecutionContext
     * @return a resolution if one of the Before or After methods returns one,
     * or if the nested interceptors return one
    ...
    /**
     * Helper method that will invoke the supplied method and manage any
     * exceptions and returns from the object. Specifically it will log any
     * exceptions except for InvocationTargetExceptions which it will attempt to
     * unwrap and rethrow. If the method returns a Resolution it will be
     * returned; returns of other types will be ignored.
     *
     * @param bean - ActionBean object
     * @param m - Method to execute
     * @param stage - Stage to execute on
     * @param when - The annotation method specifying when to execute
     * @return The resolution after the invocation has completed.
     * @throws java.lang.Exception If an error occurs when this resolution is
     * invoked.
     */
    ...
    /**
     * Gets the Before/After methods for the ActionBean. Lazily examines the
     * ActionBean and stores the information in a cache. Looks for all
     * non-abstract, no-arg methods that are annotated with either
    ...
    /**
     * Helper class used to collect Before and After methods for a class and
     * provide easy and rapid access to them by LifecycleStage.
     *
     * @author Jeppe Cramon
     */
    protected static class FilterMethods {
         ...
         *
         * @param stages All the LifecycleStages that the given filter method
         * should be invoked before
         * @param method The filter method to be invoked before the given
         * LifecycleStage(s)
         */
         ...
         * @param method The filter method to be invoked after the given
         * LifecycleStage(s)
         */
        ...
        /**
         * Helper method to add methods to a method map keyed by the
         * LifecycleStage.
         *
