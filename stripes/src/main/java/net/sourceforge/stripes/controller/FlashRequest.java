...
/**
 * Captures the state of an {@link HttpServletRequest} so that the information contained therein can
 * be carried over to the next request for use by the flash scope. There are several methods in here
 * that cannot be faked and so must delegate to an active {@link HttpServletRequest} object, the
 * {@link #delegate}. If one of these methods is called and there is no delegate object set on the
 * instance, they will throw a {@link net.sourceforge.stripes.exception.StripesRuntimeException}.
 * Unless this class is used outside its intended context (during a live request processed through
