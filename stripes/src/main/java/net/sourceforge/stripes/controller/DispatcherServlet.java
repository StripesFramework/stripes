...
public class DispatcherServlet extends HttpServlet {
   ...
   protected Resolution doCustomValidation( ExecutionContext ctx ) throws Exception {
      return DispatcherHelper.doCustomValidation(ctx, _alwaysInvokeValidate);
   }
   ...
   protected void service( final HttpServletRequest request, final HttpServletResponse response ) throws ServletException {
      ...
      try {
         ...
         final ActionBeanContext context = config.getActionBeanContextFactory().getContextInstance(request, response);
         ...
         try {
            ...
            if ( ExpressionValidator.getExecutor() instanceof Jsp20ExpressionExecutor ) {
               pageContext = JspFactory.getDefaultFactory().getPageContext(this, // the servlet inst
                     ...
                     abc.getResponse().getBufferSize(), true); // autoflush
   ...
   private void doOneTimeConfiguration() {
      if ( _alwaysInvokeValidate == null ) {
         ...
         if ( callValidateWhenErrorsExist != null ) {
            ...
            _alwaysInvokeValidate = c.convert(callValidateWhenErrorsExist, Boolean.class, null);
         } else {
            _alwaysInvokeValidate = false; // Default behaviour
         }
      }
   }
