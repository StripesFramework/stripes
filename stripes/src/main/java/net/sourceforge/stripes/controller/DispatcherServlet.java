 ...
 *
 * <p>While the DispatcherServlet is structured so that it can be easily subclassed and overridden
 * much of the processing work is delegated to the {@link DispatcherHelper} class.
 *
 * @author Tim Fennell
 */
public class DispatcherServlet extends HttpServlet {
   ...
   *
   * <ul>
   *   <li>{@link #resolveActionBean(ExecutionContext)}
   *   <li>{@link #resolveHandler(ExecutionContext)}
   *   <li>{@link #doBindingAndValidation(ExecutionContext)}
   *   <li>{@link #doCustomValidation(ExecutionContext)}
   *   <li>{@link #handleValidationErrors(ExecutionContext)}
   *   <li>{@link #invokeEventHandler(ExecutionContext)}
   * </ul>
   *
  ...
  /**
   * Performs a simple piece of one time configuration that requires access to the Configuration
   * object delivered through the Stripes Filter.
   */
    ...
    if (alwaysInvokeValidate == null) {
      ...
      String callValidateWhenErrorsExist =
          StripesFilter.getConfiguration()
              ...
              .getProperty(RUN_CUSTOM_VALIDATION_WHEN_ERRORS);
  ...
  /**
   * Fetches, and lazily creates if required, a Stack in the request to store ActionBeans should the
  ...
  /**
   * Saves the current value of the 'actionBean' attribute in the request so that it can be restored
   * at a later date by calling {@link #restoreActionBean(HttpServletRequest)}. If no ActionBean is
  ...
  /**
   * Restores the previous value of the 'actionBean' attribute in the request. If no ActionBeans
   * have been saved using {@link #saveActionBean(HttpServletRequest)} then this method has no
   * effect.
   *
   * @param request the current HttpServletRequest
   */
