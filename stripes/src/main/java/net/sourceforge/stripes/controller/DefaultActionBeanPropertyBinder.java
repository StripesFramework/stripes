...
/**
 * <p>
 * Implementation of the ActionBeanPropertyBinder interface that uses Stripes'
 * built in property expression support to perform JavaBean property binding.
 * Several additions/enhancements are available above and beyond the standard
 * JavaBean syntax. These include:
 * </p>
 *
 * <ul>
 * <li>The ability to instantiate and set null intermediate properties in a
 * property chain</li>
 * <li>The ability to use Lists and Maps directly for indexed properties</li>
 * <li>The ability to infer type information from the generics information in
 * classes</li>
 * </ul>
 *
 * @author Tim Fennell
...
public class DefaultActionBeanPropertyBinder implements ActionBeanPropertyBinder {
     ...
     *
     * @return Configuration object associated with this action bean property
     * binder
     */
     ...
     * bind each one to the supplied ActionBean. Invokes validation for each of
     * the properties on the bean before binding is attempted. Only fields which
     * do not produce validation errors will be bound to the ActionBean.
     * </p>
     *
     * <p>
     * Individual property binding is delegated to the other interface method,
     * bind(ActionBean, String, Object), in order to allow for easy extension of
     * this class.
     * </p>
     *
     ...
     * @param validate true indicates that validation should be run, false
     * indicates that only type conversion should occur
     */
    @Override
    ...
    /**
     * <p>
     * Checks to see if binding is permitted for the provided expression
     * evaluation. Note that the expression is available through the
     * {@code getExpression()} and the ActionBean is available through the
     * {@code getBean()} method on the evaluation.
     * </p>
     *
     * <p>
     * By default checks to ensure that the expression is not attempting to bind
     * into the ActionBeanContext for security reasons.
     * </p>
     *
     ...
     * property to an ActionBean. By default logs some information about the
     * occurrence, but could be overridden to do more intelligent things based
     * on the application.
     *
     ...
     * error occurred when binding a null value
    ...
    /**
     * Uses a hidden field to determine what (if any) fields were present in the
     * form but did not get submitted to the server. For each such field the
     * value is "softly" set to null on the ActionBean. This is not uncommon for
     * checkboxes, and also for multi-selects.
     *
    ...
    /**
     * In a lot of cases (and specifically during wizards) the Stripes form
     * field writes out a hidden field containing a set of field names. This is
     * encrypted to stop the user from monkeying with it. This method retrieves
     * the list of field names, decrypts it and splits it out into a Collection
     * of field names.
     *
    ...
    /**
     * Internal helper method to bind one or more values to a single property on
     * an ActionBean. If the target type is an array of Collection, then all
     * values are bound. If the target type is a scalar type then the first
     * value in the List of values is bound.
     *
     * @param bean the ActionBean instance to which the property is being bound
     * @param propertyEvaluation the property evaluation to be used to set the
     * property
    ...
    /**
     * Internal helper method that determines what to do when no value was
     * supplied for a given form field (but the field was present on the page).
     * In all cases if the property is already null, or intervening objects in a
     * nested property are null, nothing is done. If the property is non-null,
     * it will be set to null. Unless the property is a collection, in which
     * case it will be clear()'d.
     *
     ...
     * @throws ExpressionException if the value cannot be manipulated for any
     * reason
     */
    ...
    /**
     * Converts the map of parameters in the request into a Map of ParameterName
     * to String[]. Returns a SortedMap so that when iterated over parameter
     * names are accessed in order of length of parameter name.
     *
     * @param bean - Action bean object to get parameters for
     ...
     * fails for any reason (property does not exist, type conversion not
     * possible etc.) an exception will be thrown.
     *
     ...
     * looping through the set of validation annotations and checking that each
     * field marked as required was submitted in the request and submitted with
     * a non-empty value.
     *
     * @param parameters Parameter name map
     * @param bean Action bean object
     * @param errors validation errors object to use
     */
    ...
    /**
     * <p>
     * Checks to see if a single field's set of values are 'present', where that
     * is defined as having one or more values, and where each value is a
     * non-empty String after it has had white space trimmed from each end.
     * </p>
     *
     * <p>
     * For any fields that fail validation, creates a ScopedLocaliableError that
     * uses the stripped name of the field to find localized info (e.g. foo.bar
     * instead of foo[1].bar). The error is bound to the actual field on the
     * form though, e.g. foo[1].bar.
     * </p>
     *
     * @param name the name of the parameter verbatim from the request
     * @param strippedName the name of the parameter with any indexing removed
     * from it
    ...
    /**
     * Performs several basic validations on the String value supplied in the
     * HttpServletRequest, based on information provided in annotations on the
     * ActionBean.
     *
     ...
     * validated
     * @param errors a collection of errors to be populated with any validation
     * errors discovered
     */
    ...
    /**
     * Performs basic post-conversion validations on the properties of the
     * ActionBean after they have been converted to their rich type by the type
     * conversion system. Validates single properties in isolation from other
     * properties.
     *
     * @param bean the ActionBean that is undergoing validation and binding
     * @param convertedValues a map of ParameterName to all converted values for
     * each field
    ...
    /**
     * Performs validation of attribute values using a JSP EL expression if one
     * is defined in the {@literal @}Validate annotation. The expression is
     * evaluated once for each value converted. See
     * {@link net.sourceforge.stripes.validation.expression.ExpressionValidator}
    ...
    /**
     * <p>
     * Converts the String[] of values for a given parameter in the
     * HttpServletRequest into the desired type of Object. If a converter is
     * declared using an annotation for the property (or getter/setter) then
     * that converter will be used - if it does not convert to the right type an
     * exception will be logged and values will not be converted. If no
     * Converter was specified then a default converter will be looked up based
     * on the target type of the property. If there is no default converter,
     * then a Constructor will be looked for on the target type which takes a
     * single String parameter. If such a Constructor exists it will be invoked.
     * </p>
     *
     * <p>
     * Only parameter values that are non-null and do not equal the empty String
     * will be converted and returned. So an input array with one entry equaling
     * the empty string, [""], will result in an <b>empty</b> List being
     * returned. Similarly, if a length three array is passed in with one item
     * equaling the empty String, a List of length two will be returned.
     * </p>
     *
     ...
     * @param scalarType if the declaredType is a collection, map or array then
     * this will be the type contained within the collection/map value/array,
     * otherwise the same as declaredType
     * @param validationInfo the validation metadata for the property if defined
     * @param errors a List into which ValidationError objects will be populated
     * for any errors discovered during conversion.
     * @return List of objects containing only objects of the desired type. It
     * is not guaranteed to be the same length as the values array passed in.
    ...
    /**
     * Inspects the given {@link ValidationMetadata} object to determine if the
     * given {@code values} should be trimmed. If so, then the trimmed values
     * are returned. Otherwise, the values are returned unchanged. If
     * {@code meta} is null, then the default action is taken, and the values
     * are trimmed. Either {@code values} or {@code meta} (or both) may be null.
     *
     * @param values - Array of values to evaluate
     * @param meta - Validation metadata to use to determine if trimming should
     * be used
     * @return Resulting array of optionally-trimmed values according to
     * validation metadata
     */
     ...
     * the same index so that we can validate all those properties together.
     */
    protected static class Row extends HashMap<ParameterName, String[]> {
         ...
         * any non-null values for the row so far.
         * 
         ...
         * false.
         *
         * @return true if the row had any non-empty values in it, otherwise
         * false.
         */
