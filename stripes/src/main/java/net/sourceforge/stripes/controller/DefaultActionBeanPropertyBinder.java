 ...
 *
 * <ul>
 *   <li>The ability to instantiate and set null intermediate properties in a property chain
 *   <li>The ability to use Lists and Maps directly for indexed properties
 *   <li>The ability to infer type information from the generics information in classes
 * </ul>
 *
 * @author Tim Fennell
 * @since Stripes 1.4
...
public class DefaultActionBeanPropertyBinder implements ActionBeanPropertyBinder {
   ...
   * ActionBean.
   *
   * <p>Individual property binding is delegated to the other interface method, bind(ActionBean,
  ...
  public ValidationErrors bind(ActionBean bean, ActionBeanContext context, boolean validate) {
    ...
    Map<String, ValidationMetadata> validationInfos =
        this.configuration.getValidationMetadataProvider().getValidationMetadata(bean.getClass());
    ...
    for (Map.Entry<ParameterName, String[]> entry : parameters.entrySet()) {
      ...
      try {
        String paramName = name.getName(); // exact name of the param in the request
        if (!StripesConstants.SPECIAL_URL_KEYS.contains(paramName)
            && !fieldErrors.containsKey(paramName)) {
          ...
          try {
            eval =
                new PropertyExpressionEvaluation(PropertyExpression.getExpression(paramName), bean);
          } catch (Exception e) {
            if (paramName.equals(context.getEventName())) continue;
            else throw e;
          }
          ...
          if (type == null && (validationInfo == null || validationInfo.converter() == null)) {
            if (!paramName.equals(context.getEventName())) {
              log.trace(
                  "Could not find type for property '",
                  name.getName(),
                  "' of '",
          ...
          } else if (!convertedValues.isEmpty()) {
            if (type != null) {
    ...
    if (request.isMultipart()) {
      ...
      while (fileParameterNames.hasMoreElements()) {
        ...
        log.trace(
            "Attempting to bind file parameter with name [",
            fileParameterName,
            "] and value: ",
            fileBean);
        ...
        if (fileBean != null) {
          try {
            ...
            log.debug(
                e,
                "Could not bind file property with name [",
                fileParameterName,
                "] and value: ",
                fileBean);
          }
   ...
   *
   * <p>By default checks to ensure that the expression is not attempting to bind into the
  ...
  protected boolean isBindingAllowed(PropertyExpressionEvaluation eval) {
    boolean allowed =
        BindingPolicyManager.getInstance(eval.getBean().getClass()).isBindingAllowed(eval);
    if (!allowed) {
      ...
      log.warn(
          ...
          param,
          "]. Use @Validate to allow binding in conjunction with @StrictBinding.");
    }
    return allowed;
  }

  /**
   * Invoked whenever an exception is thrown when attempting to bind a property to an ActionBean. By
   * default, logs some information about the occurrence, but could be overridden to do more
   ...
   */
  protected void handlePropertyBindingError(
      ActionBean bean,
      ParameterName name,
      List<Object> values,
      Exception e,
      ValidationErrors errors) {
    if (e instanceof NoSuchPropertyException noSuchPropertyException) {
      // No stack trace if it's a no such property exception
      log.debug(
          "Could not bind property with name [",
          name,
          "] to bean of type: ",
          bean.getClass().getSimpleName(),
          " : ",
          noSuchPropertyException.getMessage());
    } else {
      log.debug(
          e,
          "Could not bind property with name [",
      ...
      if (!parametersSubmitted.contains(name)) {
        try {
          ...
          handlePropertyBindingError(
              bean, new ParameterName(name), null, e, context.getValidationErrors());
        }
      }
    }
  }
  ...
  /**
   * In a lot of cases (and specifically during wizards) the Stripes form field writes out a hidden
   * field containing a set of field names. This is encrypted to stop the user from monkeying with
   * it. This method retrieves the list of field names, decrypts it and splits it out into a
  ...
  protected Collection<String> getFieldsPresentInfo(ActionBean bean) {
    ...
    if (fieldsPresent == null || fieldsPresent.isEmpty()) {
  ...
  /**
   * Internal helper method to bind one or more values to a single property on an ActionBean. If the
   * target type is an array of Collection, then all values are bound. If the target type is a
    ...
    if (targetType.isArray() && !valueType.isArray()) {
      ...
      if (EnumSet.class.isAssignableFrom(targetType) && Enum.class.isAssignableFrom(scalarType)) {
        ...
        collection =
            getConfiguration()
                .getObjectFactory()
                .newInstance((Class<? extends Collection>) targetType);
      }
   ...
   * field (but the field was present on the page). In all cases if the property is already null, or
   * intervening objects in a nested property are null, nothing is done. If the property is
   * non-null, it will be set to null. Unless the property is a collection, in which case it will be
   * clear()'d.
   *
          ...
          if (!wizard || fieldsOnPage.contains(propertyName)) {
            ...
            if (validationInfo.encrypted()) {
              for (int i = 0, n = values.length; i < n; i++) {
                if (values[i] != null) values[i] = CryptoUtil.decrypt(values[i]);
              }
            }
      ...
      for (Row row : rows.values()) {
        if (row.hasNonEmptyValues()) {
          for (Map.Entry<ParameterName, String[]> entry : row.entrySet()) {
            ParameterName name = entry.getKey();
            String[] values = entry.getValue();
            ValidationMetadata validationInfo = null;
            if (validationInfos != null) {
              validationInfo = validationInfos.get(name.getStrippedName());
            }

            if (validationInfo != null && validationInfo.requiredOn(context.getEventName())) {
              checkSingleRequiredField(
                  name.getName(), name.getStrippedName(), values, stripesReq, errors);
            }
          }
        } else {
  ...
  /**
   * Checks to see if a single field's set of values are 'present', where that is defined as having
  ...
  protected void checkSingleRequiredField(
    ...
    if (req.isMultipart() && (file = req.getFileParameterValue(name)) != null) {
      if (file.getSize() <= 0) {
        errors.add(name, new ScopedLocalizableError("validation.required", "valueNotPresent"));
      }
    }
    // And if not, see if any regular parameters were sent
    else if (values == null || values.length == 0) {
      ValidationError error = new ScopedLocalizableError("validation.required", "valueNotPresent");
   ...
   */
  protected void doPreConversionValidations(
    ...
    for (String value : values) {
      ...
      if (value != null && !value.isEmpty()) {
        if (validationInfo.minlength() != null && value.length() < validationInfo.minlength()) {
          ValidationError error =
              new ScopedLocalizableError(
                  "validation.minlength", "valueTooShort", validationInfo.minlength());
        ...
        if (validationInfo.maxlength() != null && value.length() > validationInfo.maxlength()) {
          ValidationError error =
              new ScopedLocalizableError(
                  "validation.maxlength", "valueTooLong", validationInfo.maxlength());
          error.setFieldValue(value);
          errors.add(error);
        }
        ...
        if (validationInfo.mask() != null && !validationInfo.mask().matcher(value).matches()) {
          ...
          ValidationError error =
              new ScopedLocalizableError("validation.mask", "valueDoesNotMatch");
  ...
  /**
   * Performs basic post-conversion validations on the properties of the ActionBean after they have
   * been converted to their rich type by the type conversion system. Validates single properties in
   * isolation from other properties.
   *
   ...
   */
  protected void doPostConversionValidations(
      ActionBean bean, Map<ParameterName, List<Object>> convertedValues, ValidationErrors errors) {
    ...
    for (Map.Entry<ParameterName, List<Object>> entry : convertedValues.entrySet()) {
      ...
      if (values.isEmpty() || validationInfo == null) {
        continue;
      }
      ...
      for (Object value : values) {
        ...
        if (value instanceof Number number) {
          ...
          if (validationInfo.minvalue() != null
            ...
            ValidationError error =
                new ScopedLocalizableError(
                    "validation.minvalue", "valueBelowMinimum", validationInfo.minvalue());
          ...
          if (validationInfo.maxvalue() != null
            ...
            ValidationError error =
                new ScopedLocalizableError(
                    "validation.maxvalue", "valueAboveMaximum", validationInfo.maxvalue());
            error.setFieldValue(String.valueOf(value));
            errors.add(name.getName(), error);
          }
        }
   ...
   * {@literal @}Validate annotation. The expression is evaluated once for each value converted. See
   * {@link net.sourceforge.stripes.validation.expression.ExpressionValidator} for details on how
   * this is implemented.
   *
   * @param bean the ActionBean whose property is being validated
   ...
   */
  protected void doExpressionValidation(
      ActionBean bean,
      ParameterName name,
      List<Object> values,
      ValidationMetadata validationInfo,
   ...
   * exception will be logged and values will not be converted. If no Converter was specified then a
   * default converter will be looked up based on the target type of the property. If there is no
   * default converter, then a Constructor will be looked for on the target type which takes a
   ...
   * and returned. So an input array with one entry equaling the empty string, [""], will result in
   * an <b>empty</b> List being returned. Similarly, if a length three array is passed in with one
   * item equaling the empty String, a List of length two will be returned.
   *
   ...
   * @param scalarType if the declaredType is a collection, map or array then this will be the type
   *     contained within the collection/map value/array, otherwise the same as declaredType
  ...
  protected List<Object> convert(
    ...
    // the following cases:
    // 1. We need to simply find a converter for the declared type of simple property
    ...
    for (String value : values) {
      ...
      if (value != null && !value.isEmpty()) {
        try {
          ...
          if (converter != null) {
            returnValue = converter.convert(value, returnType, errors);
          } else {
            try {
              returnValue =
                  getConfiguration()
                      .getObjectFactory()
                      .constructor(returnType, String.class)
                      .newInstance(value);
            } catch (StripesRuntimeException e) {
              log.debug(
                  "Could not find a way to convert the parameter ",
                  propertyName.getName(),
                  " to a ",
                  returnType.getSimpleName(),
                  ". No TypeConverter could be ",
                  "found and the class does not ",
  ...
  protected String[] trim(String[] values, ValidationMetadata meta) {
    if (values != null && values.length > 0 && (meta == null || meta.trim())) {
      ...
      for (int i = 0; i < values.length; i++) {
        if (values[i] != null) copy[i] = values[i].trim();
      }
      return copy;
    } else {
      return values;
  ...
  /**
   * An inner class that represents a "row" of form properties that all have the same index so that
   * we can validate all those properties together.
   */
