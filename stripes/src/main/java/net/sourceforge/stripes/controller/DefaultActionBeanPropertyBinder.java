...
public class DefaultActionBeanPropertyBinder implements ActionBeanPropertyBinder {
   ...
   public ValidationErrors bind( ActionBean bean, ActionBeanContext context, boolean validate ) {
      ...
      Map<String, ValidationMetadata> validationInfos = _configuration.getValidationMetadataProvider().getValidationMetadata(bean.getClass());
      ...
      for ( String name : getFieldsPresentInfo(bean) ) {
         if ( !parametersSubmitted.contains(name) ) {
            try {
               ...
               handlePropertyBindingError(bean, new ParameterName(name), null, e, context.getValidationErrors());
            }
         }
      }
   }
   ...
   protected void bindNonNullValue( ActionBean bean, PropertyExpressionEvaluation propertyEvaluation, List<Object> valueOrValues, Class targetType,
      ...
      if ( targetType.isArray() && !valueType.isArray() ) {
         ...
         if ( EnumSet.class.isAssignableFrom(targetType) && Enum.class.isAssignableFrom(scalarType) ) {
            ...
            collection = getConfiguration().getObjectFactory().newInstance((Class<? extends Collection>)targetType);
         }
    ...
    */
   @SuppressWarnings({ "unchecked", "rawtypes" })
   protected List<Object> convert( ActionBean bean, ParameterName propertyName, String[] values, Class<?> declaredType, Class<?> scalarType,
      ...
      TypeConverterFactory factory = _configuration.getTypeConverterFactory();
    ...
    */
   protected void doExpressionValidation( ActionBean bean, ParameterName name, List<Object> values, ValidationMetadata validationInfo,
         ...
         for ( Object value : values ) {
            ...
            if ( value instanceof Number ) {
               ...
               if ( validationInfo.minvalue() != null && number.doubleValue() < validationInfo.minvalue() ) {
                  ValidationError error = new ScopedLocalizableError("validation.minvalue", "valueBelowMinimum", validationInfo.minvalue());
                  error.setFieldValue(String.valueOf(value));
                  errors.add(name.getName(), error);
               }

               if ( validationInfo.maxvalue() != null && number.doubleValue() > validationInfo.maxvalue() ) {
                  ValidationError error = new ScopedLocalizableError("validation.maxvalue", "valueAboveMaximum", validationInfo.maxvalue());
                  error.setFieldValue(String.valueOf(value));
                  errors.add(name.getName(), error);
               }
            }
      ...
      for ( String value : values ) {
         ...
         if ( value != null && value.length() > 0 ) {
            if ( validationInfo.minlength() != null && value.length() < validationInfo.minlength() ) {
               ValidationError error = new ScopedLocalizableError("validation.minlength", "valueTooShort", validationInfo.minlength());
            ...
            if ( validationInfo.maxlength() != null && value.length() > validationInfo.maxlength() ) {
               ValidationError error = new ScopedLocalizableError("validation.maxlength", "valueTooLong", validationInfo.maxlength());
               error.setFieldValue(value);
               errors.add(error);
            }
            ...
            if ( validationInfo.mask() != null && !validationInfo.mask().matcher(value).matches() ) {
               ...
               ValidationError error = new ScopedLocalizableError("validation.mask", "valueDoesNotMatch");
      ...
      if ( fieldsPresent == null || "".equals(fieldsPresent) ) {
         if ( isWizard && !CollectionUtil.contains(wizard.startEvents(), ctx.getEventName()) ) {
            throw new StripesRuntimeException(
                  "Submission of a wizard form in Stripes absolutely requires that " + "the hidden field Stripes writes containing the names of the fields "
                        + "present on the form is present and encrypted (as Stripes write it). "
                        + "This is necessary to prevent a user from spoofing the system and " + "getting around any security/data checks.");
         } else {
   ...
   protected void validateRequiredFields( Map<ParameterName, String[]> parameters, ActionBean bean, ValidationErrors errors ) {
      ...
      if ( validationInfos != null ) {
         ...
         for ( Map.Entry<String, ValidationMetadata> entry : validationInfos.entrySet() ) {
            ...
            if ( validationInfo.requiredOn(context.getEventName()) && !indexedParams.contains(propertyName) ) {
            ...
            if ( row.hasNonEmptyValues() ) {
               for ( Map.Entry<ParameterName, String[]> entry : row.entrySet() ) {
                  ...
                  if ( validationInfo != null && validationInfo.requiredOn(context.getEventName()) ) {
                     checkSingleRequiredField(name.getName(), name.getStrippedName(), values, stripesReq, errors);
                  }
               }
            } else {
   ...
   protected static class Row extends HashMap<ParameterName, String[]> {
      ...
      public String[] put( ParameterName key, String[] values ) {
         if ( !hasNonEmptyValues ) {
            hasNonEmptyValues = (values != null) && (values.length > 0) && (values[0] != null) && (values[0].trim().length() > 0);
