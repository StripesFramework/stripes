 ...
 * into RequestScope for each request that references the bean, thereby allowing the rest of Stripes
 * to treat it like any other ActionBean.
 *
 * <p>Since session scope ActionBeans are not generally encouraged by the author, very few
 * allowances will be made in Stripes to accommodate session scope beans. This means that additional
 * mechanisms to handle session scope beans do not exist. However, there are general mechanisms
 * built in to Stripes that will allow you to overcome most if not all issues that arise from
 * Session scoping ActionBeans.
 *
 ...
 * HttpServletRequest to find out if empty values were submitted for fields, and null out just those
 * fields.
 *
 ...
 * generated, and take action accordingly.
 *
 * <p>Lastly, an alternative to session scoping for wizard pattern/page-spanning forms that
 * ActionBean authors may wish to consider is the use of the {@link
 * net.sourceforge.stripes.tag.WizardFieldsTag} which will carry all the fields submitted in the
 * request into the next request by writing hidden form fields.
 *
