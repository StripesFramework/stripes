...
/**
 * <p>
 * Encapsulates information about the current request. Also provides access to
 * the underlying Servlet API should you need to use it for any reason.</p>
 *
 * <p>
 * Developers should generally consider subclassing ActionBeanContext to provide
 * a facade to contextual state for their application. Type safe getters and
...
public class ActionBeanContext {
    ...
    /**
     * Used by the DispatcherServlet to set the HttpServletRequest for the
     * current request
     *
    ...
    /**
     * Retrieves the HttpServletResponse that is associated with the current
     * request.
     *
    ...
    /**
     * Used by the DispatcherServlet to set the HttpServletResponse that is
     ...
     * in which the current request is being processed.
     *
     ...
     * which the current request is being processed.
     *
    ...
    /**
     * Supplies the name of the event being handled. While a specific method is
     * usually invoked on an ActionBean, through the use of default handlers
     * ambiguity can arise. This allows ActionBeans to definitively know the
     ...
     * Lazily initialized the set of errors, and will never return null.
     *
    ...
    /**
     * <p>
     * Returns the default set of non-error messages associated with the current
     * request. Guaranteed to always return a List, though the list may be
     * empty. It is envisaged that messages will normally be added to the
     ...
     *
     * <p>
     * To remove messages from the current request fetch the list of messages
     * and invoke remove() or clear(). Messages will be made available to JSPs
     * during the current request and in the subsequent request if a redirect is
     * issued.</p>
     *
     * @return a List of Message objects associated with the current request,
     * never null.
    ...
    /**
     * <p>
     * Returns the set of non-error messages associated with the current request
     * under the specified key. Can be used to manage multiple lists of
     * messages, for different purposes. Guaranteed to always return a List,
     * though the list may be empty. It is envisaged that messages will normally
     * be added to the request as follows:</p>
     *
     * <pre>
     ...
     * <p>
     * To remove messages from the current request fetch the list of messages
     * and invoke remove() or clear().</p>
     *
     * <p>
     * Messages are stored in a
     * {@link net.sourceforge.stripes.controller.FlashScope} for the current
     * request. This means that they are available in request scope using the
     * supplied key during both this request, and the subsequent request if it
     * is the result of a redirect.</p>
     *
    ...
    /**
     * Gets the Locale that is being used to service the current request. This
     * is *not* the value that was submitted in the request, but the value
     * picked by the configured LocalePicker which takes into consideration the
     * locales preferred in the request.
     *
     ...
     * construct a source page resolution cannot be found in the request.
     * @see #getSourcePage()
     */
    ...
    /**
     * <p>
     * Returns the context-relative path to the page from which the user
    ...
    /**
     * Returns a String with the name of the event for which the instance holds
     * context, and the set of validation errors, if any.
     * @return A stringified representation of this action bean context.
     */
    @Override
