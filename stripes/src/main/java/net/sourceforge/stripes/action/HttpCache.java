 ...
 *       set.
 *   <li>{@code @HttpCache(allow=true)} - Same as above.
 *   <li>{@code @HttpCache(allow=false)} - Set headers to disable caching and immediately expire the
 *       document.
 *   <li>{@code @HttpCache(expires=600)} - Caching is allowed. The document expires in 10 minutes.
 * </ul>
 *
 * @author Ben Gunter
 * @since Stripes 1.5
...
public @interface HttpCache {
   ...
   * false, then this value is ignored and zero is used. If {@link #allow()} is true and this value
   * is less than zero, then no Expires header is sent.
   */
