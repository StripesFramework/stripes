...
/**
 * A non-error message class that can localize (or at least externalize) the message String in a
 * resource bundle. The bundle used is the Stripes error message bundle, which can be configured but
 * by default is called 'StripesResources.properties'. In all other ways this class behaves like
 * it's parent {@link SimpleMessage}.
 *
 * @author Tim Fennell
 */
public class LocalizableMessage extends SimpleMessage {
  ...
  /**
   * Method responsible for using the information supplied to the message object to find a message
   * template. In this class this is done simply by looking up the resource corresponding to the
   * messageKey supplied in the constructor.
   */
  @Override
  ...
  /**
   * Generated equals method which will return true if the other object is of the same type as this
   * instance, and would produce the same user message.
   *
