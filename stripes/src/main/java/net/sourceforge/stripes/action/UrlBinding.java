 ...
 *
 * <p>Stripes supports "Clean URLs" through the {@link UrlBinding} annotation. Parameters may be
 ...
 * be any string.
 *
 * <p>The special parameter name $event may be used to embed the event name in a clean URL. For
 * example, given {@code @UrlBinding("/foo/{$event}")} the "bar" event could be invoked with the URL
 * /foo/bar.
 *
 * <p>Clean URL parameters can be assigned default values using the {@code =} operator. For example,
 ...
 * the {@code @UrlBinding}.
 *
 * <p>Clean URLs support both prefix mapping ({@code /action/foo/{bar}}) and extension mapping
 * ({@code /foo/{bar}.action}). Any number of parameters and/or literals may be omitted from the end
 * of a request URL.
 *
 * @author Tim Fennell
 */
