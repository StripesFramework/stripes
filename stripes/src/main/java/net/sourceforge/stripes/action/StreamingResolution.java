...
public class StreamingResolution implements Resolution {
   ...
   private       boolean           _rangeSupport = false;
   private       List<Range<Long>> _byteRanges;
   ...
   final public void execute( HttpServletRequest request, HttpServletResponse response ) throws Exception {
        ...
        -*/
      if ( _rangeSupport && (_length >= 0) && (_inputStream != null) ) {
         _byteRanges = parseRangeHeader(request.getHeader("Range"));
      }
   ...
   protected void applyHeaders( HttpServletResponse response ) {
      ...
      if ( (_byteRanges == null) || (_byteRanges.size() == 1) ) {
      ...
      if ( _characterEncoding != null ) {
         response.setCharacterEncoding(_characterEncoding);
      }
      ...
      if ( _length >= 0 ) {
         if ( _byteRanges == null ) {
            ...
            response.addHeader("Content-Length", Long.toString(_length));
         } else if ( _byteRanges.size() == 1 ) {
            Range<Long> byteRange;

            byteRange = _byteRanges.get(0);
            response.setHeader("Content-Length", Long.toString(byteRange.getEnd() - byteRange.getStart() + 1));
            response.setHeader("Content-Range", "bytes " + byteRange.getStart() + "-" + byteRange.getEnd() + "/" + _length);
         }
      }

      // Set Last-Modified header
      if ( _lastModified >= 0 ) {
         response.setDateHeader("Last-Modified", _lastModified);
      }

      // For Content-Disposition spec, see http://www.ietf.org/rfc/rfc2183.txt
      if ( _attachment || _filename != null ) {
         ...
         StringBuilder header = new StringBuilder(_attachment ? "attachment" : "inline");
         if ( _filename != null ) {
            String escaped = _filename.replace("\"", "\\\"");
            header.append(";filename=\"").append(escaped).append("\"");
         }
         if ( _lastModified >= 0 ) {
            SimpleDateFormat format = new SimpleDateFormat(RFC_822_DATE_FORMAT);
            String value = format.format(new Date(_lastModified));
            header.append(";modification-date=\"").append(value).append("\"");
         }
         if ( _length >= 0 ) {
            header.append(";size=").append(_length);
         }
   ...
   protected List<Range<Long>> parseRangeHeader( String value ) {
      ...
      for ( String byteRangeSpec : byteRangeSet ) {
         ...
         if ( (firstBytePos == null) && (lastBytePos == null) ) {
            ...
            firstBytePos = _length - lastBytePos;
            lastBytePos = _length - 1;
      ...
      while ( i.hasNext() ) {
         ...
         if ( lastEnd >= range.getStart() ) {
            ...
            if ( (range.getStart() >= _length) || (range.getStart() > range.getEnd()) ) {
               i.remove();
            } else {
   ...
   protected void stream( HttpServletResponse response ) throws Exception {
      ...
      if ( _reader != null ) {
         ...
         try {
            ...
            while ( (length = _reader.read(buffer)) != -1 ) {
         ...
         try {
            ...
            if ( _byteRanges == null ) {
               ...
               for ( Range<Long> byteRange : _byteRanges ) {
                  // See RFC 2616 section 14.16
                  if ( _byteRanges.size() > 1 ) {
                     out.print("--" + MULTIPART_BOUNDARY + "\r\n");
                     out.print("Content-Type: " + _contentType + "\r\n");
                     out.print("Content-Range: bytes " + byteRange.getStart() + "-" + byteRange.getEnd() + "/" + _length + "\r\n");
                     out.print("\r\n");
                  }
                  ...
                  while ( (length = _inputStream.read(buffer, 0, (int)Math.min(buffer.length, byteRange.getEnd() + 1 - count))) != -1 ) {
                     out.write(buffer, 0, length);
                     count += length;
         ...
         throw new StripesRuntimeException(
               "A StreamingResolution was constructed without " + "supplying a Reader or InputStream, but stream() was not overridden. Please "
