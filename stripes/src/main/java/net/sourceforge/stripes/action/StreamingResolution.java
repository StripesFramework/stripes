...
/**
 * <p>
 * Resolution for streaming data back to the client (in place of forwarding the
 * user to another page). Designed to be used for streaming non-page data such
 * as generated images/charts and XML islands.</p>
 *
 * <p>
 * Optionally supports the use of a file name which, if set, will cause a
 * Content-Disposition header to be written to the output, resulting in a "Save
 * As" type dialog box appearing in the user's browser. If you do not wish to
 * supply a file name, but wish to achieve this behaviour, simple supply a file
 * name of "".</p>
 *
 * <p>
 * StreamingResolution is designed to be subclassed where necessary to provide
 * streaming output where the data being streamed is not contained in an
 * InputStream or Reader. This would normally be done using an anonymous inner
 * class as follows:</p>
 *
 * <pre>
...
public class StreamingResolution implements Resolution {
     ...
     * (usually using an anonymous inner class. If this constructor is used, and
     * stream() is not overridden then an exception will be thrown!
     *
     * @param contentType the content type of the data in the stream (e.g.
     * image/png)
     */
    ...
    /**
     * Constructor that builds a StreamingResolution that will stream binary
     * data back to the client and identify the data as being of the specified
     * content type.
     *
     * @param contentType the content type of the data in the stream (e.g.
     * image/png)
     * @param inputStream an InputStream from which to read the data to return
     * to the client
     */
    ...
    /**
     * Constructor that builds a StreamingResolution that will stream character
     * data back to the client and identify the data as being of the specified
     * content type.
     *
     * @param contentType the content type of the data in the stream (e.g.
     * text/xml)
     * @param reader a Reader from which to read the character data to return to
     * the client
     */
    ...
    /**
     * Constructor that builds a StreamingResolution that will stream character
     * data from a String back to the client and identify the data as being of
    ...
    /**
     * Sets the filename that will be the default name suggested when the user
     * is prompted to save the file/stream being sent back. If the stream is not
     * for saving by the user (i.e. it should be displayed or used by the
     * browser) this value should not be set.
     *
    ...
    /**
     * Sets the character encoding that will be set on the request when
     * executing this resolution. If none is set, then the current character
     * encoding (either the one selected by the LocalePicker or the container
     * default one) will be used.
     *
     * @param characterEncoding the character encoding to use instead of the
     * default
     */
     ...
     * browser may be able to apply it to the downloaded file. If this property
     * is unset, the modification-date parameter will be omitted.
     *
     ...
     * reported in the HTTP header. This may help with file download progress
     * indicators. If this property is unset, the size parameter will be
     * omitted.
     *
     ...
     * (Defaults to true).
     *
     * @param attachment Whether the content should be treated as an attachment,
     ...
     * (Defaults to false). Besides setting this flag, the ActionBean also needs
     * to set the length of the response and provide an
     * {@link InputStream}-based input. Reasons for disabling byte range
     * serving:
     * <ul>
     * <li>The stream method is overridden and does not support byte range
     * serving</li>
     * <li>The input to this {@link StreamingResolution} was created on-demand,
     * and retrieving in byte ranges would redo this process for every byte
     * range.</li>
     * </ul>
     ...
     * OutputStream or PrinterWriter, using a moderately sized buffer to ensure
     * that the operation is reasonably efficient. Once the InputStream or
     * Reader signaled the end of the stream, close() is called on it.
     *
     * @param request the HttpServletRequest being processed
     * @param response the paired HttpServletResponse
     * @throws IOException if there is a problem accessing one of the streams or
     * reader/writer objects used.
     */
     ...
     * bytes (byte offsets 500-999, inclusive): bytes=500-600,601-999
     ...
     * this method should be overridden to stream back data other than data
     * supplied by an InputStream or a Reader supplied to a constructor. If not
     * implementing byte range serving, be sure not to set rangeSupport to true.
     * </p>
     *
     * <p>
     * If an InputStream or Reader was supplied to a constructor, this
     * implementation uses a moderately sized buffer to stream data from it to
     * the response to make the operation reasonably efficient, and closes the
     * InputStream or the Reader. If an IOException occurs when closing it, that
     * exception will be logged as a warning, and <em>not</em> thrown to avoid
     * masking a possibly previously thrown exception.
     * </p>
     *
     * @param response the HttpServletResponse from which either the output
     * stream or writer can be obtained
