 ...
 *
 * <p>For example, suppose you have two ActionBeans with the URL bindings <code>/foo/{param}/bar
 * </code> and <code>/foo/{param}/blah</code>. The paths {@code /foo} and {@code /foo/X} -- while
 * legal, since any number of parameters or literals may be omitted from the end of a clean URL --
 * match both of the URL bindings. Since Stripes cannot determine from the URL the ActionBean to
 * which to dispatch the request, it throws this exception to indicate the conflict.
 *
 * @author Ben Gunter
 * @since Stripes 1.5.1
 */
public class UrlBindingConflictException extends StripesRuntimeException {
  ...
  protected static String getMessage(
      Class<? extends ActionBean> targetClass, String path, Collection<String> matches) {
    return (targetClass == null ? "" : "Failure generating URL for " + targetClass + ". ")
        + "The path "
        + path
        + " cannot be mapped to a single ActionBean because multiple "
   ...
   */
  public UrlBindingConflictException(
      String message,
   ...
   * thrown. If the exception occurred while dispatching a request, then this property will be null
   * since the path cannot be associated with an ActionBean class. However, if it is thrown while
   * generating a URL that is intended to point to an ActionBean, then this property will indicate
   * the class that was being targeted.
   */
