...
/**
 * Default ExceptionHandler implementation that makes it easy for users to extend and add custom
 * handling for different types of exception. When extending this class methods can be added that
 * meet the following requirements:
 *
 * <ul>
 *   <li>Methods must be public
 *   <li>Methods must be non-abstract
 *   <li>Methods must have exactly three parameters
 *   <li>The first parameter type must be Throwable or a subclass thereof
 *   <li>The second and third arguments must be of type HttpServletRequest and HttpServletResponse
 *       respectively
 *   <li>Methods may <i>optionally</i> return a Resolution in which case the resolution will be
 *       executed
 * </ul>
 *
 ...
 * container.
 *
 * <p>The following are examples of method signatures that might be added by subclasses:
 *
 * <pre>
...
public class DefaultExceptionHandler implements ExceptionHandler {
    ...
    try {
      ...
      if (proxy != null) {
        ...
        Resolution resolution =
            handle((FileUploadLimitExceededException) throwable, request, response);
        if (resolution != null) resolution.execute(request, response);
      } else if (throwable instanceof SourcePageNotFoundException) {
        Resolution resolution = handle((SourcePageNotFoundException) throwable, request, response);
        if (resolution != null) resolution.execute(request, response);
      } else {
   ...
   * validation errors occur on a request but the source page cannot be determined from the request
   * parameters. Such a condition generally arises during application development when, for example,
   * a parameter is accidentally omitted from a generated hyperlink or AJAX request.
   *
   * <p>In the past, it was very difficult to determine what validation errors triggered the
   * exception. This method returns a {@link ValidationErrorReportResolution}, which sends a simple
   * HTML response to the client that very clearly details the validation errors.
   *
   * <p>In production, most applications will provide their own handler for {@link
   * SourcePageNotFoundException} by extending this class and overriding this method.
   *
   ...
   *       Stripes components rely on the presence of this wrapper, yet it cannot be created
   *       normally.
   *   <li>It happens before the request lifecycle has begun. There is no {@link ExecutionContext},
   *       {@link ActionBeanContext}, or {@link ActionBean} associated with the request yet.
   *   <li>None of the request parameters in the POST body can be read without risking denial of
   *       service. That includes the {@code _sourcePage} parameter that indicates the page from
   *       which the request was submitted.
   * </ul>
   *
   * <p>This exception handler makes an attempt to handle the exception as gracefully as possible.
   * It relies on the HTTP Referer header to determine where the request was submitted from. It uses
   ...
   * @param request The servlet request
   * @param response The servlet response
   * @return A {@link Resolution} to forward to the path returned by {@link
   *     #getFileUploadExceededExceptionPath(HttpServletRequest)}
   * @throws FileUploadLimitExceededException If {@link
   *     #getFileUploadExceededExceptionPath(HttpServletRequest)} returns null or this method is
   *     unable for any other reason to forward to the error page
   */
  protected Resolution handle(
      FileUploadLimitExceededException exception,
      HttpServletRequest request,
      HttpServletResponse response)
    ...
    try {
      ...
      context = configuration.getActionBeanContextFactory().getContextInstance(wrapper, response);
    ...
    LocalizableError error =
        new LocalizableError(
            "validation.file.postBodyTooBig", format.format(max), format.format(posted));
    if (fieldName == null) context.getValidationErrors().addGlobalError(error);
  ...
  /**
   * Get the path to which the {@link Resolution} returned by {@link
   * #handle(FileUploadLimitExceededException, HttpServletRequest, HttpServletResponse)} should
   * forward to report the error. The default implementation attempts to determine this from the
   * HTTP Referer header. If it is unable to do so, it returns null. Subclasses may override this
   * method to return whatever they wish. The return value must be relative to the application
   * context root.
   *
  ...
  protected String getFileUploadExceededExceptionPath(HttpServletRequest request) {
    ...
    if (contextPath.length() > 1) {
      ...
      if (!path.startsWith(contextPath + "/")) return null;
  ...
  /**
   * Adds a class to the set of configured delegate handlers. Examines all the methods on the class
   * looking for public non-abstract methods with a signature matching that described in the class
   * level javadoc. Each method is wrapped in a HandlerProxy and stored in a cache by the exception
   * type it takes.
   *
  ...
  /**
   * Adds an object instance to the set of configured handles. Examines all the methods on the class
   * looking for public non-abstract methods with a signature matching that described in the class
   * level javadoc. Each method is wrapped in a HandlerProxy and stored in a cache by the exception
   * type it takes.
   *
  ...
  protected void addHandler(Object handler) {
    ...
    for (Method method : methods) {
      ...
      if (previous != null) {
        log.warn(
            "More than one exception handler for exception type ",
            type,
            " in ",
  ...
  /**
   * Unwraps the throwable passed in. If the throwable is a ServletException and has a root case,
