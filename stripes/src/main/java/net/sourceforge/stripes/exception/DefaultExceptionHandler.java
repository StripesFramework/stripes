...
/**
 * <p>
 * Default ExceptionHandler implementation that makes it easy for users to
 * extend and add custom handling for different types of exception. When
 * extending this class methods can be added that meet the following
 * requirements:</p>
 *
 * <ul>
 ...
 * resolution will be executed</li>
 * </ul>
 *
 * <p>
 * When an exception is caught the exception handler attempts to find a method
 * that can handle that type of exception. If none is found the exception's
 * super-types are iterated through and methods looked for which match the
 * super-types. If a matching method is found it will be invoked. Otherwise the
 * exception will simply be rethrown by the exception handler - though first it
 * will be wrapped in a StripesServletException if necessary in order to make it
 * acceptable to the container.</p>
 *
 * <p>
 * The following are examples of method signatures that might be added by
 * subclasses:</p>
 *
 * <pre>
...
public class DefaultExceptionHandler implements ExceptionHandler {
    ...
    protected static class HandlerProxy {
        ...
        /**
         * Constructs a new HandlerProxy that will tie together the instance and
         * method used.
         * @param handler - Handler that this handler should delegate to.
         * @param handlerMethod - Method on the handler to delegate to.
         */
         ...
         * @param t - The throwable which should be handled by this exception handler
         * @param req - HTTP servlet request
         * @param res - HTTP servlet response
         * @throws java.lang.Exception If an error occurs handling this exception
         */
    ...
    /**
     * Implementation of the ExceptionHandler interface that attempts to find a
     * method that is capable of handing the exception. If it finds one then it
     * is delegated to, and if it returns a resolution it will be executed.
     * Otherwise rethrows any unhandled exceptions, wrapped in a
    ...
    /**
     * <p>
     * A default handler for {@link SourcePageNotFoundException}. That exception
     * is thrown when validation errors occur on a request but the source page
     * cannot be determined from the request parameters. Such a condition
     * generally arises during application development when, for example, a
     * parameter is accidentally omitted from a generated hyperlink or AJAX
     * request.
     * </p>
     * <p>
     * In the past, it was very difficult to determine what validation errors
     * triggered the exception. This method returns a
     * {@link ValidationErrorReportResolution}, which sends a simple HTML
     * response to the client that very clearly details the validation errors.
     * </p>
     * <p>
     ...
     * <ul>
     * <li>The exception is thrown during construction of the
     * {@link StripesRequestWrapper}. Many Stripes components rely on the
     * presence of this wrapper, yet it cannot be created normally.</li>
     * <li>It happens before the request lifecycle has begun. There is no {@link ExecutionContext},
     * {@link ActionBeanContext}, or {@link ActionBean} associated with the
     * request yet.</li>
     * <li>None of the request parameters in the POST body can be read without
     * risking denial of service. That includes the {@code _sourcePage}
     * parameter that indicates the page from which the request was
     * submitted.</li>
     * </ul>
     * <p>
     * This exception handler makes an attempt to handle the exception as
     * gracefully as possible. It relies on the HTTP Referer header to determine
     * where the request was submitted from. It uses introspection to guess the
     * field name of the {@link FileBean} field that exceeded the POST limit. It
     * instantiates an {@link ActionBean} and {@link ActionBeanContext} and adds
     * a validation error to report the field name, maximum POST size, and
     * actual POST size. Finally, it forwards to the referer.
     * </p>
     * <p>
     * While this is a best effort, it won't be ideal for all situations. If
     * this method is unable to handle the exception properly for any reason, it
     * rethrows the exception. Subclasses can call this method in a {@code try}
     * block, providing additional processing in the {@code catch} block.
     * </p>
     * <p>
     ...
     * {@link #getFileUploadExceededExceptionPath(HttpServletRequest)} returns
     * null or this method is unable for any other reason to forward to the
     * error page
     */
     ...
     * to determine this from the HTTP Referer header. If it is unable to do so,
     * it returns null. Subclasses may override this method to return whatever
     * they wish. The return value must be relative to the application context
     * root.
     *
     ...
     * methods.
     *
     * @throws java.lang.Exception If an error occurs initializing this handler
     */
    ...
    /**
     * Adds a class to the set of configured delegate handlers. Examines all the
     * methods on the class looking for public non-abstract methods with a
     * signature matching that described in the class level javadoc. Each method
     * is wrapped in a HandlerProxy and stored in a cache by the exception type
     * it takes.
     *
    ...
    /**
     * Adds an object instance to the set of configured handles. Examines all
     * the methods on the class looking for public non-abstract methods with a
     * signature matching that described in the class level javadoc. Each method
     * is wrapped in a HandlerProxy and stored in a cache by the exception type
     * it takes.
     *
    ...
    protected void addHandler(Object handler) throws Exception {
        ...
        for (Method method : methods) {
            ...
            if (!Throwable.class.isAssignableFrom(parameters[0])) {
                continue;
            }
            if (!HttpServletRequest.class.equals(parameters[1])) {
                continue;
            }
            if (!HttpServletResponse.class.equals(parameters[2])) {
                continue;
            }
            if (handler == this && method.getName().equals("handle")
                    && Throwable.class.equals(parameters[0])) {
                continue;
            }
     ...
     * @return Returns the configuration object for this exception handler.
     */
    ...
    /**
     * Unwraps the throwable passed in. If the throwable is a ServletException
     * and has a root case, the root cause is returned, otherwise the throwable
     * is returned as is.
     *
