...
/**
 * <p>
 * An alternative implementation of {@link ExceptionHandler} that discovers and
 * automatically configures individual {@link AutoExceptionHandler} classes to
 * handle specific types of exceptions. This implementation is most useful when
 * ActionBeans may produce many different types of exceptions and it is
 * desirable to separate exception handling logic for different groups or
 * classes of exceptions. Using this approach multiple AutoExceptionHandlers can
 * be configured simultaneously but do not have to be co-located.</p>
 *
 * <p>
 * Searches for implementations of AutoExceptionHandler using the same mechanism
 * as is used to discover ActionBean implementations - a search of the classpath
 * for classes that implement the interface. The search requires one parameter,
 * DelegatingExceptionHandler.Packages, which should contain a comma separated
 * list of root packages to search for AutoExceptionHandler classes. If this
 * parameter is <i>not</i> specified, the DelegatingExceptionHandler will use
 * the configuration parameter that is used for discovering ActionBean instances
 * (ActionResolver.Packages). The configuration parameter is usually specified
 * as an init-param for the Stripes Filter, e.g.:</p>
 *
 * <pre>
 *&lt;init-param&gt;
 ...
 *
 * <p>
 * When the {@link #handle(Throwable, HttpServletRequest, HttpServletResponse)}
 * is invoked the set of AutoExceptionHandlers is examined to find the handler
 * with the most specific signature that is capable of handling the exception.
 * If no handler is available to handle the exception type supplied then the
 * exception will be rethrown; if the exception is not a ServletException it
 * will be wrapped in a StripesServletException before being rethrown.</p>
 *
 * <p>
 * If it is desirable to ensure that all exceptions are handled simply create an
 * AutoExceptionHandler that takes with {@link java.lang.Exception} (preferable)
 * or {@link java.lang.Throwable} (this may catch unhandlable errors like
 * OutOfMemoryError).</p>
 *
...
public class DelegatingExceptionHandler extends DefaultExceptionHandler {
     ...
     * {@link ResolverUtil} to find implementations of AutoExceptionHandler.
     * Each implementation found is then examined and cached by calling
     * {@link #addHandler(Class)}
     *
     * @param configuration the Configuration for this Stripes application
     * @throws Exception thrown if any of the discovered handler types cannot be
     * safely instantiated
     */
    @Override
     ...
     * packages specified in Configuration using the {@link ResolverUtil} class.
     *
     * @return a set of Class objects that represent subclasses of
