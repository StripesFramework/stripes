 ...
 *
 * <p>Searches for implementations of AutoExceptionHandler using the same mechanism as is used to
 * discover ActionBean implementations - a search of the classpath for classes that implement the
 * interface. The search requires one parameter, DelegatingExceptionHandler.Packages, which should
 * contain a comma separated list of root packages to search for AutoExceptionHandler classes. If
 * this parameter is <i>not</i> specified, the DelegatingExceptionHandler will use the configuration
 * parameter that is used for discovering ActionBean instances (ActionResolver.Packages). The
 * configuration parameter is usually specified as an init-param for the Stripes Filter, e.g.:
 *
 * <pre>
 * &lt;init-param&gt;
 ...
 * </pre>
 *
 * <p>When the {@link #handle(Throwable, HttpServletRequest, HttpServletResponse)} is invoked the
 * set of AutoExceptionHandlers is examined to find the handler with the most specific signature
 * that is capable of handling the exception. If no handler is available to handle the exception
 * type supplied then the exception will be rethrown; if the exception is not a ServletException it
...
public class DelegatingExceptionHandler extends DefaultExceptionHandler {
  ...
  /**
   * Looks up the filters as defined in the Configuration and then invokes the {@link ResolverUtil}
   * to find implementations of AutoExceptionHandler. Each implementation found is then examined and
   * cached by calling {@link #addHandler(Class)}
   *
   * @param configuration the Configuration for this Stripes application
   * @throws Exception thrown if any of the discovered handler types cannot be safely instantiated
   */
  @Override
  ...
  protected Set<Class<? extends AutoExceptionHandler>> findClasses() {
    ...
    if (packages == null || packages.length == 0) {
      ...
      if (!classes.isEmpty()) {
        ...
        log.info(
            "Autodiscovery found no implementations of AutoExceptionHandler. Using ",
            "the value of '",
            AnnotatedClassActionResolver.PACKAGES,
            "' instead.");
        packages =
            StringUtil.standardSplit(bootstrap.getProperty(AnnotatedClassActionResolver.PACKAGES));
      }
    }
