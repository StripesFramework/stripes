...
/**
 * Mock implementation of an HttpServletRequest object. Allows for setting most values that are
 * likely to be of interest (and can always be subclassed to affect others). Of key interest and
 * perhaps not completely obvious, the way to get request parameters into an instance of
 * MockHttpServletRequest is to fetch the parameter map using getParameterMap() and use the put()
 * and putAll() methods on it. Values must be String arrays. Examples follow:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>It should also be noted that unless you generate an instance of MockHttpSession (or another
 * implementation of HttpSession) and set it on the request, then your request will <i>never</i>
 * have a session associated with it.
 *
 * @author Tim Fennell
 * @since Stripes 1.1.1
 */
public class MockHttpServletRequest implements HttpServletRequest {
  ...
  /**
   * Minimal constructor that makes sense. Requires a context path (should be the same as the name
   ...
   * methods. If the header is a date header it should be set with a Long. If the header is an Int
   * header it should be set with an Integer.
   */
   ...
   * modifiable parameter map. As a result it can be used to insert parameters when constructing the
   * request.
   */
  ...
  /**
   * Returns an instance of MockRequestDispatcher that just records what URLs are forwarded to or
   * included. The results can be examined later by calling getForwardUrl() and getIncludedUrls().
   */
