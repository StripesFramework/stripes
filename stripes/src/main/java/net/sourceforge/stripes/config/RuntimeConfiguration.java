...
/**
 * Configuration class that uses the BootstrapPropertyResolver to look for configuration values, and
 * when it cannot find a value, falls back on the DefaultConfiguration to supply default values. In
 * general, the RuntimeConfiguration will operate in the following pattern:
 *
 * <ul>
 *   <li>Look for the value of a configuration property in the BootstrapProperties
 *   <li>If the value exists, the configuration will attempt to use it (usually to instantiate a
 *       class). If an exception occurs, the RuntimeConfiguration will throw an exception and not
 *       provide a value. In most cases this will be fatal!
 *   <li>If the value does not exist, the default from DefaultConfiguration will be used.
 * </ul>
 *
 * @author Tim Fennell
 */
public class RuntimeConfiguration extends DefaultConfiguration {
  ...
  /**
   * Looks for a list of class names separated by commas under the configuration key {@link
   * #CORE_INTERCEPTOR_LIST}. White space surrounding the class names is trimmed, the classes
   * instantiated and then stored under the lifecycle stage(s) they should intercept.
   *
  ...
  /**
   * Looks for a list of class names separated by commas under the configuration key {@link
   * #INTERCEPTOR_LIST}. White space surrounding the class names is trimmed, the classes
   * instantiated and then stored under the lifecycle stage(s) they should intercept.
   *
   * @return a Map of {@link LifecycleStage} to Collection of {@link Interceptor}
   */
  ...
  protected Map<LifecycleStage, Collection<Interceptor>> initInterceptors() {
    return initInterceptors(
        getBootstrapPropertyResolver().getClassPropertyList(INTERCEPTOR_LIST, Interceptor.class));
  }
  ...
  protected <T extends ConfigurableComponent> T initializeComponent(
    ...
    if (clazz != null) {
      try {
        ...
        throw new StripesRuntimeException(
            "Could not instantiate configured "
  ...
  public void init() {
    ...
    for (Class<? extends Formatter> formatter : formatters) {
      ...
      if ((typeArguments != null)
        ...
        if (typeArguments[0] instanceof Class) {
          log.debug(
              "Adding auto-discovered Formatter [",
              formatter,
              "] for [",
              typeArguments[0],
              "] (from type parameter)");
          getFormatterFactory()
              .add((Class<?>) typeArguments[0], (Class<? extends Formatter<?>>) formatter);
        } else {
    ...
    for (Class<? extends TypeConverter> typeConverter : typeConverters) {
      ...
      if ((typeArguments != null)
        ...
        if (typeArguments[0] instanceof Class) {
          log.debug(
              "Adding auto-discovered TypeConverter [",
              typeConverter,
              "] for [",
              typeArguments[0],
              "] (from type parameter)");
          getTypeConverterFactory()
              .add((Class<?>) typeArguments[0], (Class<? extends TypeConverter<?>>) typeConverter);
        } else {
          log.warn(
              "Type parameter for non-abstract TypeConverter [",
              typeConverter,
              "] is not a class.");
        }
      }
