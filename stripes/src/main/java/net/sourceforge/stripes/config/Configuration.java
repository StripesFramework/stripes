 ...
 * Configuration implementations are handed a reference to the BootstrapPropertyResolver to enable
 * them to find initial values and fully initialize themselves. Through the
 ...
 *
 * <p>Implementations of Configuration should fail fast. At initialization time they should detect
 * as many failures as possible and raise an exception. Since exceptions in Configuration are
 * considered fatal there are no exception specifications and implementations are expected to throw
 * runtime exceptions with plenty of details about the failure and its suspected cause(s).
 *
 * @author Tim Fennell
 */
public interface Configuration {
  /**
   * Supplies the Configuration with a BootstrapPropertyResolver. This method is guaranteed to be
   * invoked prior to the init method.
   *
   * @param resolver a BootStrapPropertyResolver which can be used to find any values required by
   *     the Configuration in order to initialize
   */
  void setBootstrapPropertyResolver(BootstrapPropertyResolver resolver);

  /**
   * Called by the DispatcherServlet to initialize the Configuration. Any operations which may fail
   * and cause the Configuration to be inaccessible should be performed here (e.g. opening a
   * configuration file and reading the contents).
   */
  void init();
   ...
   * ActionBeans. The instance should be cached by the Configuration since multiple entities in the
   * system may access the ActionResolver throughout the lifetime of the application.
   *
  ...
  /**
   * Returns an instance of ActionBeanPropertyBinder that is responsible for binding all properties
   * to all ActionBeans at runtime. The instance should be cached by the Configuration since
   * multiple entities in the system may access the ActionBeanPropertyBinder throughout the lifetime
   * of the application.
   *
  ...
  /**
   * Returns an instance of LocalizationBundleFactory that is responsible for looking up resource
   * bundles for the varying localization needs of a web application. The instance should be cached
   * by the Configuration since multiple entities in the system may access the
  ...
  /**
   * Returns an instance of LocalePicker that is responsible for choosing the Locale for each
   * request that enters the system.
   *
  ...
  /**
   * Returns an instance of FormatterFactory that is responsible for creating Formatter objects for
   * converting rich types into Strings for display on pages.
   *
   * @return LocalePicker an instance of a LocalePicker implementation
   */
  ...
  /**
   * Returns an instance of a tag error renderer factory for building custom error renderers for
   * form input tags that have field errors.
   *
  ...
  /**
   * Returns an instance of a PopulationStrategy that determines from where a tag's value should be
   * repopulated.
   *
  ...
  /**
   * Returns an instance of an action bean context factory which will used throughout Stripes to
   * manufacture ActionBeanContext objects. This allows projects to extend ActionBeanContext and
   * provide additional type safe methods for accessing contextual information cleanly.
   *
  ...
  /**
   * Fetches the interceptors that should be executed around the lifecycle stage applied. Must
   * return a non-null collection, but the collection may be empty. The Interceptors are invoked
  ...
  /**
   * Returns an instance of ExceptionHandler that can be used by Stripes to handle any exceptions
   * that arise as the result of processing a request.
   *
   ...
   * MultipartWrapper instances for dealing with multipart requests (those containing file uploads).
   *
   ...
   * determine what validations need to be applied during {@link
