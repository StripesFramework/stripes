...
/**
 * Represents a Node in an expression which has been evaluated against a specific bean in order to
 * determine additional type information etc. NodeEvaluation is to {@link Node}, what {@link
 * PropertyExpressionEvaluation} is to {@link PropertyExpression}.
 *
 * @author Tim Fennell
 * @since Stripes 1.4
 */
public class NodeEvaluation {
  ...
  /**
   * Gets the class object which represents the key type determined during evaluation of the
   * expression against the provided bean. Only usually relevant for Map entries, when it stores the
   * Class specified by the generic signature of the Map.
   *
