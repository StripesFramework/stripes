 ...
 * information stored in the node evaluation to determine the correct type for
 * the Map key and converts the key using Stripes' type conversion system.
 *
 * @author Tim Fennell
 * @since Stripes 1.4
 */
public class MapPropertyAccessor implements PropertyAccessor<Map<?, ?>> {
    ...
    /**
     * Gets the value stored in the Map under the key specified by the current
     * node.
     *
    ...
    /**
     * Attempts to convert the key to from the expression node to the correct
     * type as determined by reflection (using generics to find the Map key
     * type). If generics information is not present then the type found in the
     * expression is used (String, int etc.).
     *
     * @param evaluation the current node evaluation
