...
public class PropertyExpression {
  ...
  private static final Pattern REGEX_LONG =
      Pattern.compile("^(-?\\d+)L$", Pattern.CASE_INSENSITIVE);
  private static final Pattern REGEX_DOUBLE = Pattern.compile("^-?\\d+\\.\\d+$");
  private static final Pattern REGEX_FLOAT =
      Pattern.compile("^(-?\\d+\\.?\\d+)F$", Pattern.CASE_INSENSITIVE);
  private static final Pattern REGEX_BOOLEAN =
  ...
  /**
   * Fetches the root or first node in this expression. In an expression like 'foo.bar' this would
   * return the node that contains 'foo'.
   *
  ...
  /**
   * Fetches the original 'source' of the expression - the String value that was parsed to create
   * the PropertyExpression object.
   *
  ...
  /**
   * Performs the internal parsing of the expression and stores the results in a chain of nodes
   * internally. Passes through the String a character at a time looking for transitions between
   * nodes and invalid states.
   *
    ...
    for (int i = 0; i < chars.length; ++i) {
      ...
      // Deal with single quotes
        ...
        if (i != chars.length - 1 && TERMINATOR_CHARS.indexOf(chars[i + 1]) == -1) {
          throw new ParseException(
              "A quoted String must be terminated by a matching "
      ...
      if (i == chars.length - 1) {
        if (inSingleQuotedString) {
          throw new ParseException(
              expression, "Expression appears to terminate inside of single quoted string.");
        } else if (inDoubleQuotedString) {
          throw new ParseException(
              expression, "Expression appears to terminate inside of double quoted string.");
        } else if (inSquareBrackets) {
          throw new ParseException(
              expression,
   ...
   * @param typedValue a strongly typed value for the nodeValue if one is indicated by the
   *     expression String, otherwise null to automatically determine
