...
/**
 * Implementation of {@link PropertyAccessor} for reading JavaBean properties from JavaBeans. Will
 * attempt property access first using the standard PropertyDescriptor, but if no PropertyDescriptor
 * is present then falls back to attempting Field access for public fields.
 *
 * @author Tim Fennell
 * @since Stripes 1.4
 */
public class JavaBeanPropertyAccessor implements PropertyAccessor<Object> {
  ...
  public Object getValue(NodeEvaluation evaluation, Object bean)
    ...
    try {
      if (pd != null) {
        ...
        if (m != null) {
          ...
          throw new EvaluationException(
              "Could not read write-only property '"
                  + property
                  + "' on bean of type "
      ...
      throw new EvaluationException(
          "Could not read value of property '"
              + property
              + "' on bean of type "
              + bean.getClass().getName()
              + " due to an exception.",
    ...
    try {
      if (pd != null) {
        ...
        if (m != null) {
          ...
          throw new EvaluationException(
              "Could not write read-only property '"
                  + property
                  + "' on bean of type "
                  + bean.getClass().getName());
        }
      ...
      throw new EvaluationException(
          "Could not write value of property '"
              + property
              + "' on bean of type "
              + bean.getClass().getName()
              + " due to an exception.",
