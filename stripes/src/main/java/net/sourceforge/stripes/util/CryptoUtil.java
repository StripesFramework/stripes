...
public class CryptoUtil {
   ...
   * transmitted and stored as a String. Can be decrypted by a subsequent call to {@link
   * #decrypt(String)}. Because, null and "" are equivalent to the Stripes binding engine, if {@code
   * input} is null, then it will be encrypted as if it were "".
   *
  ...
  public static String encrypt(String input) {
    ...
    if (configuration != null && configuration.isDebugMode()) return input;
    ...
    try {
      ...
      hmac(
          ...
          output,
          0,
          output.length - CIPHER_HMAC_LENGTH,
          output,
          output.length - CIPHER_HMAC_LENGTH);
  ...
  /**
   * Performs keyed authentication using HMAC. Note: When building ciphertext+hmac array, data and
   * mac will be the same array, and dataLength == macPos.
   *
  ...
  private static void hmac(
      SecretKey key, byte[] data, int dataPos, int dataLength, byte[] mac, int macPos)
      throws Exception {
  ...
  /**
   * Takes in a base64 encoded and encrypted String that was generated by a call to {@link
   * #encrypt(String)} and decrypts it. If {@code input} is null, then null will be returned.
   *
  ...
  public static String decrypt(String input) {
    ...
    try {
      output =
          cipher.doFinal(
              bytes, CIPHER_BLOCK_LENGTH, bytes.length - CIPHER_HMAC_LENGTH - CIPHER_BLOCK_LENGTH);
   ...
   */
  private static boolean hmacEquals(
      SecretKey key, byte[] mac1, int mac1pos, byte[] mac2, int mac2pos) throws Exception {
    ...
    for (int i = 0; i < CIPHER_HMAC_LENGTH; i++)
      if (mac1[mac1pos + i] != mac2[mac2pos + i]) return false;
    return true;
  }
      ...
      if (CryptoUtil.secretKey == null) {
        ...
        // Hash the key string given in config
          ...
          for (int i = 0; i < hashed.length; i += length) {
            ...
            System.arraycopy(material, 0, hashed, i, Math.min(length, MIN_KEY_BYTES - i));
          }
          material = hashed;
        }
   ...
   * Configuration. If config is unavailable or there is no material configured null will be
   * returned.
   *
   ...
   * In general the use of the config property (Stripes.EncryptionKey) should be preferred, but if
   * specific encryption methods are required, this method allows the caller to set a SecretKey
