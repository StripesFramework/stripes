...
/**
 * <p>
 * Cryptographic utility that can encrypt and decrypt Strings using a key stored
 * in HttpSession. Strings are encrypted by default using a 168bit DEDede
 * (triple DES) key and then Base 64 encoded in a way that is compatible with
 * being inserte into web pages.</p>
 *
 * <p>
 * A single encryption key is used to encrypt values for all sessions in the web
 * application. The key can come from multiple sources. Without any
 * configuration the key will be generated using a SecureRandom the first time
 * it is needed. <b>Note: this will result in encrypted values that are not
 * decryptable across application restarts or across nodes in a cluster.</b>
 * Alternatively specific key material can be specified using the configuration
 * parameter <code>Stripes.EncryptionKey</code> in web.xml. This key is text
 * that is used to generate a secret key, and ideally should be quite long (at
 * least 20 characters). If a key is configured this way the same key will be
 * used across all nodes in a cluster and across restarts.</p>
 *
 * <p>
 * Finally a key can be specified by calling
 * {@link #setSecretKey(javax.crypto.SecretKey)} and providing your own
 * {@link SecretKey} instance. This method allows the specification of any key
 * from any source. In addition the provided key can be for any algorithm
 ...
 * in STS-934 to perform the Encrypt-then-Mac pattern. Also the encryption mode
 * was changed from ECB to CBC.</p>
 * 
 * For more information, refer to https://en.wikipedia.org/wiki/Authenticated_encryption
 * @author Tim Fennell
 * @since Stripes 1.2
 */
public class CryptoUtil {
     ...
     * byte[] so that it can be transmitted and stored as a String. Can be
     * decrypted by a subsequent call to {@link #decrypt(String)}. Because, null
     * and "" are equivalent to the Stripes binding engine, if {@code input} is
     * null, then it will be encrypted as if it were "".
     *
     ...
     * which is intended to prevent analyzing a cipher mode as an xor
     * substitution cipher.
     *
    ...
    /**
     * Performs keyed authentication using HMAC. Note: When building
     * ciphertext+hmac array, data and mac will be the same array, and
     * dataLength == macPos.
     *
     ...
     * @return the length of ciphertext, calculated based on blockcipher block
     * size
     */
     ...
     * call to {@link #encrypt(String)} and decrypts it. If {@code input} is
    ...
    /**
     * Compares HMAC in a manner secured against timing attacks, as per NCC
     * Group "Double Hmac Verification" recepie. Destructive compare, the hmac's
     * will be replaced with the hmac's of themselves as a side effect.
     *
     ...
     * will be generated the first time it is requested. Will look for source
     * material for the key in config and use that if found. Otherwise will
     * generate key material using a SecureRandom and then manufacture the key.
     * Once the key is created it is cached locally and the same key instance
     * will be returned until the application is shutdown or restarted.
     *
    ...
    /**
     * Attempts to load material from which to manufacture a secret key from the
     * Stripes Configuration. If config is unavailable or there is no material
    ...
    /**
     * Sets the secret key that will be used by the CryptoUtil to perform
     * encryption and decryption. In general the use of the config property
     * (Stripes.EncryptionKey) should be preferred, but if specific encryption
     * methods are required, this method allows the caller to set a SecretKey
     * suitable to any symmetric encryption algorithm available in the JVM.
     *
     * @param key the secret key to be used to encrypt and decrypt values going
     * forward
     */
