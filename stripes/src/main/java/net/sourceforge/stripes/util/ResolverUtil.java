 ...
 * specific annotation.
 *
 * <p>The standard usage pattern for the ResolverUtil class is as follows:
 *
 * <pre>
...
public class ResolverUtil<T> {
  ...
  /**
   * A Test that checks to see if each class is annotated with a specific annotation. If it is, then
  ...
  /**
   * Sets an explicit ClassLoader that should be used when scanning for classes. If none is set then
   * the context classloader will be used.
   *
   ...
   * interface is provided this method will collect implementations. In the case of a non-interface
   * class, subclasses will be collected. Accumulated classes can be accessed by calling {@link
   * #getClasses()}.
   *
  ...
  /**
   * Attempts to discover classes that are annotated with the annotation. Accumulated classes can be
  ...
  /**
   * Scans for classes starting at the package provided and descending into subpackages. Each class
   * is offered up to the Test as it is discovered, and if the Test returns true the class is
   * retained. Accumulated classes can be fetched by calling {@link #getClasses()}.
   *
  ...
  /**
   * Converts a Java package name to a path that can be looked up with a call to {@link
