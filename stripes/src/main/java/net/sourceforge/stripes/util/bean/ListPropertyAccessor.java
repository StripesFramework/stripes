...
/**
 * Implementation of {@link PropertyAccessor} for interacting with Lists. Automatically expands the
 * list to make the supplied list index valid for set operations, and suppresses
...
public class ListPropertyAccessor implements PropertyAccessor<List<?>> {
  /**
   * Fetches the value stored at the index specified by the current node. If the index is out of
   * bounds, will return null.
   *
  ...
  /**
   * Sets the value at the index specified by the current node to the supplied value. If the list is
   * not long enough it is expanded (filling with nulls) until it is large enough to accommodate the
   * supplied index.
   *
   * @param evaluation the current node evaluation
   * @param list the target list
  ...
  /**
   * Fetches the key and casts/unboxes it to an int. If the key is not an int, it will throw an
