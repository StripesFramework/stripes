...
public class CollectionUtil {
  /**
   * Checks to see if an array contains an item. Works on unsorted arrays. If the array is null this
   * method will always return false. If the item is null, will return true if the array contains a
   * null entry, false otherwise. In all other cases, item.equals() is used to determine equality.
   *
   ...
   *
   * <p>Calling this method with a null or zero length array will always return true.
   *
   ...
   * @return an Object[], either the array passed in, or in the case of primitives, a new Object[]
   *     containing a wrapper for each element in the input array
  ...
  /**
   * Converts an Object reference that is known to be an array into a List. Semantically very
   * similar to {@link java.util.Arrays#asList(Object[])} except that this method can deal with
   * arrays of primitives in the same manner as arrays as objects.
   *
   * <p>A new List is created of the same size as the array, and elements are copied from the array
   * into the List. If elements are primitives then they are converted to the appropriate wrapper
   * types in order to return a List.
   *
