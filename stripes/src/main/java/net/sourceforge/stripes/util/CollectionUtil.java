...
public class CollectionUtil {
    ...
    /**
     * Checks to see if an array contains an item. Works on unsorted arrays. If
     * the array is null this method will always return false. If the item is
     * null, will return true if the array contains a null entry, false
     * otherwise. In all other cases, item.equals() is used to determine
     * equality.
     *
     ...
     * empty-string values. If it does, returns false. Returns true for null
     * arrays and zero length arrays, as well as for arrays consisting only of
     ...
     * The array is usually derived from the <code>on</code> attribute of one of the
     * Stripes annotations (e.g.
     * {@link net.sourceforge.stripes.validation.ValidationMethod}). The array
     * can be composed of <i>positive</i> event names (e.g. {"foo", "bar"}) in
     * which case the event must be contained in the array, or negative event
     * names (e.g. {"!splat", "!whee"}) in which case the event must not be
     * contained in the array.</p>
     *
     * <p>
     * Calling this method with a null or zero length array will always return
     * true.</p>
     *
     ...
     * primitives, a new Object[] containing a wrapper for each element in the
     * input array
     ...
     * are converted to the appropriate wrapper types in order to return a
     * List.</p>
     *
