 ...
 * of mappings: direct and indirect. A direct mapping is one that is explicitly
 * created by a call to {@link #add(Class, Object)}. An indirect mapping is one
 * that is discovered by examining a target type's implemented interfaces and
 * superclasses. If {@code searchHierarchy} is set to false, then only direct
 * mappings will be considered and the class hierarchy will not be searched.
 * </p>
 * <p>
 * For example, let's assume a direct mapping is created for type {@code A} to
 * handler {@code H}. A request for a handler for type {@code A} returns
 * {@code H} due to the direct mapping. If {@code
 * searchHierarchy} is true and a handler is requested later for type {@code B},
 * which implements {@code A}, then an indirect mapping will be created that
 * maps the handler {@code H} to type {@code B}. (If {@code A} were a superclass
 * of {@code B}, it would behave likewise.) However, if {@code searchHierarchy}
 * is false then a request for a handler for type {@code B} would return
 * {@link #getDefaultHandler()}.
 * </p>
 * <p>
 * This class is used within Stripes to map {@link Formatter}s,
 * {@link TypeConverter}s and {@link ObjectPostProcessor}s to specific classes
 * and interfaces.
 * </p>
 *
 * @author Ben Gunter
...
public class TypeHandlerCache<T> {
    ...
    /**
     * Get the default handler to return if no handler is found for a requested
     * target type.
     * @return 
     */
    ...
    /**
     * Set the default handler to return if no handler is found for a requested
     * target type.
    ...
    /**
     * Set the flag that enables or disables searching of the class hierarchy to
     * find the best available handler in case a direct mapping is not available
     * for a given target type.
     *
     * @param searchHierarchy True to enable hierarchy search; false to disable
     * it.
     */
     ...
     * handler registered for the annotation class. Defaults to true.
     * @return 
     */
    ...
    /**
     * Set the flag that enables or disables searching for handlers for a target
     * type's annotations.
     * @param searchAnnotations
     */
    ...
    /**
     * Gets the (rather confusing) map of handlers. The map uses the target type
     * as the key in the map, and the handler as the value.
     *
    ...
    /**
     * Adds a handler to the set of registered handlers, overriding an existing
     * handler if one was already registered for the target type. Calls
     * {@link #clearCache()} because a new direct mapping can affect the
     ...
     * returned from a call to {@link #getDefaultHandler()}.
     */
     ...
     * checking the specified class, then all the interfaces it implements, then
     * all its superclasses and the interfaces they implement, and finally all
     * the superclasses of the interfaces implemented by {@code targetClass}.
     *
     * @param targetType The type for which a handler is requested.
    ...
    /**
     * Called first by {@link #findHandler(Class)}. Search for a handler class
     * that best matches the requested class, first checking the specified
     * class, second all the interfaces it implements, third annotations. If no
     * match is found, repeat the process for each superclass.
     *
     * @param targetType The type for which a handler is requested.
    ...
    /**
     * Called second by {@link #findHandler(Class)}, after
     * {@link #findInSuperclasses(Class)} . Search for a handler that best
     * matches the requested class by checking the superclasses of every
     * interface implemented by {@code targetClass}.
     *
     * @param targetType The type for which a handler is requested.
