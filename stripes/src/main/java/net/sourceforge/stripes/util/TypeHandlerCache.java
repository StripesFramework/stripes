 ...
 * and indirect. A direct mapping is one that is explicitly created by a call to {@link #add(Class,
 * Object)}. An indirect mapping is one that is discovered by examining a target type's implemented
 * interfaces and superclasses. If {@code searchHierarchy} is set to false, then only direct
 * mappings will be considered and the class hierarchy will not be searched.
 *
 * <p>For example, let's assume a direct mapping is created for type {@code A} to handler {@code H}.
 * A request for a handler for type {@code A} returns {@code H} due to the direct mapping. If {@code
 ...
 *
 * <p>This class is used within Stripes to map {@link Formatter}s, {@link TypeConverter}s and {@link
 * ObjectPostProcessor}s to specific classes and interfaces.
 *
 * @author Ben Gunter
 */
public class TypeHandlerCache<T> {
  ...
  /**
   * Indicates if the class hierarchy will be searched to find the best available handler in case a
   * direct mapping is not available for a given target type. Defaults to true.
   */
   ...
   *
   * @param targetType The type for which a handler is requested.
   * @return An appropriate handler, if one is found. Otherwise, whatever is returned from a call to
   *     {@link #getDefaultHandler()}.
   */
  ...
  /**
   * Search for a handler class that best matches the requested class, first checking the specified
   * class, then all the interfaces it implements, then all its superclasses and the interfaces they
   * implement, and finally all the superclasses of the interfaces implemented by {@code
   * targetClass}.
   *
   * @param targetType The type for which a handler is requested.
  ...
  protected T findHandler(Class<?> targetType) {
    ...
    if (handler == null) {
      if (isSearchHierarchy()) handler = findInInterfaces(targetType, targetType.getInterfaces());
      else handler = cacheHandler(targetType, null);
    }
  ...
  protected T findInSuperclasses(Class<?> targetType) {
    ...
    if ((handler = handlers.get(targetType)) != null) {
      ...
      if (handler != null) return cacheHandler(targetType, handler);
    }

    // Check directly implemented interfaces
    if (isSearchHierarchy()) {
      for (Class<?> iface : targetType.getInterfaces()) {
        if ((handler = handlers.get(iface)) != null) return cacheHandler(targetType, handler);
  ...
  /**
   * Called second by {@link #findHandler(Class)}, after {@link #findInSuperclasses(Class)} . Search
   * for a handler that best matches the requested class by checking the superclasses of every
