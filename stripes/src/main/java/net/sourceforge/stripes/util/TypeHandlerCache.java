...
public class TypeHandlerCache<T> {
    ...
    */
   private final Map<Class<?>, T> _indirectCache = new ConcurrentHashMap<>();

   /**
    * Cache of classes that have been searched, yet no handler (besides the default one) could be
    * found for them.
    */
   private final Set<Class<?>> _negativeCache = new ConcurrentHashSet<>();
   ...
   public void add( Class<?> targetType, T handler ) {
      _handlers.put(targetType, handler);
      clearCache();
   }
   ...
   protected T findInInterfaces( Class<?> targetType, Class<?>... ifaces ) {
      ...
      for ( Class<?> iface : ifaces ) {
         if ( (handler = _handlers.get(iface)) != null ) {
            return cacheHandler(targetType, handler);
         } else if ( (handler = _indirectCache.get(iface)) != null ) {
            return cacheHandler(targetType, handler);
   ...
   protected T findInSuperclasses( Class<?> targetType ) {
      ...
      T handler;
      if ( (handler = _handlers.get(targetType)) != null ) {
         return handler;
      } else if ( (handler = _indirectCache.get(targetType)) != null ) {
         return handler;
      } else if ( _negativeCache.contains(targetType) ) {
         return null;
      ...
      if ( isSearchAnnotations() ) {
         for ( Annotation annotation : targetType.getAnnotations() ) {
            ...
            if ( _handlers.containsKey(annotationType) ) {
               return cacheHandler(targetType, _handlers.get(annotationType));
            }
         }
      }
