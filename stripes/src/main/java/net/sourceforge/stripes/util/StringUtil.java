...
public class StringUtil {
  /**
   * A regular expression for splitting apart a String where individual parts are separated by any
   * whitespace (including new lines) and/or a comma.
   */
   ...
   */
  private static final Pattern URI_FRAGMENT_DISALLOWED_CHARACTERS =
      Pattern.compile("[^\\p{Alnum}._~!$&'()*+,;=:@/?-]");

  /**
   * Splits apart the input String on any whitespace and/or commas. Leading and trailing whitespace
   * are ignored. If a null String is provided as input a zero length array will be returned.
   *
  ...
  /**
   * URL-encodes {@code value} using the UTF-8 charset. Using this method eliminates the need for a
   * try/catch since UTF-8 is guaranteed to exist.
   *
  ...
  /**
   * URL-decodes {@code value} using the UTF-8 charset. Using this method eliminates the need for a
   * try/catch since UTF-8 is guaranteed to exist.
   *
   ...
   * set of characters than other parts of the URI, and characters that are allowed in the fragment
   * must not be encoded.
   *
  ...
  public static String uriFragmentEncode(String value) {
    ...
    if (buf == null) return value;

    // Append tail
    if (end < value.length()) buf.append(value.substring(end));

    return buf.toString();
  }

  /**
   * Return the specified URL or path string with the fragment part removed. If the string does not
   * contain a fragment then return the string unchanged.
   *
