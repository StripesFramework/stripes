 ...
 * found, otherwise the toString() method will be invoked. The presence of a "selected" attribute is
 ...
 * selected attribute (or lack thereof) is used to turn selection on or off.
 *
 * <p>If the option has a body then the String value of that body will be used to generate the body
 * of the generated HTML option. If the body is empty or not present then the label attribute will
 * be written into the body of the tag.
 *
 * @author Tim Fennell
 */
public class InputOptionTag extends InputTagSupport implements BodyTag {
  ...
  /**
   * Locates the option's parent select tag, determines selection state and then writes out an
   * option tag with an appropriate body.
   *
   * @return EVAL_PAGE in all cases.
   * @throws JspException if the option is not contained inside an InputSelectTag or output cannot
   *     be written.
   */
  @Override
  ...
  /**
   * Overridden to make sure that options do not try and register themselves with the form tag. This
   * is done because options are not standalone input tags, but always part of a select tag (which
   * gets registered).
   */
  @Override
