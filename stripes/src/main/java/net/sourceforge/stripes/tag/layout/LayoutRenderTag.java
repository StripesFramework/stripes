 ...
 * attributes provided to the class other than 'name' will be placed into page context during the
 * evaluation of the layout, making them available to other tags, and in EL.
 *
...
public class LayoutRenderTag extends LayoutTag implements BodyTag, DynamicAttributes {
  ...
  /**
   * Initialize fields before execution begins. Typically, this would be done by overriding {@link
   * #setPageContext(jakarta.servlet.jsp.PageContext)}, but that isn't possible in this case because
   * some of the logic depends on {@link #setName(String)} having been called, which does not happen
   * until after {@link #setPageContext(jakarta.servlet.jsp.PageContext)} has been called.
   */
  protected void initialize() {
    LayoutContext context = LayoutContext.lookup(pageContext);

    boolean create =
        context == null || !context.isComponentRenderPhase() || isChildOfCurrentComponent();
   ...
   *       of the tag.
   *   <li>Create a new context and places it in request scope.
   *   <li>Include the layout definition page named by the {@code name} attribute.
   * </ul>
   *
  ...
  public int doStartTag() throws JspException {
    try {
      ...
      if (context.isComponentRenderPhase()) {
        log.debug(
            "Start component render phase for ",
  ...
  /**
   * Set the tag's body content. Called by the JSP engine during component registration phase, when
   * {@link #doStartTag()} returns {@link BodyTag#EVAL_BODY_BUFFERED}
   */
   ...
   *
   * <ul>
   *   <li>Ensure the layout rendered successfully by checking {@link LayoutContext#isRendered()}.
   *   <li>Remove the current layout context from request scope.
   *   <li>Restore previous page context attribute values.
   * </ul>
   *
  ...
  public int doEndTag() throws JspException {
    try {
        ...
        try {
          ...
          throw new StripesJspException(
              "An exception was raised while invoking a layout. The layout used was "
                  + "'"
                  + getName()
                  + "'. The following information was supplied to the render "
                  + "tag: "
                  + context.toString(),
              e);
        }
        ...
        if (!context.isRendered()) {
          throw new StripesJspException(
              "Attempt made to render a layout that does not exist. The layout name "
                  + "provided was '"
                  + getName()
                  + "'. Please check that a JSP/view exists at "
      ...
      if (context.isComponentRenderPhase()) {
        log.debug(
            "End component render phase for ",
            context.getComponent(),
            " in ",
            context.getRenderPage());
