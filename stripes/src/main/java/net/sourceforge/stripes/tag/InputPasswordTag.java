...
/**
 * Tag class that implements an input tag of type password. Defines one
 * attribute in addition to those provided by the HTML tag. If
 * {@code repopulate} is set then the password tag will behave just like the
 * text tag, and will repopulate values on error and during wizard flows. If
 * {@code repopulate} is not set, or is set to false then values will not be
 * re-populated, but initial/default values supplied to the tag on the JSP will
 * be used.
 *
 * @author Tim Fennell
 * @since Stripes 1.1
...
public class InputPasswordTag extends InputTextTag {
    ...
    /**
     * Constructs a new tag for generating password fields. Delegates to
     * InputTextTag which it extends, and then overrides the type of input field
     * to "password".
     */
