...
/**
 * Writes a set of {@literal <option value="foo">bar</option>} tags to the page based on the
 * contents of a Collection. Each element in the collection is represented by a single option tag on
 * the page. Uses the label and value attributes on the tag to name the properties of the objects in
 * the Collection that should be used to generate the body of the HTML option tag and the value
 * attribute of the HTML option tag respectively.
 *
 * <p>E.g. a tag declaration that looks like:
 *
 * <pre>{@literal <stripes:options-collection collection="${cats} value="catId" label="name"/>}
 * </pre>
 *
 * <p>would cause the container to look for a Collection called "cats" across the various JSP scopes
 * and set it on the tag. The tag would then proceed to iterate through that collection calling
 ...
 *
 * <p>To do label localization, the tag will look up labels in the field resource bundle using:
 *
 * <ul>
 *   <li>{className}.{labelPropertyValue}
 *   <li>{packageName}.{className}.{labelPropertyValue}
 *   <li>{className}.{valuePropertyValue}
 *   <li>{packageName}.{className}.{valuePropertyValue}
 * </ul>
 *
 * <p>For example for a class com.myco.Gender supplied to the options-collection tag with
 * label="description" and value="key", when rendering for an instance Gender[key="M",
 * description="Male"] the following localized properties will be looked for:
 *
 * <ul>
 *   <li>Gender.Male
 *   <li>com.myco.Gender.Male
 *   <li>Gender.M
 *   <li>com.myco.Gender.M
 * </ul>
 *
 * <p>If no localized label can be found, or if the localizeLabels attribute is set to false, then
 * the value of the label property will be used.
 *
 * <p>Optionally, the group attribute may be used to generate &lt;optgroup&gt; tags. The value of
 * this attribute is used to retrieve the corresponding property on each object of the collection. A
 * new optgroup will be created each time the value changes.
 *
 * <p>The rendered group may be localized by specifying one of the following properties:
 *
 * <ul>
 *   <li>{className}.{groupPropertyValue}
 *   <li>{packageName}.{className}.{groupPropertyValue}
 * </ul>
 *
 * <p>All other attributes on the tag (other than collection, value, label and group) are passed
 * directly through to the InputOptionTag which is used to generate the individual HTML options
 * tags. As a result the InputOptionsCollectionTag will exhibit the same re-population/selection
 * behaviour as the regular options tag.
 *
 * <p>Since the tag has no use for one it does not allow a body.
 *
 * @author Tim Fennell
 */
public class InputOptionsCollectionTag extends HtmlTagSupport {
  ...
  /**
   * A little container class that holds an entry in the collection of items being used to generate
   * the options, along with the determined label and value (either from a property, or a localized
   * value).
   */
   ...
   *
   * <p>In the case of any input which is not an {@link java.util.Collection} it is converted to a
   * Collection before storing it.
   *
  ...
  /**
   * Returns the value set by {@link #setCollection(Object)}. In the case that a {@link
   * java.util.Collection} was supplied, the same collection will be returned. In all other cases a
   * new collection created to hold the supplied elements will be returned.
   */
  ...
  /**
   * Sets the name of the property that will be fetched on each bean in the collection in order to
   * generate the value attribute of each option.
   *
  ...
  /**
   * Sets the name of the property that will be fetched on each bean in the collection in order to
   * generate the body of each option (i.e. what is seen by the user).
   *
  ...
  /**
   * Sets a comma separated list of properties by which the beans in the collection will be sorted
   * prior to rendering them as options. 'label' and 'value' are special case properties that are
   * used to indicate the generated label and value of the option.
   *
  ...
  /**
   * Iterates through the collection and generates the list of Entry objects that can then be sorted
   * and rendered into options. It is assumed that each element in the collection has non-null
   * values for the properties specified for generating the label and value.
   *
   * @return SKIP_BODY in all cases
   * @throws JspException if either the label or value attributes specify properties that are not
   *     present on the beans in the collection
   */
  @Override
  public int doStartTag() throws JspException {
    if (this.collection == null) return SKIP_BODY;
    ...
    try {
      ...
      for (Object item : this.collection) {
        ...
        Object label =
            (labelProperty == null) ? item : BeanUtil.getPropertyValue(labelProperty, item);
        Object value =
            (valueProperty == null) ? item : BeanUtil.getPropertyValue(valueProperty, item);
        Object group =
            (groupProperty == null) ? null : BeanUtil.getPropertyValue(groupProperty, item);
      ...
      throw new StripesJspException(
          "A problem occurred generating an options-collection. "
              + "Most likely either ["
              + labelProperty
              + "] or ["
              + valueProperty
              + "] is not a "
              + "valid property of the beans in the collection: "
              + this.collection,
          ee);
    }
  ...
  /**
   * Optionally sorts the assembled entries and then renders them into a series of option tags using
   * an instance of InputOptionTag to do the rendering work.
   *
    ...
    for (Entry entry : sortedEntries) {
      ...
      try {
        if (entry.group != null && !entry.group.equals(lastGroup)) {
          if (lastGroup != null) optgroupSupport.writeCloseTag(out, "optgroup");
  ...
  /**
   * Sets the name of the property that will be fetched on each bean in the collection in order to
   * generate optgroups. A new optgroup will be created each time the value changes.
   *
