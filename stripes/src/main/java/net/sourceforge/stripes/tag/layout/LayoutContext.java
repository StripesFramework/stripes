...
/**
 * Used to move contextual information about a layout rendering between a LayoutRenderTag and a
 * LayoutDefinitionTag. Holds the set of overridden components and any parameters provided to the
 * render tag.
 *
...
public class LayoutContext {
   ...
   *
   * @param pageContext The JSP page context to search for the layout context stack.
   * @return The layout context that was popped off the stack, or null if the stack was not found or
   *     was empty.
   */
  ...
  public LayoutContext getFirst() {
    for (LayoutContext c = this; ; c = c.getPrevious()) {
      if (c.getPrevious() == null) return c;
    }
  }
   ...
   *
   * <p>This method calls {@link PageContext#include(String, boolean)} with {@code false} as the
  ...
  public void doInclude(PageContext pageContext, String relativeUrlPath)
    ...
    try {
      ...
      if (isIncludeBroken(pageContext)) doIncludeHack(pageContext, relativeUrlPath);
      else pageContext.include(relativeUrlPath, false);
    } finally {
      pageContext.getRequest().removeAttribute(LAYOUT_CONTEXT_KEY);
    }
  }
  ...
  /**
   * Returns true if the current thread is executing in an application server that is known to have
   * issues with doing normal includes using {@link PageContext#include(String, boolean)}.
   */
  ...
  /**
   * An alternative to {@link PageContext#include(String, boolean)} that works around broken include
   * functionality in certain application servers, including several current and recent releases of
   * WebLogic.
   */
  protected void doIncludeHack(PageContext pageContext, String relativeUrlPath)
    ...
    class MyServletOutputStream extends ServletOutputStream {
      ...
      void setCharset(String charset) {
        ...
        if (!charset.equals(this.charset)) decoder = null;
      ...
      /**
       * Clear the byte buffer. If the byte buffer has any data remaining to be read, then those
       * bytes are shifted to the front of the buffer and the buffer's position is updated
       * accordingly.
       */
      void resetBuffer() {
      ...
      /**
       * Decode the contents of the byte buffer to the character buffer and then write the contents
    ...
    pageContext
        .getRequest()
        .getRequestDispatcher(relativeUrlPath)
  ...
  /**
   * Get the path to the page that contains the {@link LayoutDefinitionTag} referenced by the render
   * tag.
   */
