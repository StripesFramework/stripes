 ...
 * be returned as a String[] in order to support multiple-value parameters.
 *
...
public class DefaultPopulationStrategy implements PopulationStrategy {
  ...
  /**
   * Implementation of the interface method that will follow the search described in the class level
   * JavaDoc and attempt to find a value for this tag.
   *
   * @param tag the form input tag whose value to populate
   * @return Object will be one of null, a single Object or an Array of Objects depending upon what
   *     was submitted in the prior request, and what is declared on the ActionBean
   */
  ...
  /**
   * Helper method that will check the current request for user submitted values for the tag
   * supplied and return them as a String[] if there is one or more present.
   *
  ...
  /**
   * Helper method that will check to see if there is an ActionBean present in the request, and if
   * so, retrieve the value for this tag from the ActionBean.
   *
   * @param tag the tag whose values to look for
  ...
  /**
   * Helper method that will retrieve the preferred value set on the tag in the JSP. For most tags
   * this is usually the body if it is present, or the value attribute. In some cases tags implement
   * this differently, notably the radio and checkbox tags.
   *
  ...
  /**
   * Helper method that will check to see if the form containing this tag is being rendered as a
   * result of validation errors. This is not actually used by the default strategy, but is here to
   * help subclasses provide different behaviour for when the form is rendering normally vs. in
   * error.
   *
   * @param tag the tag that is being repopulated
