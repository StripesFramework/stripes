...
/**
 * A straightforward implementation of the TagErrorRendererFactory interface that looks up the name
 * of the renderer class in config, and if one is not supplied defaults to using the {@link
 * DefaultTagErrorRenderer}. The same TagErrorRenderer is instantiated for all tags, and must be
 * public and have a public no-arg constructor.
 *
 * <p>To configure a different TagErrorRenderer use the configuration key {@code
 * TagErrorRenderer.Class} and supply a fully qualified class name. For example, to do this in
 * web.xml you would add the following parameter to the Stripes Filter:
 *
 * <pre>
 * {@literal <init-param>}
...
public class DefaultTagErrorRendererFactory implements TagErrorRendererFactory {
  ...
  public void init(Configuration configuration) throws Exception {
    ...
    if (this.rendererClass == null) this.rendererClass = DefaultTagErrorRenderer.class;
  }
  ...
  public TagErrorRenderer getTagErrorRenderer(InputTagSupport tag) {
    try {
      TagErrorRenderer renderer =
          getConfiguration().getObjectFactory().newInstance(this.rendererClass);
      renderer.init(tag);
      return renderer;
    } catch (Exception e) {
      throw new StripesRuntimeException(
          "Could not create an instance of the configured "
              + "TagErrorRenderer class '"
              + this.rendererClass.getName()
              + "'. Please check "
