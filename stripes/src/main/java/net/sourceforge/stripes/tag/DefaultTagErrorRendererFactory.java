...
/**
 * <p>
 * A straightforward implementation of the TagErrorRendererFactory interface
 * that looks up the name of the renderer class in config, and if one is not
 * supplied defaults to using the {@link DefaultTagErrorRenderer}. The same
 * TagErrorRenderer is instantiated for all tags, and must be public and have a
 * public no-arg constructor.</p>
 *
 * <p>
 * To configure a different TagErrorRenderer use the configuration key
 * {@code TagErrorRenderer.Class} and supply a fully qualified class name. For
 * example, to do this in web.xml you would add the following parameter to the
 * Stripes Filter:</p>
 *
 * <pre>
 *{@literal <init-param>}
...
public class DefaultTagErrorRendererFactory implements TagErrorRendererFactory {
    ...
    /**
     * Looks up the name of the configured renderer class in the configuration
     ...
     * By default returns an instance of {@link DefaultTagErrorRenderer}. If a
     * custom class is configured and cannot be instantiated, an exception will
     * be thrown.
     */
