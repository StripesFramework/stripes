 ...
 * @author Tim Fennell
 */
public class FormTag extends HtmlTagSupport
    implements BodyTag, TryCatchFinally, ParameterizableTag {
  ...
  /**
   * The {@link ActionBean} class to which the form will submit, as determined by the {@link
   * ActionResolver}. This may be null if the action attribute is set but its value does not resolve
  ...
  public int doStartTag() throws JspException {
    if (this.actionWithoutContext == null) {
      throw new StripesJspException(
          "The form tag attributes 'beanClass' and 'action' "
    ...
    urlBuilder =
        new UrlBuilder(pageContext.getRequest().getLocale(), getAction(), false).setEvent(null);
   ...
   *
   * <ul>
   *   <li>The form open tag
   *   <li>Hidden fields for the form name and source page
   *   <li>The buffered body content
   *   <li>The form close tag
   * </ul>
   *
   ...
   * method is POST and the enctype is correct.
   */
  @Override
  ...
  /**
   * Used to ensure that the form is always removed from the tag stack so that there is never any
   * confusion about tag-parent hierarchies.
   */
   ...
   * present on the page. This is done because the list is used to drive required field validation
   * knowing that in a wizard required fields may be spread across several pages.
   *
   * <p>In both cases the value is encrypted to stop the user maliciously spoofing the value.
   *
  ...
  protected ActionBean getActionBean() {
    ...
    if (bean == null) {
      ...
      if (session != null) bean = (ActionBean) session.getAttribute(binding);
    }
    return bean;
  }

  /**
   * Returns true if the ActionBean this form posts to represents a Wizard action bean and false in
   * all other situations. If the form cannot determine the ActionBean being posted to for any
    ...
    if (bean == null) {
      ...
      if (clazz == null) {
        log.error(
            ...
            this.actionWithoutContext,
            "]. Without an ActionBean class Stripes ",
  ...
  /**
   * Writes out hidden fields for all fields that are present in the request but are not explicitly
   * present in this form. Excludes any fields that have special meaning to Stripes and are not
   * really application data. Uses the stripes:wizard-fields tag to do the grunt work.
   */
  ...
  /**
   * Used by nested tags to notify the form that a field with the specified name has been written to
   * the form.
   *
  ...
  /**
   * Checks to see if the field should receive focus either because it is the named field for
   * receiving focus, because it is the first field in the form (and first field focus was
   * specified), or because it is the first field in error.
   *
  ...
  /**
   * Gets the set of all field names for which fields have been referred within the form up until
   * the point of calling this method. If this is called during doEndTag it will contain all field
   * names, if it is called during the body of the tag it will only contain the input elements which
  ...
  /**
   * Appends a parameter to the "action" attribute of the form tag. For clean URLs the value will be
   * embedded in the URL if possible. Otherwise, it will be added to the query string.
   *
