...
/**
 * Displays a list of non-error messages to the user. The list of messages can come from either the
 * request (preferred) or the session (checked 2nd). Lists of messages can be stored under any
 * arbitrary key in request or session and the key can be specified to the messages tag. If no key
 * is specified then the default key (and therefore default set of messages) is used. Note that by
 * default the ActionBeanContext stores messages in a {@link
 * net.sourceforge.stripes.controller.FlashScope} which causes them to be exposed as request
 * attributes in both the current and subsequent request (assuming a redirect is used).
 *
 * <p>While similar in concept to the ErrorsTag, the MessagesTag is significantly simpler. It deals
 * with a List of Message objects, and does not understand any association between messages and form
 * fields, or even between messages and forms. It is designed to be used to show arbitrary messages
 * to the user, the prime example being a confirmation message displayed on the subsequent page
 * following an action.
 *
 * <p>The messages tag outputs a header before the messages, the messages themselves, and a footer
 * after the messages. Default values are set for each of these four items. Different values can be
 * specified in the error messages resource bundle (StripesResources.properties unless you have
 * configured another). The default configuration would look like this:
 *
 * <ul>
 *   <li>stripes.messages.header={@literal <ul class="messages">}
 *   <li>stripes.messages.footer={@literal </ul>}
 *   <li>stripes.messages.beforeMessage={@literal <li>}
 *   <li>stripes.messages.afterMessage={@literal </li>}
 * </ul>
 *
 * <p>It should also be noted that while the errors tag supports custom headers and footers through
 * the use of nested tags, the messages tag does not support this. In fact the messages tag does not
 * support body content at all - it will simply be ignored.
 *
 * @author Tim Fennell
 * @since Stripes 1.1.2
 */
  ...
  public int doEndTag() throws JspException {
    try {
      ...
      throw new JspException(
          "IOException encountered while writing messages " + "tag to the JspWriter.", e);
    }
  }
