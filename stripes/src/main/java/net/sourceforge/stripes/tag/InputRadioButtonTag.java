 ...
 * only a single selected value at a time the checked attribute of the tag must also be a scalar
 * value.
 *
 * <p>Radio buttons perform automatic (re-)population of state. They prefer, in order, the value in
 * the HttpServletRequest, the value in the ActionBean and lastly the value set using checked="" on
 * the page. If the value of the current radio button matches the checked value from the preferred
 * source then the attribute checked="checked" will be written in the HTML tag.
 *
 * <p>The tag may include a body and if present the body is converted to a String and overrides the
 * <b>checked</b> tag attribute.
 *
 * @author Tim Fennell
 */
public class InputRadioButtonTag extends InputTagSupport implements BodyTag {
  ...
  /**
   * Returns the body of the tag if it is present and not empty, otherwise returns the value of the
   * 'checked' attribute.
   */
  @Override
  ...
  /**
   * Determines the state of the set of radio buttons and then writes the radio button to the output
   * stream with checked="checked" or not as appropriate.
   *
