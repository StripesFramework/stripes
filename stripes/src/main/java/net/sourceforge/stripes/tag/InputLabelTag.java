...
/**
 * Tag handler for a tag that produces an HTML label tag which is capable of looking up localized
 * field names and formatting the label when validation errors exist. The field being labeled is
 * identified using either the {@code name} attribute (preferred) or the {@code for} attribute. If
  ...
  public int doEndInputTag() throws JspException {
    try {
      ...
      throw new StripesJspException(
          "Encountered an exception while trying to write to "
              + "the output from the stripes:label tag handler class, InputLabelTag.",
          ioe);
    }
  }
