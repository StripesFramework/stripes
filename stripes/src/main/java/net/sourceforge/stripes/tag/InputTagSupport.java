...
public abstract class InputTagSupport extends HtmlTagSupport implements TryCatchFinally {
  ...
  /**
   * Gets the value for this tag based on the current population strategy. The value returned could
   * be a scalar value, or it could be an array or collection depending on what the population
   * strategy finds. For example, if the user submitted multiple values for a checkbox, the default
   * population strategy would return a String[] containing all submitted values.
   *
  ...
  /**
   * Locates the enclosing stripes form tag. If no form tag can be found, because the tag was not
   * enclosed in one on the JSP, an exception is thrown.
   *
   ...
   * object may be either a String, String[], Object, Object[] or Collection. Used primarily by the
   * InputCheckBoxTag and InputSelectTag to determine if specific check boxes or options should be
   * selected based on the values contained in the JSP, HttpServletRequest and the ActionBean.
   *
  ...
  /**
   * Fetches the localized name for this field if one exists in the resource bundle. Relies on there
   * being a "name" attribute on the tag, and the pageContext being set on the tag. First checks for
   * a value of {actionBean FQN}.{fieldName} in the specified bundle, then {actionPath}.{fieldName}
   * then just "fieldName".
   *
  ...
  /**
   * Attempts to fetch a "field name" resource from the localization bundle. Delegates to {@link
  ...
  /**
   * Attempts to format an object using the Stripes formatting system. If no formatter can be found,
   * then a simple String.valueOf(input) will be returned. If the value passed in is null, then the
  ...
  protected String format(Object input, boolean forOutput) {
    ...
    if (forOutput && formatted != null) {
      try {
        ...
        if (validate != null && validate.encrypted()) formatted = CryptoUtil.encrypt(formatted);
  ...
  /**
   * Find errors that are related to the form field this input tag represents and place them in an
   * instance variable to use during error rendering.
   */
  ...
  /**
   * Returns true if one or more validation errors exist for the field represented by this input
   * tag.
   */
  ...
  /**
   * Fetches the ActionBean associated with the form if one is present. An ActionBean will not be
   * created (and hence not present) by default. An ActionBean will only be present if the current
   * request got bound to the same ActionBean as the current form uses. E.g. if we are re-showing
   * the page as the result of an error, or the same ActionBean is used for a &quot;pre-Action&quot;
   * and the &quot;post-action&quot;.
   *
  ...
  /**
   * Final implementation of the doStartTag() method that allows the base InputTagSupport class to
   * insert functionality before and after the tag performs it's doStartTag equivalent method. Finds
   * errors related to this field and intercepts with a {@link TagErrorRenderer} if appropriate.
   *
  ...
  public final int doStartTag() throws JspException {
    ...
    if (getFieldErrors() != null) {
      this.errorRenderer =
          StripesFilter.getConfiguration().getTagErrorRendererFactory().getTagErrorRenderer(this);
  ...
  /**
   * Final implementation of the doEndTag() method that allows the base InputTagSupport class to
   * insert functionality before and after the tag performs it's doEndTag equivalent method.
   *
   * @return int the value returned by the child class from doStartInputTag()
   */
  @Override
  ...
  /**
   * Used to ensure that the input tag is always removed from the tag stack so that there is never
   * any confusion about tag-parent hierarchies.
   */
  ...
  /**
   * Informs the tag that it should render JavaScript to ensure that it is focused when the page is
   * loaded. If the tag does not have an 'id' attribute a random one will be created and set so that
   * the tag can be located easily.
   *
  ...
  protected void makeFocused() throws JspException {
    try {
      ...
      out.write(
          "<script type=\"text/javascript\">setTimeout(function(){try{var z=document.getElementById('");
      out.write(getId());
      out.write("');z.focus();");
  ...
  /**
   * Sets the value of the readonly attribute to "readonly" but only when the value passed in is
   * either "readonly" itself, or is converted to true by the {@link
   ...
   * because it is not a simple one-liner and is used by more than one tag.
   */
