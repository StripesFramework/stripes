...
public class InputSelectTag extends InputTagSupport implements BodyTag {
  ...
  /**
   * If the text value passed in matches the empty string or (ignoring case) "multiple", or if the
   * value can be converted to true by the {@link BooleanTypeConverter} then the attribute will be
   * set to "multiple", otherwise the attribute will not be output.
   */
  ...
  /**
   * Stores the value attribute in an instance variable since it is used to determine which options
   * are checked.
   */
  ...
  /**
   * Returns the String value set by the tag attribute on the JSP, or the override value set in the
   * HttpRequest, or the override value bound to the ActionBean object in scope.
   *
   * @return Object one of <em>String</em> if the value comes from the tag on the JSP,
   *     <em>String[]</em> if the value(s) comes from the HttpServletRequest, or the type of the
   *     field on the ActionBean which could be <em>Collection</em>, <em>Object[]</em> or
   *     <em>Object</em>.
   */
  ...
  /**
   * Returns the scalar value or Array or Collection of values that are to be selected in the select
   * tag. This will either be the value returned by the PopulationStrategy, or the value supplied by
   * the container to setValue().
   *
