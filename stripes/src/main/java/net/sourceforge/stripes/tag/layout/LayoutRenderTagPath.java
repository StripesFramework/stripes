 ...
 * page, any number of render tags can be accessible via the same "path," where
 * a path consists of zero or more component tags that are parents of the render
 * tag. This class helps to distinguish between multiple render tags with the
 * same component path by assigning sequential indexes to them.
 *
 * @author Ben Gunter
 * @since Stripes 1.5.7
 */
public class LayoutRenderTagPath {
     ...
     * components that must execute, in order, so that the specified render tag
     * can execute.
     *
    ...
    /**
     * Get the names of the {@link LayoutComponentTag}s that are parent tags of
     * the render tag.
     * @return 
     */
    ...
    /**
     * Get the index (zero-based) of the combined render page and component path
     * within the page.
     * @return 
     */
    public int getIndex() {
     ...
     * current component tag can execute. That is, this tag is a parent of the
     * current component.
     *
    ...
    /**
     * Recursive method called from {@link #isPathComponent(LayoutComponentTag)}
     * that returns true if the specified tag's name is present in the component
     * path iterator at the same position where this tag occurs in the
     * render/component tag tree. For example, if the path iterator contains the
     * component names {@code ["foo", "bar"]} then this method will return true
     * if the tag's name is {@code "bar"} and it is a child of a render tag that
     * is a child of a component tag whose name is {@code "foo"}.
     *
