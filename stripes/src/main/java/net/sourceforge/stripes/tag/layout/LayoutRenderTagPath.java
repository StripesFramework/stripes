...
public class LayoutRenderTagPath {
  ...
  /**
   * Recursive method called from {@link #isPathComponent(LayoutComponentTag)} that returns true if
   * the specified tag's name is present in the component path iterator at the same position where
   * this tag occurs in the render/component tag tree. For example, if the path iterator contains
   * the component names {@code ["foo", "bar"]} then this method will return true if the tag's name
   * is {@code "bar"} and it is a child of a render tag that is a child of a component tag whose
   * name is {@code "foo"}.
   *
