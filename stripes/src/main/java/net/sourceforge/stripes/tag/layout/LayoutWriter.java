 ...
 * the output temporarily. This is required to prevent the non-layout contents
 * of a {@link LayoutDefinitionTag} from rendering more than once when
 * {@link LayoutRenderTag}s and {@link LayoutComponentTag}s are nested within
 * it. The definition tag silences output during a component render phase, and
 * the component that wishes to render turns output back on during its body
 * evaluation.
 *
 * @author Ben Gunter
 * @since Stripes 1.5.4
...
public class LayoutWriter extends Writer {
    ...
    /**
     * If true, then discard all output. If false, then resume sending output to
     * the JSP writer.
     * @return 
     */
     ...
     * context will be flushed before silent mode is enabled to ensure all
     * buffered data are written.
     *
     ...
     * The buffer can be closed and its contents retrieved by calling
     * {@link #closeBuffer(PageContext)}.
     * @param pageContext
     */
     ...
     * writer that was receiving output prior to calling
     * {@link #openBuffer(PageContext)}.
     *
     * @param pageContext
    ...
    /**
     * Try to flush the page context's output buffer. If an exception is thrown,
     * just log it.
     * @param pageContext
     */
