...
/**
 * Provides a tag to override the {@link StripesFilter} configuration property <code>
 * Stripes.HtmlMode</code>.
 *
 * <p><code>htmlMode</code> accepts any string value, however any value not equal to <code>html
 * </code>, case-insensitive, puts Stripes into its default mode of XHTML-compatible output.
 *
 * <p>Examples of the tag's use then might be:
 *
 * <ul>
 *   <li>&lt;s:options htmlMode="html" /&gt; produces HTML4 and HTML5 form elements, e.g., &lt;img
 *       src &#8230; &gt;
 *   <li>&lt;s:options htmlMode="xhtml" /&gt; produces XHTML-compatible form elements, e.g., &lt;img
 *       src &#8230; /&gt;
 *   <li>&lt;s:options htmlMode="default" /&gt; produces XHTML form elements
 * </ul>
 *
 * <p>Typical use of the tag in context of a Stripes application follows:
 *
 * <p>Deployer will set the application RuntimeConfiguration of <code>Stripes.HtmlMode</code>. A
 * deployer choosing not to set this option, defaults the Stripes application to its
 * XHTML-compatible format. <code>Stripes.HtmlMode</code> will set the default X/HTML output for the
 * <strong>entire</strong> application. Individual views of the application wishing to alter the
 * application default will provide this tag, at or near the beginning of the view, or JSP.
 *
...
public class PageOptionsTag extends StripesTagSupport {
  ...
  /**
   * Get the HTML mode for the given page context. If the request attribute {@link
   * #REQ_ATTR_HTML_MODE} is present then use that value. Otherwise, use the global configuration
   * property {@link #CFG_KEY_HTML_MODE}.
   */
  public static String getHtmlMode(PageContext pageContext) {
    String htmlMode =
        (String) pageContext.getAttribute(REQ_ATTR_HTML_MODE, PageContext.REQUEST_SCOPE);
