...
public abstract class LinkTagSupport extends HtmlTagSupport implements ParameterizableTag {
  ...
  /**
   * Gets the URL that is supplied by the user/developer on the page. This is the basis for
   * constructing the eventual URL that the tag generates.
   *
  ...
  /**
   * Sets the URL that is supplied by the user/developer on the page. This is the basis for
   * constructing the eventual URL that the tag generates.
   *
  ...
  /**
   * Used by stripes:param tags (and possibly other tags at some distant point in the future) to add
  ...
  /**
   * Clears all existing parameters. Subclasses should be careful to call this in doEndTag() to
   * ensure that parameter state is cleared between pooled used of the tag.
   */
  ...
  /**
   * Sets the bean class (String FQN or Class) to generate a link for. Provides an alternative to
   * using href for targeting ActionBeans.
   *
  ...
  /**
   * Gets the bean class (String FQN or Class) to generate a link for. Provides an alternative to
   * using href for targeting ActionBeans.
   *
  ...
  /**
   * Sets the anchor element that is appended at the end of the URL. If the provided URL (set using
   * <code>setUrl</code> method) already contains the anchor, then the anchor specified by this
  ...
  /**
   * Returns the base URL that should be used for building the link. This is derived from the
   * 'beanclass' attribute if it is set, else from the 'url' attribute.
   *
   * @return the preferred base URL for the link
   * @throws StripesJspException if a beanclass attribute was specified, but does not identify an
   *     existing ActionBean
   */
    ...
    if (this.beanclass != null) {
      ...
      if (beanHref == null) {
        throw new StripesJspException(
            "The value supplied for the 'beanclass' attribute "
                + "does not represent a valid ActionBean. The value supplied was '"
                + this.beanclass
                + "'. If you're prototyping, or your bean isn't ready yet "
   ...
   * parameters are appended into the URL, along with event name if necessary and the source page
   * information.
   *
  ...
  protected String buildUrl() throws StripesJspException {
    ...
    if (addSourcePage) {
      builder.addParameter(
          StripesConstants.URL_KEY_SOURCE_PAGE, CryptoUtil.encrypt(request.getServletPath()));
    }
    if (this.anchor != null) {
    ...
    if (contextPath.length() > 1) {
      boolean prepend =
          prependContext != null && prependContext
      ...
      if (prepend) {
        if (url.startsWith("/")) url = contextPath + url;
          ...
          log.warn(
              "Use of prependContext=\"true\" is only valid with a URL that starts with \"/\"");
      }
    }
