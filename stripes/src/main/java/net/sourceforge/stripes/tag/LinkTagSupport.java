 ...
 * generates regular {@literal <a href=""/>} style links) and the UrlTag which
 * is a rough similie of the JSTL url tag.
 *
 * @author Tim Fennell
 * @since Stripes 1.4
 */
public abstract class LinkTagSupport extends HtmlTagSupport implements ParameterizableTag {
    ...
    /**
     * Gets the URL that is supplied by the user/developer on the page. This is
     * the basis for constructing the eventual URL that the tag generates.
     *
    ...
    /**
     * Sets the URL that is supplied by the user/developer on the page. This is
     * the basis for constructing the eventual URL that the tag generates.
     *
    ...
    /**
     * Used by stripes:param tags (and possibly other tags at some distant point
     * in the future) to add a parameter to the parent link tag.
     *
    ...
    /**
     * Clears all existing parameters. Subclasses should be careful to call this
     * in doEndTag() to ensure that parameter state is cleared between pooled
     * used of the tag.
     */
     ...
     * Provides an alternative to using href for targeting ActionBeans.
     *
     ...
     * provided URL (set using <code>setUrl</code> method) already contains the
     * anchor, then the anchor specified by this attribute takes precedence.
     *
    ...
    /**
     * Get the flag that indicates if the application context should be included
     * in the URL.
     * @return 
     */
    ...
    /**
     * Set the flag that indicates if the application context should be included
     * in the URL.
     * @param prependContext
     */
     ...
     * derived from the 'beanclass' attribute if it is set, else from the 'url'
     * attribute.
     *
     * @return the preferred base URL for the link
     * @throws StripesJspException if a beanclass attribute was specified, but
     * does not identify an existing ActionBean
     */
     ...
     * Ensures that all parameters are appended into the URL, along with event
     * name if necessary and the source page information.
     *
