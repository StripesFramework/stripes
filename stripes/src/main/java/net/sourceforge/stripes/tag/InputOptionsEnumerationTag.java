 ...
 * based on the values of a enum. Each value in the enum is represented by a
 * single option tag on the page. The options will be generated in ordinal value
 * order (i.e. the order they are declared in the enum).</p>
 *
 * <p>
 * Please note that as with the options-collection and options-map tags,
 * resource bundle lookups are on by default but can be turned off by setting
 * the localizeLabels attribute to false.</p>
 *
 * <p>
 * The label (the value the user sees) is generated in one of three ways: by
 * looking up a localized value (unless localizeLabels is set to false), by
 * using the property named by the 'label' tag attribute if it is supplied and
 * lastly by toString()'ing the enumeration value. For example the following
 * tag:</p>
 *
 * <pre>{@literal <stripes:options-enumeration enum="net.kitty.EyeColor" label="description"/>}</pre>
 *
 * <p>
 * when generating the option for the value {@code EyeColor.BLUE} will look for
 * a label in the following order:</p>
 *
 * <ul>
 * <li>resource: EyeColor.BLUE (skipped if localizeLabels is set to false)</li>
 * <li>resource: net.kitty.EyeColor.BLUE (skipped if localizeLabels is set to
 * false)</li>
 ...
 *
 * <p>
 * If the class specified does not exist, or does not specify a Java 1.5 enum
 * then a JspException will be raised.</p>
 *
 * <p>
 * All attributes of the tag, other than enum and label, are passed directly
 * through to the InputOptionTag which is used to generate the individual HTML
 * options tags. As a result the InputOptionsEnumerationTag will exhibit the
...
public class InputOptionsEnumerationTag extends InputOptionsCollectionTag {
    ...
    /**
     * Attempts to instantiate the Class object representing the enum and fetch
     * the values of the enum. Then generates an option per value using an
     * instance of an InputOptionTag.
     *
     * @return SKIP_BODY in all cases.
     * @throws JspException if the class name supplied is not a valid class, or
     * cannot be cast to an Enum.
     */
    @Override
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            ...
            try {
                ...
                throw new StripesJspException("Could not process class [" + this.className + "]. Attribute 'enum' on "
        ...
        if (!clazz.isEnum()) {
            throw new StripesJspException("The class name supplied, [" + this.className + "], does not appear to be "
                    + "a JDK1.5 enum class.");
        }
