 ...
 *
 * <ul>
 *   <li>resource: EyeColor.BLUE (skipped if localizeLabels is set to false)
 *   <li>resource: net.kitty.EyeColor.BLUE (skipped if localizeLabels is set to false)
 *   <li>property: EyeColor.BLUE.getDescription() (because of the label="description" above)
 *   <li>failsafe: EyeColor.BLUE.toString()
 * </ul>
 *
 * <p>If the class specified does not exist, or does not specify a Java 1.5 enum then a JspException
 * will be raised.
 *
 * <p>All attributes of the tag, other than enum and label, are passed directly through to the
 * InputOptionTag which is used to generate the individual HTML options tags. As a result the
 * InputOptionsEnumerationTag will exhibit the same re-population/selection behaviour as the regular
 * options tag.
 *
  ...
  public int doStartTag() throws JspException {
    ...
    try {
      ...
      for (Enum item : enums) {
        ...
        if (attemptToLocalizeLabels) {
          ...
          label =
              LocalizationUtility.getLocalizedFieldName(
                  simpleName + "." + item.name(), packageName, null, locale);
        }
        if (label == null) {
          if (getLabel() != null) {
        ...
        Object group = null;
        if (getGroup() != null) group = BeanUtil.getPropertyValue(getGroup(), item);

        addEntry(item, label, item, group);
      }
    } catch (ExpressionException ee) {
      throw new StripesJspException(
          "A problem occurred generating an options-enumeration. "
              + "Most likely either the class ["
              + getEnum()
              + "] is not an enum or, ["
              + getLabel()
              + "] is not a valid property of the enum.",
          ee);
    }
