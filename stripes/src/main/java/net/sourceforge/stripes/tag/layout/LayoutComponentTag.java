...
/**
 * Defines a component in a layout. Used both to define the components in a layout definition and to
 * provide overridden component definitions during a layout rendering request.
 *
...
public class LayoutComponentTag extends LayoutTag {
  ...
  private static final Pattern javaIdentifierPattern =
  ...
  public void setPageContext(PageContext pageContext) {
    ...
    if (context == null) {
      throw new StripesRuntimeException(
          "A component tag named \""
              + getName()
              + "\" in "
              + getCurrentPagePath()
              + " was unable to find a layout context.");
    }
  ...
  /**
   * True if this tag is the component to be rendered on this pass from {@link LayoutDefinitionTag}.
   *
  ...
  public boolean isCurrentComponent() throws StripesJspException {
    ...
    if (name == null || !name.equals(getName())) return false;
   ...
   * {@link LayoutDefinitionTag}. If, after evaluating the corresponding tag, the component has not
   * been rendered then evaluate this tag's body by returning {@code EVAL_BODY_INCLUDE}.
   *
   * <p>If this tag is nested within a {@link LayoutRenderTag} and this tag is the current
   * component, as indicated by {@link LayoutContext#getComponent()}, then evaluate this tag's body
   * by returning {@code EVAL_BODY_INCLUDE}.
   *
   * <p>In all other cases, skip this tag's body by returning SKIP_BODY.
   *
  ...
  public int doStartTag() throws JspException {
    try {
      if (context.isComponentRenderPhase()) {
        if (isChildOfRender()) {
          ...
          LayoutComponentRenderer renderer =
              (LayoutComponentRenderer) pageContext.getAttribute(getName());
          if (renderer == null)
        ...
        if (isChildOfRender()) {
          ...
          if (renderer == null) renderer = new LayoutComponentRenderer(getName());
      ...
      log.error(
          e,
          "Unhandled exception trying to render component \"",
          getName(),
          "\" to a string in context ",
          context.getRenderPage(),
          " -> ",
          context.getDefinitionPage());

      if (e instanceof RuntimeException) throw (RuntimeException) e;
      else throw new StripesJspException(e);
    }
  }

  /**
   * If this tag is the component that needs to be rendered, as indicated by {@link
   * LayoutContext#getComponent()}, then set the current component name back to null to indicate
  ...
  public int doEndTag() throws JspException {
    try {
      ...
      if (componentRenderPhase != null) context.setComponentRenderPhase(componentRenderPhase);
