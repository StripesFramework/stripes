...
public abstract class StripesTagSupport implements Tag {
  ...
  /**
   * A map that is used to store values of page context attributes before they were replaced with
   * other values for the body of the tag.
   */
  ...
  /**
   * Pushes new values for the attributes supplied into the page context, preserving the old values
   ...
   * pushPageContextAttributes(). Attributes that had values prior to the execution of this tag have
   * their values restored. Attributes that did not have values are removed from the page context.
   */
  ...
  protected Stack<StripesTagSupport> getTagStack() {
    Stack<StripesTagSupport> stack =
        (Stack<StripesTagSupport>)
  ...
  /**
   * Helper method that takes an attribute which may be either a String class name or a Class object
   * and returns the Class representing the appropriate ActionBean. If for any reason the Class
   * cannot be determined, or it is not an ActionBean, null will be returned instead.
   *
  ...
  protected Class<? extends ActionBean> getActionBeanType(Object nameOrClass) {
    ...
    if (nameOrClass instanceof String) {
      ...
      log.error(
          "The value supplied to getActionBeanType() was neither a String nor a "
              + "Class. Cannot infer ActionBean type from value: "
              + nameOrClass);
      return null;
    }
    ...
    if (ActionBean.class.isAssignableFrom(result)) {
      ...
      log.error(
          "Class '", result.getName(), "' specified in tag does not implement ", "ActionBean.");
      return null;
    }
  }

  /**
   * Similar to the {@link #getActionBeanType(Object)} method except that instead of returning the
   * Class of ActionBean it returns the URL Binding of the ActionBean.
   *
   * @param nameOrClass either the String FQN of an ActionBean class, or a Class object
