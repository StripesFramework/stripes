...
/**
 * The errors tag has two modes, one where it displays all validation errors in a list and a second
 * mode when there is a single enclosed field-error tag that has no name attribute in which case
 * this tag iterates over the body, displaying each error in turn in place of the field-error tag.
 *
 * <p>In the first mode, where the default output is used, it is possible to change the output for
 * the entire application using a set of resources in the error messages bundle
 ...
 * then output an unordered list of error messages. The following four resource strings (shown with
 * their default values) can be modified to create different default output:
 *
 * <ul>
 *   <li>stripes.errors.header={@literal <div class="errorHeader">Validation Errors</div><ul>}
 *   <li>stripes.errors.footer={@literal </ul>}
 *   <li>stripes.errors.beforeError={@literal <li>}
 *   <li>stripes.errors.afterError={@literal </li>}
 * </ul>
 *
 * <p>The errors tag can also be used to display errors for a single field by supplying it with a
 * 'field' attribute which matches the name of a field on the page. In this case the tag will
 * display only if errors exist for the named field. In this mode the tag will first look for
 * resources named:
 *
 * <ul>
 *   <li>stripes.fieldErrors.header
 *   <li>stripes.fieldErrors.footer
 *   <li>stripes.fieldErrors.beforeError
 *   <li>stripes.fieldErrors.afterError
 * </ul>
 *
 * <p>If the {@code fieldErrors} resources cannot be found, the tag will default to using the same
 * resources and defaults as when displaying for all fields.
 *
 * <p>Similar to the above, field specific, manner of display the errors tag can also be used to
 * output only errors not associated with a field, i.e. global errors. This is done by setting the
 * {@code globalErrorsOnly} attribute to true.
 *
 * <p>This tag has several ways of being attached to the errors of a specific action request. If the
 * tag is inside a form tag, it will display only errors that are associated with that form. If
 * supplied with an 'action' attribute, it will display errors only errors associated with a request
 * to that URL. Finally, if neither is the case, it will always display as described in the
 * paragraph above.
 *
...
public class ErrorsTag extends HtmlTagSupport implements BodyTag {
  ...
  /**
   * True if this tag will display errors, otherwise false. This is determined by the logic laid out
   * in the class level Javadoc around whether this errors tag is for the action that was submitted
   * in the request.
   */
  private boolean display = false;

  /**
   * True if this tag contains a field-error child tag, which controls the place of output of each
   * error
   */
  ...
  /**
   * Sets the action attribute by figuring out what ActionBean class is identified and then in turn
  ...
  /**
   * Determines if the tag should display errors based on the action that it is displaying for, and
   * then fetches the appropriate list of errors and makes sure it is non-empty.
   *
  ...
  public int doStartTag() throws JspException {
    ...
    if (getAction() != null) {
      if (mainBean != null) {
        String mainAction =
            StripesFilter.getConfiguration().getActionResolver().getUrlBinding(mainBean.getClass());
  ...
  /**
   * Manages iteration, running again if there are more errors to display. If there is no nested
   * FieldError tag, will ensure that the body is evaluated only once.
   *
    ...
    try {
      ...
      if (this.display && !this.nestedErrorTagPresent) {
        ...
        try {
          ...
          log.warn(
              "The errors tag could not find the error messages resource bundle. ",
