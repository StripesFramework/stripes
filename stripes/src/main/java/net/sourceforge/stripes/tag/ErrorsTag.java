...
/**
 * <p>
 * The errors tag has two modes, one where it displays all validation errors in
 * a list and a second mode when there is a single enclosed field-error tag that
 * has no name attribute in which case this tag iterates over the body,
 * displaying each error in turn in place of the field-error tag.</p>
 *
 * <p>
 * In the first mode, where the default output is used, it is possible to change
 * the output for the entire application using a set of resources in the error
 * messages bundle (StripesResources.properties unless you have configured
 * another). If the properties are undefined, the tag will output the text
 * "Validation Errors" in a div with css class errorHeader, then output an
 * unordered list of error messages. The following four resource strings (shown
 * with their default values) can be modified to create different default
 * output:</p>
 *
 * <ul>
 ...
 *
 * <p>
 * The errors tag can also be used to display errors for a single field by
 * supplying it with a 'field' attribute which matches the name of a field on
 * the page. In this case the tag will display only if errors exist for the
 * named field. In this mode the tag will first look for resources named:</p>
 *
 * <ul>
 ...
 *
 * <p>
 * If the {@code fieldErrors} resources cannot be found, the tag will default to
 * using the same resources and defaults as when displaying for all fields.</p>
 *
 * <p>
 * Similar to the above, field specific, manner of display the errors tag can
 * also be used to output only errors not associated with a field, i.e. global
 * errors. This is done by setting the {@code globalErrorsOnly} attribute to
 * true.</p>
 *
 * <p>
 * This tag has several ways of being attached to the errors of a specific
 * action request. If the tag is inside a form tag, it will display only errors
 * that are associated with that form. If supplied with an 'action' attribute,
 * it will display errors only errors associated with a request to that URL.
 * Finally, if neither is the case, it will always display as described in the
 * paragraph above.</p>
 *
...
public class ErrorsTag extends HtmlTagSupport implements BodyTag {
    ...
    /**
     * The header that will be emitted if no header is defined in the resource
     * bundle.
     */
    ...
    /**
     * The footer that will be emitted if no footer is defined in the resource
     * bundle.
     */
    public static final String DEFAULT_FOOTER = "</ul>";

    /**
     * True if this tag will display errors, otherwise false. This is determined
     * by the logic laid out in the class level Javadoc around whether this
     * errors tag is for the action that was submitted in the request.
     */
    private boolean display = false;

    /**
     * True if this tag contains a field-error child tag, which controls the
     * place of output of each error
     */
    ...
    /**
     * An optional attribute that declares a particular field to output errors
     * for.
     */
    ...
    /**
     * Called by the IndividualErrorTag to fetch the current error from the set
     * being iterated.
     *
    ...
    /**
     * Sets the (optional) action of the form to display errors for, if they
     * exist.
     * @param action
     */
    ...
    /**
     * Sets the action attribute by figuring out what ActionBean class is
     * identified and then in turn finding out the appropriate URL for the
     * ActionBean.
     *
    ...
    /**
     * Sets the (optional) name of a field to display errors for, if errors
     * exist.
     * @param field
     */
    ...
    public boolean isGlobalErrorsOnly() {
        return globalErrorsOnly;
    }

    /**
     * Tells the tag to display (or not) only global errors and no field level
     * errors.
     * @param globalErrorsOnly
     */
    ...
    /**
     * Determines if the tag should display errors based on the action that it
     * is displaying for, and then fetches the appropriate list of errors and
     * makes sure it is non-empty.
     *
    ...
    /**
     * Manages iteration, running again if there are more errors to display. If
     * there is no nested FieldError tag, will ensure that the body is evaluated
     * only once.
     *
    ...
    /**
     * Output the error list if this was an empty body tag and we're fully
     * controlling output*
     *
    ...
    /**
     * Inner class Comparator used to provide a consistent ordering of
     * validation errors. Sorting is done by field name (the programmatic one,
     * not the user visible one). Errors without field names sort to the top
     * since it is assumed that these are global errors as oppose to field
     * specific ones.
     */
