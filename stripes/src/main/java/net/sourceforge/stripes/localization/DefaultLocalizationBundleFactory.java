...
/**
 * Very simple default implementation of a bundle factory. Looks for configuration parameters in the
 * bootstrap properties called "LocalizationBundleFactory.ErrorMessageBundle" and
 * "LocalizationBundleFactory.FieldNameBundle". If one or both of these values is not specified the
 * default bundle name of "StripesResources" will be used in its place.
 *
...
public class DefaultLocalizationBundleFactory implements LocalizationBundleFactory {
   ...
   * configured bundle names in the servlet init parameters etc. If those can't be found then the
   * default bundle names are put in place.
   */
  public void init(Configuration configuration) throws Exception {
    setConfiguration(configuration);

    this.errorBundleName =
        configuration.getBootstrapPropertyResolver().getProperty(ERROR_MESSAGE_BUNDLE);
    if (this.errorBundleName == null) {
      this.errorBundleName = BUNDLE_NAME;
    }

    this.fieldBundleName =
        configuration.getBootstrapPropertyResolver().getProperty(FIELD_NAME_BUNDLE);
  ...
  /**
   * Looks for a bundle called StripesResources with the supplied locale if one is provided, or with
   * the default locale if the locale provided is null.
   *
