...
/**
 * Provides simple localization utility methods that are used in multiple places
 * in the Stripes code base.
 *
 * @author Tim Fennell
 * @since Stripes 1.1
 */
public class LocalizationUtility {
    ...
    /**
     * <p>
     * Fetches the localized name for a form field if one exists in the form
     * field resource bundle. If for any reason a localized value cannot be
     * found (e.g. the bundle cannot be found, or does not contain the required
     * properties) then null will be returned.</p>
     *
     * <p>
     * Looks first for a property called {@code beanClassFQN.fieldName} in the
     * resource bundle. If that is undefined, it next looks for
     * {@code actionPath.fieldName} and if not defined, looks for a property
     * called {@code fieldName}. Will strip any indexing from the field name
     * prior to using it to construct property names (e.g. foo[12] will become
     * simply foo).</p>
     *
     * @param fieldName The name of the field whose localized name to look up
     * @param actionPath The action path of the form in which the field is
     * nested. If for some reason this is not available, null may be passed
    ...
    /**
     * Makes a half hearted attempt to convert the property name of a field into
     * a human friendly name by breaking it on periods and upper case letters
     * and capitalizing each word. This is only used when developers do not
     * provide names for their fields.
     *
     * @param fieldNameKey the programmatic name of a form field
     * @return String a more user friendly name for the field in the absence of
     * anything better
     */
     ...
     * This is usually the same as {@link Class#getSimpleName()}, but static
     * inner classes are handled such that the simple name is
     * {@code OuterClass.InnerClass}. Multiple layers of nesting are supported.
     *
