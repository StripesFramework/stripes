...
/**
 * Extremely simple interface that is implemented to resolve the ResourceBundles from which various
 * Strings are pulled at runtime. Can be implemented using property resource bundles, class resource
 * bundles or anything else a developer can dream up.
 *
...
public interface LocalizationBundleFactory extends ConfigurableComponent {
  ...
  /**
   * Returns the ResourceBundle from which to draw the names of form fields for the specified
   * locale.
   *
   * @param locale the locale that is in use for the current request
