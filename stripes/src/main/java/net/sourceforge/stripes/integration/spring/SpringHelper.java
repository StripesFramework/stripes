...
/**
 * Static helper class that is used to lookup Spring beans and inject them into objects (often
 * ActionBeans). Is capable of injecting beans through setter methods (property access) and also
 * through direct field access if the security policy allows it. Methods and fields must be
 * annotated using the {@code @SpringBean} annotation.
 *
 * <p>Methods and fields may be public, protected, package-access or private. If they are not public
 * an attempt is made to call {@link Method#setAccessible(boolean)} in order to make them accessible
 * from this class. If the attempt fails, an exception will be thrown.
 *
 * <p>Method names can take any form. For example {@code setSomeBean(Bean b)} or {@code
 * someBean(bean b)}. In both cases, if a specific SpringBean name is not supplied, the default name
 * of {@code someBean} will be used.
 *
 * <p>The value of the {@code @SpringBean} annotation should be the bean name in the Spring
 * application context if it is different from the field/property name. If the value is left blank,
 * an attempt is made to auto-wire the bean; first by field/property name and then by type. If the
 * value is left blank and more than one bean of the same type is found, an exception will be
 * raised.
 *
...
public class SpringHelper {
  ...
  private static final Map<Class<?>, Collection<Method>> methodMap = new ConcurrentHashMap<>();

  /** Lazily filled in map of Class to fields annotated with SpringBean. */
  private static final Map<Class<?>, Collection<Field>> fieldMap = new ConcurrentHashMap<>();

  /**
   * Injects Spring managed beans into using a Web Application Context that is derived from the
   * ServletContext, which is in turn looked up using the ActionBeanContext.
   *
  ...
  public static void injectBeans(Object bean, ServletContext ctx) {
    ApplicationContext ac =
        (ApplicationContext)
            ctx.getAttribute("org.springframework.web.context.WebApplicationContext.ROOT");
    ...
    for (Method m : getMethods(bean.getClass())) {
      try {
        ...
        throw new StripesRuntimeException(
            "Exception while trying to lookup and inject "
   ...
   * for a particular class it will introspect the class and cache the results. All non-overridden
   * methods are examined, including protected and private methods. If a method is not public an
   * attempt it made to make it accessible - if it fails it is removed from the collection and an
   * error is logged.
   *
   ...
   * for a particular class it will introspect the class and cache the results. All non-overridden
   * fields are examined, including protected and private fields. If a field is not public an
   * attempt it made to make it accessible - if it fails it is removed from the collection and an
   * error is logged.
   *
  ...
  /**
   * Looks up a Spring managed bean from an Application Context. First looks for a bean with name
   * specified. If no such bean exists, looks for a bean by type. If there is only one bean of the
   * appropriate type, it is returned. If zero or more than one bean of the correct type exists, an
   * exception is thrown.
   *
   * @param ctx the Spring Application Context
   ...
   *     supplied.
   */
  public static Object findSpringBean(
      ApplicationContext ctx, String name, Class<?> type, boolean allowFindByType) {
    ...
    if (beanNames.length == 0) {
      throw new StripesRuntimeException(
          "Unable to find SpringBean with name ["
              + name
              + "] or type ["
              + type.getName()
      ...
      throw new StripesRuntimeException(
          "Unable to find SpringBean with name ["
              + name
              + "] or unique bean with type ["
              + type.getName()
              + "] in the Spring application context. Found "
              + beanNames.length
              + "beans of matching type.");
    } else {
      log.debug(
          "Found unique SpringBean with type [" + type.getName() + "]. Matching on ",
   ...
   * Assumes that the name is in fact a mutator for a property and will do the usual {@code setFoo}
   * to {@code foo} conversion if the method follows the normal syntax, otherwise will just return
   * the method name.
   *
