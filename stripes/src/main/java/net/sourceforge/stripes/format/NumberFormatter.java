 ...
 *   <li>integer - Outputs text with grouping characters and no decimals.
 *   <li>decimal - Outputs text with grouping characters and 2-6 decimal positions as needed.
 * </ul>
 *
 * @author Tim Fennell
 */
  ...
  public void init() {
    ...
    if (this.formatType.equalsIgnoreCase("number")) {
      ...
      throw new StripesRuntimeException(
          "Invalid format type supplied for formatting a "
              + "number: "
              + this.formatType
              + ". Valid values are 'number', 'currency' "
              + "and 'percentage'.");
    }
    ...
    if (this.formatPattern.equalsIgnoreCase("plain")) {
      ...
      try {
        ...
        throw new StripesRuntimeException(
            "Custom pattern could not be applied to "
                + "NumberFormat instance.  Pattern was: "
                + this.formatPattern,
            e);
      }
    }
