...
public class PropertyExpressionEvaluationTests extends FilterEnabledTestBase {
  ...
  public void testGetBasicPropertyType() {
    ...
    PropertyExpressionEvaluation eval =
        new PropertyExpressionEvaluation(expr, new TestActionBean());
  ...
  public void testGetNestedProperties() throws Exception {
    ...
    Assert.assertEquals(
        TestEnum.Seventh, BeanUtil.getPropertyValue("nestedBean.enumProperty", root));
  }
  ...
  public void testListProperties() throws Exception {
    ...
    for (int i = 0; i < 10; ++i) {
      if (i % 2 == 0) {
        Assert.assertNull(
            "String list index " + i + " should be null.", root.getStringList().get(i));
      } else {
        Assert.assertEquals(
            "String list index " + i + " should be 'testValue'.",
            "testValue",
            root.getStringList().get(i));
      }
    }
  }
  ...
  /**
   * Tests a bug whereby the Introspector/PropertyDescriptor returns inaccessible methods for
   * getKey() and getValue() on all the JDK implementations of Map. A general fix has been
   * implmented to work up the chain and find an accessible method to invoke.
   */
  @Test
  ...
  /**
   * Fix for a problem whereby in certain circumstances the PropertyExpressionEvaluation would not
   * fall back to looking at instance information to figure out the type info.
   */
  @Test
