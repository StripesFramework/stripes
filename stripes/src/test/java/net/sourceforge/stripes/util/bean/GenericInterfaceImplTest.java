...
/**
 * Tests a corner cases where a property's getter and/or setter method(s) are specified in an
 * interface using a type parameter. In this case the compiler generates bridge method(s) that wrap
 * the actual implementations of of the interface methods, and have erased types! This test ensures
 * that BeanUtil does the right thing and finds the non-bridge method(s) and determines their types.
 *
 * @author Tim Fennell
 */
public class GenericInterfaceImplTest extends FilterEnabledTestBase {
  ...
  public static class GenericImpl implements GenericInterface<GenericImpl> {
    ...
    public GenericImpl getProp() {
      return prop;
    }

    public void setProp(GenericImpl prop) {
      this.prop = prop;
    }
  ...
  public static class WriteOnlyGenericImpl
      implements WriteOnlyGenericInterface<WriteOnlyGenericImpl> {
