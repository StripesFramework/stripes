...
/**
 * Tests for various methods in the NameBasedActionResolver that can be tested in isolation. The
 * resolver is also tested by a lot of the mock tests that run from request through the action
 * layer.
 *
 * @author Tim Fennell
 */
public class NameBasedActionResolverTest {
  private NameBasedActionResolver resolver =
      new NameBasedActionResolver() {
        @Override
  ...
  public void generateBindingWithMultipleBasePackages2() {
    String binding =
        this.resolver.getUrlBinding("foo.web.stripes.www.admin.ControlCenterActionBean");
    Assert.assertEquals(binding, "/admin/ControlCenter.action");
  }
  ...
  public void testWithAnnotatedClass() {
    ...
    Assert.assertEquals(
        binding,
        net.sourceforge.stripes.test.TestActionBean.class.getAnnotation(UrlBinding.class).value());
  }
