...
/**
 * Tests all reasonable variations of binding involving Maps. String keys, numeric keys, enum keys,
 * date keys (!). Maps as terminal values (foo[bar]) and maps in the middle of property chains
 * (foo[bar].splat).
 *
 * @author Tim Fennell
 */
public class MapBindingTests extends FilterEnabledTestBase implements ActionBean {
  ...
  public void writeThroughBeanInMap() throws Exception {
    ...
    Assert.assertEquals(
        bean.getMapEnumTestBean().get(Color.red).getLongProperty(), Long.valueOf(1L));
    Assert.assertEquals(
        Integer.valueOf(bean.getMapEnumTestBean().get(Color.red).getIntProperty()),
        Integer.valueOf(2));
    Assert.assertEquals(
        bean.getMapEnumTestBean().get(Color.green).getLongProperty(), Long.valueOf(3L));
    Assert.assertEquals(
        Integer.valueOf(bean.getMapEnumTestBean().get(Color.green).getIntProperty()),
        Integer.valueOf(4));
  }
  ...
  public void bindThroughTypelessMap() throws Exception {
    ...
    MapBindingTests bean = trip.getActionBean(MapBindingTests.class);
    Assert.assertEquals(
        ((TestBean) bean.getTypelessMap().get(1)).getLongProperty(), Long.valueOf(1234L));
    Assert.assertEquals(
        ((TestBean) bean.getTypelessMap().get(2l)).getNestedBean().getLongProperty(),
        Long.valueOf(4321L));
    Assert.assertEquals(
        ((TestBean) bean.getTypelessMap().get("foo")).getEnumProperty(), TestEnum.Sixth);
  }
}
