...
public class DefaultTypeConverterFactoryTest {
  ...
  public abstract static class BaseTC<T> implements TypeConverter<T> {
    public T convert(
        String input, Class<? extends T> targetType, Collection<ValidationError> errors) {
      return null;
    }
  ...
  protected void checkTypeConverter(
      TypeConverterFactory factory, Class<?> targetType, Class<?> expect) throws Exception {
    log.debug(
        "Checking type converter for ",
