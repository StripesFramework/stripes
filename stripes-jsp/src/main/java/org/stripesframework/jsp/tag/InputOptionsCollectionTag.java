...
import org.stripesframework.web.util.bean.BeanComparator;
...
public class InputOptionsCollectionTag extends HtmlTagSupport {
   ...
   private final HtmlTagSupport _optgroupSupport = new HtmlTagSupport() {
   ...
   private Boolean       _localizeLabels;
   ...
   public int doEndTag() throws JspException {
      ...
      List<Entry> sortedEntries = new LinkedList<>(_entries);
      if ( _sort != null ) {
         String[] props = StringUtil.standardSplit(_sort);
         ...
         Collections.sort(sortedEntries, new BeanComparator(getPageContext().getRequest().getLocale(), props));
      }
      ...
      for ( Entry entry : sortedEntries ) {
         ...
         try {
            if ( entry.group != null && !entry.group.equals(lastGroup) ) {
               if ( lastGroup != null ) {
                  _optgroupSupport.writeCloseTag(out, "optgroup");
               }

               _optgroupSupport.set("label", String.valueOf(entry.group));
               _optgroupSupport.writeOpenTag(out, "optgroup");
   ...
   public int doStartTag() throws JspException {
      ...
      try {
         ...
         throw new StripesJspException(
               "A problem occurred generating an options-collection. " + "Most likely either [" + labelProperty + "] or [" + valueProperty + "] is not a "
                     + "valid property of the beans in the collection: " + _collection, ee);
      }
   ...
   public void setCollection( Object in ) {
      if ( in == null ) {
         ...
         _collection = (Collection)in;
      } else if ( in instanceof Iterable ) {
         _collection = CollectionUtil.asList((Iterable)in);
      } else if ( in.getClass().isArray() ) {
         _collection = CollectionUtil.asList(in);
      } else {
   ...
   protected void addEntry( Object item, Object label, Object value ) {
      _entries.add(new Entry(item, label, value, null));
   }
   ...
   protected void addEntry( Object item, Object label, Object value, Object group ) {
      _entries.add(new Entry(item, label, value, group));
   }

   protected boolean isAttemptToLocalizeLabels() {
      return (_localizeLabels == null) || (_localizeLabels != null && _localizeLabels.booleanValue());
   }
