...
import org.stripesframework.web.ajax.JavaScriptBuilder;
...
public class FieldMetadataTag extends HtmlTagSupport implements BodyTag {
   ...
   public void setAction( String action ) {
      ...
      if ( binding != null ) {
         _actionWithoutContext = binding;
      } else {
         _actionWithoutContext = action;
      }
   }
   ...
   private String getMetadata() {
      ...
      if ( (_fields != null) && (_fields.trim().length() > 0) ) {
         fields.addAll(Arrays.asList(_fields.split(",")));
      } else if ( form == null ) {
      ...
      for ( String field : fields ) {
         ...
         if ( fieldType.isPrimitive() || Number.class.isAssignableFrom(fieldType) || Date.class.isAssignableFrom(fieldType) || _includeType ) {
            fieldInfo.append("type:").append(JavaScriptBuilder.quote(_fqn ? fieldType.getName() : fieldType.getSimpleName()));
         }
         ...
         if ( data != null ) {
            ...
            if ( data.label() == null ) {
               label = LocalizationUtility.getLocalizedFieldName(field, form == null ? null : form.getAction(), form == null ? null : form.getActionBeanClass(),
                     locale);
            }
            if ( label != null ) {
         ...
         if ( typeConverterClass == null ) {
            try {
               typeConverterClass = StripesFilter.getConfiguration()
                     .getTypeConverterFactory()
                     .getTypeConverter(fieldType, _pageContext.getRequest().getLocale())
                     .getClass();
            }
            catch ( Exception e ) {
               // Just ignore it
         ...
         if ( typeConverterClass != null ) {
            fieldInfo.append(fieldInfo.length() > 0 ? "," : "")
                  ...
                  .append(JavaScriptBuilder.quote(_fqn ? typeConverterClass.getName() : typeConverterClass.getSimpleName()));
         }
