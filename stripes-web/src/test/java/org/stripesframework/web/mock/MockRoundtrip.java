...
public class MockRoundtrip {
   ...
   public MockRoundtrip( MockServletContext context, String actionBeanUrl, MockHttpSession session ) {
      ...
      request = new MockHttpServletRequest("/" + context.getServletContextName(), path);
   ...
   public void addParameter( String name, String... value ) {
      if ( request.getParameterValues(name) == null ) {
         ...
         String[] oldValues = request.getParameterMap().get(name);
   ...
   public <A extends ActionBean> A getActionBean( Class<A> type ) {
      A bean = (A)request.getAttribute(getUrlBinding(type, context));
      if ( bean == null ) {
         bean = (A)request.getSession().getAttribute(getUrlBinding(type, context));
      }
      return bean;
   }
   ...
   public String getDestination() {
      String forward = request.getForwardUrl();
   ...
   public byte[] getOutputBytes() {
      return response.getOutputBytes();
   }
