...
public class PropertyExpressionEvaluation {
   ...
   private final PropertyExpression _expression;
   private final Object             _bean;
   private       NodeEvaluation     _root;
   private       NodeEvaluation     _leaf;
   ...
   public PropertyExpressionEvaluation( PropertyExpression expression, Object bean ) {
      ...
      for ( Node node = expression.getRootNode(); node != null; node = node.getNext() ) {
         ...
         if ( _root == null ) {
            ...
            _leaf.setNext(evaluation);
            evaluation.setPrevious(_leaf);
            _leaf = evaluation;
         }
      }
    ...
    */
   public Object getBean() { return _bean; }
   ...
   public Class<?> getScalarType() {
      Type type = _leaf.getValueType();
      Class<?> clazz = convertToClass(type, _leaf);

      if ( clazz.isArray() ) {
         return clazz.getComponentType();
      } else if ( Collection.class.isAssignableFrom(clazz) ) {
         if ( type instanceof ParameterizedType ) {
            return convertToClass(((ParameterizedType)type).getActualTypeArguments()[0], _leaf);
         } else {
            return String.class;
         }
      } else if ( Map.class.isAssignableFrom(clazz) ) {
         if ( type instanceof ParameterizedType ) {
            return convertToClass(((ParameterizedType)type).getActualTypeArguments()[1], _leaf);
         } else {
            return String.class;
         }
      } else {
   ...
   public Class<?> getType() {
      return convertToClass(_leaf.getValueType(), _leaf);
   }
   ...
   public Object getValue() {
      Object nodeValue = _bean;
      for ( NodeEvaluation node = _root; node != null && nodeValue != null; node = node.getNext() ) {
   ...
   public void setToNull() throws EvaluationException {
      Object nodeValue = _bean;
      for ( NodeEvaluation node = _root; node != _leaf && nodeValue != null; node = node.getNext() ) {
         nodeValue = node.getType().getPropertyAccessor().getValue(node, nodeValue);
      }

      if ( nodeValue != null ) {
         Class leafType = convertToClass(_leaf.getValueType(), _leaf);
         if ( Map.class.isAssignableFrom(leafType) || Collection.class.isAssignableFrom(leafType) ) {
            nodeValue = _leaf.getType().getPropertyAccessor().getValue(_leaf, nodeValue);
            ...
            try {
               ...
               _leaf.getType().getPropertyAccessor().setValue(_leaf, nodeValue, nvl);
            }
            catch ( RuntimeException re ) {
               throw re;
            }
            catch ( Exception e ) {
               throw new EvaluationException("Could not set a null value for property '" + _expression + "' on bean of type " + _bean.getClass().getName(), e);
            }
         }
      }
   }
   ...
   public void setValue( Object propertyValue ) throws EvaluationException {
      ...
      _leaf.getType().getPropertyAccessor().setValue(_leaf, nodeValue, propertyValue);
   }
   ...
   protected Type getTypeViaInstances( NodeEvaluation end ) throws EvaluationException, NoSuchPropertyException {
      ...
      for ( NodeEvaluation node = _root; node != end; node = node.getNext() ) {
         PropertyAccessor accessor = node.getType().getPropertyAccessor();
         previous = value;
      ...
      if ( value instanceof Map ) {
         ...
         if ( value != null ) {
            ...
            throw new EvaluationException(
                  "Not enough type information available to " + "evaluate expression. Expression: '" + _expression + "'. Type information ran "
                        ...
                        + "generic type information or that it contains a value with the key type " + end.getNode().getTypedValue().getClass().getName()
                        + " and value " + end.getNode().getStringValue());
         }
         ...
         throw new EvaluationException(
               "Not enough type information available to " + "evaluate expression. Expression: '" + _expression + "'. Type information ran " + "out at node '"
                     + end.getNode().getStringValue() + "', which represents a List "
                     + "entry. Please ensure that either the getter for the List contains appropriate "
                     + "generic type information or that the index is numeric and a value exists at " + "the supplied index (" + end.getNode().getStringValue()
                     + ").");
      } else {
         ...
         if ( type != null ) {
            ...
            throw new NoSuchPropertyException(
                  "Bean class " + previous.getClass().getName() + " does not contain a property called '" + end.getNode().getStringValue()
                        + "'. As a result the following expression could not be evaluated: " + _expression);
         }
      }
   }
      ...
      try {
         ...
         if ( m.getParameterTypes().length == 1 ) { // deal with set methods
            ...
            for ( Method m2 : m.getDeclaringClass().getMethods() ) {
               if ( name.equals(m2.getName()) && m2 != m && m2.getParameterTypes().length == m.getParameterTypes().length ) {
                  return m2;
               }
            }
   ...
   void fillInTypeInformation() {
      Type type = _bean.getClass();

      for ( NodeEvaluation current = _root; current != null; current = current.getNext() ) {
   ...
   private Object getDefaultValue( NodeEvaluation node ) throws EvaluationException {
      try {
         ...
         throw new EvaluationException(
               "Encountered an exception while trying to create " + " a default instance for property '" + node.getNode().getStringValue() + "' in "
                     + "expression '" + _expression.getSource() + "'.", e);
      }
   }
}
