...
public class DefaultExceptionHandler implements ExceptionHandler {
   ...
   private final Map<Class<? extends Throwable>, HandlerProxy> _handlers = new HashMap<>();
   ...
   public void handle( Throwable throwable, HttpServletRequest request, HttpServletResponse response ) throws ServletException, IOException {
      try {
         ...
         if ( proxy != null ) {
            ...
            Resolution resolution = handle((FileUploadLimitExceededException)throwable, request, response);
            if ( resolution != null ) {
   ...
   protected void addHandler( Object handler ) throws Exception {
      ...
      for ( Method method : methods ) {
         ...
         if ( handler == this && method.getName().equals("handle") && Throwable.class.equals(parameters[0]) ) {
            continue;
         }
    ...
    */
   protected Resolution handle( SourcePageNotFoundException exception, HttpServletRequest request, HttpServletResponse response ) throws Exception {
   ...
   protected static class HandlerProxy {
      ...
      public void handle( Throwable t, HttpServletRequest req, HttpServletResponse res ) throws Exception {
         try {
            Object resolution = handlerMethod.invoke(handler, t, req, res);
