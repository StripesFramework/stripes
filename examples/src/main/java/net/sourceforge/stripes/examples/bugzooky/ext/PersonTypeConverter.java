 ...
 * PersonManager} for a {@link Person} object with that ID. If no {@link Person} with the ID is
 * found, then it simply returns null. If the input string cannot be parsed as an integer, then this
...
public class PersonTypeConverter implements TypeConverter<Person> {
  ...
  public Person convert(
      String input, Class<? extends Person> targetType, Collection<ValidationError> errors) {
    Person person = null;
