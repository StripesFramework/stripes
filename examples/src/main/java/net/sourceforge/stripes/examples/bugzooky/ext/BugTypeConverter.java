 ...
 * BugManager} for a {@link Bug} object with that ID. If no {@link Bug} with the ID is found, then
 * it simply returns null. If the input string cannot be parsed as an integer, then this {@link
 * TypeConverter} adds a validation error and returns null.
 *
 * @author Ben Gunter
 */
public class BugTypeConverter implements TypeConverter<Bug> {
  /**
   * Attempt to parse the input string to an integer and look up the {@link Bug} with that ID using
   * a {@link BugManager}.
   *
   ...
   */
  public Bug convert(
      String input, Class<? extends Bug> targetType, Collection<ValidationError> errors) {
    Bug bug = null;
