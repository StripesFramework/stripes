...
/**
 * ActionBean that provides method for editing a single bug in detail. Includes an event for
 * pre-populating the ActionBean on the way in to an edit screen, and a single event for saving an
 * existing or new bug. Uses a FileBean property to support the uploading of a File concurrent with
 * other edits.
 *
 * @author Tim Fennell
 */
public class SingleBugActionBean extends BugzookyActionBean {
  ...
  public Resolution save() throws IOException {
    ...
    if (bug.getOpenDate() == null) bug.setOpenDate(new Date());
