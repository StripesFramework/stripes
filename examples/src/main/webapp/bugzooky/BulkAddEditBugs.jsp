		...
		<stripes:form beanclass="net.sourceforge.stripes.examples.bugzooky.MultiBugActionBean" focus="">
			...
			<table class="display">
				...
				<c:forEach items="${list}" var="bug" varStatus="loop">
					<tr>
						<td>${bug.id} <stripes:hidden name="bugs[${loop.index}]" /></td>
						<td><stripes:select name="bugs[${loop.index}].component">
						...
						<td><stripes:select name="bugs[${loop.index}].owner">
							<stripes:option value="">Select One</stripes:option>
							<stripes:options-collection collection="${personManager.allPeople}" label="username" value="id" />
						</stripes:select></td>
						<td><stripes:select name="bugs[${loop.index}].priority">
							<stripes:option value="">Select One</stripes:option>
